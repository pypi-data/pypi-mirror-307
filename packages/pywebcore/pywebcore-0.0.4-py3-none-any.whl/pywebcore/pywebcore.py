    ...
    def create_fea_section(self, meshsize, coarse=True):
        ...
        for member in self.members:
            ...
            try:
                ...
                nu = member.material.nu
                G = member.material.G
            m = SectionMaterial(name=member.material.name, elastic_modulus=E, poissons_ratio=nu,
...
def plot_section(section, plt=None):
    import plotly.graph_objects as go
    if plt is None:
        plt = go.Figure()

    solid_line = dict(line={"color": "black", "width": 2}, mode="lines")
    dash_line = dict(line={"color": "black", "width": 1, "dash": "dash"}, mode="lines")

    def scatter(points, **kwargs):
        try:
            return go.Scatter(x=[p.y for p in points], y=[p.z for p in points], **kwargs)
        except AttributeError:
            return go.Scatter(x=[p[0] for p in points], y=[p[1] for p in points], **kwargs)

    data = [scatter(m.points, fill="toself", fillcolor=None, **solid_line) for m in section.members]
    data.append(scatter([(-section.W * 0.55, section.z), (section.W * 0.55, section.z)], **dash_line))  # neutral axis

    for d in data:
        plt.add_trace(d)
    plt.layout.yaxis.scaleanchor = "x"
    plt.layout.showlegend = False
    plt.layout.template = 'simple_white'
    return plt
