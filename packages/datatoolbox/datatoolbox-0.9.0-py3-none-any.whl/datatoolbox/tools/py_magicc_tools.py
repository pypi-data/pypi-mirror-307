...
def writeMagic6ScenFile(table, filePath):
    ...
    headers['variables'] = pym.io.convert_magicc6_to_magicc7_variables(
        list(efCopy.columns)
    )
...
def run_MAGGIC6(scenFile):
    ...
    outTemperature = dt.io.read_MAGICC6_MATLAB_bulkout(
        scenario + '/CSSENS_DAT_SURFACE_TEMP_BOXGLOBAL_MAGICC_INDIVIDUAL.OUT'
    )
    ...
    print('p66: ' + str(outTemperature.loc[2100, :].quantile(0.66)))
    print('p16: ' + str(outTemperature.loc[2100, :].quantile(0.16)))
...
if __name__ == '__main__':
    ...
    for scen in [
        'WEO_2019_test.SCEN'
    ]:  # ['IEA B2DS 4', 'IEA B2DS 2 rev', 'IEA B2DS 3']:
        ...
        plt.plot(
            outTemperature.loc[outTemperature.index[::5], :].quantile(0.66, axis=1)
        )
        plt.plot(outTemperature.loc[outTemperature.index[::5], :].quantile(0.5, axis=1))
        plt.plot(
            outTemperature.loc[outTemperature.index[::5], :].quantile(0.14, axis=1)
        )

        outDf = pd.DataFrame(columns=outTemperature.index[::5])
        outDf.loc['p84', :] = outTemperature.loc[outTemperature.index[::5], :].quantile(
            0.84, axis=1
        )
        outDf.loc['p75', :] = outTemperature.loc[outTemperature.index[::5], :].quantile(
            0.75, axis=1
        )
        outDf.loc['p66', :] = outTemperature.loc[outTemperature.index[::5], :].quantile(
            0.66, axis=1
        )
        outDf.loc['p50', :] = outTemperature.loc[outTemperature.index[::5], :].quantile(
            0.5, axis=1
        )
        outDf.loc['p36', :] = outTemperature.loc[outTemperature.index[::5], :].quantile(
            0.36, axis=1
        )
        outDf.loc['p25', :] = outTemperature.loc[outTemperature.index[::5], :].quantile(
            0.25, axis=1
        )
        outDf.loc['p16', :] = outTemperature.loc[outTemperature.index[::5], :].quantile(
            0.16, axis=1
        )
