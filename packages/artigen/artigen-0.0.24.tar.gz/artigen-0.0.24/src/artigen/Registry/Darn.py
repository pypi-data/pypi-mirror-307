...
class Darn(BaseRegistry):
    ...
    async def create_new_darn(self, name, description=None, tags=None, file_paths=None):
        ...
        try:
            ...
            if any(darn.get('name') == name for darn in existing_darns):
            ...
            parent_pin_response = await self._sdk_request(
                ...
                method="POST",
                data={"type": "session_voice", "sub_type": "test"}
            )
            ...
            for file_path in file_paths:
                ...
                child_darn_response = await self._sdk_request(
                    ...
                    data={
                        ...
                        "tags": tags,
                        "details": {
                            "file_id": file_metadata.get("id")  # Use actual file ID
                        },
                        "metadata": {
                ...
                children_metadata.append({
                    "pin_no": child_pin,
                    "type": "local",
                    "connection_id": "06610099-e7e4-4872-be41-8f79dd502aad",
                    "details": [
                        {
                            "database_name": "Ecommerce",
                            "tables": [os.path.basename(file_path)],
                            "size": len(file_metadata),
                            "type": "table"
                        }
                    ]
                })
