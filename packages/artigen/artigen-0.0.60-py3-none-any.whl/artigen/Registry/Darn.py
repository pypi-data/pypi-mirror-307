    ...
    async def create_new_darn(self, name, description=None, tags=None, file_paths=None):
        ...
        try:
            ...
            for file_path in file_paths:
                ...
                async with httpx.AsyncClient() as client:
                    with open(file_path, 'rb') as file:
                        file_upload_response = await client.post(
                            ...
                            headers=self.headers,  # Ensure all necessary headers are included
                            files={"file": (os.path.basename(file_path), file)},
                            data={"type": "credentials"}
                        )
                # print("Response status:", file_upload_response.status_code)
                # print("Response content:", file_upload_response.text)

                if file_upload_response.status_code != 200:
                    return {"status": "error", "message": "Failed to upload file_1"}
                ...
                #     return {"status": "error", "message": "Failed to upload file_2"}
        #
        #         # Create a child DARN with the file metadata
        #         child_darn_response = await self._sdk_request(
        #             service_name="darn-service",
        #             endpoint="create-child-darn",
        #             method="POST",
        #             data={
        #                 "name": f"{name}:{os.path.basename(file_path)}",
        #                 "parent": [darn_id],
        #                 "pin_no": [child_pin],
        #                 "tags": tags,
        #                 # "details": {
        ...
        #             return {"status": "error", "message": "Failed to create child DARN_2"}
        #
