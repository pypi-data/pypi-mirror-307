...
from ..errors import MemorySizeError, InputConflictError, InputMissingError, DimensionError
...
def nest_shape(nested_list, is_element=looks_flat):
    ...
    while not is_element(current):
        if current[0] is current:
            raise DimensionError('nest_shape failed: obj[0] is obj. Crash to avoid infinite loop.')
        l = len(current)
        shape.append(l)
        current = current[0]
    return shape