...
from nlpx.training import is_float, convert_data, convert_data_acc, evaluate, Trainer, SimpleTrainer, EvalTrainer, SimpleEvalTrainer, \
...
class TextModelWrapper(SimpleClassModelWrapper):
	...
	def train(self, texts: Union[Collection[str], np.ndarray, pd.Series],
		...
		super().train(X, y, val_data, collate_fn, epochs, optimizer, scheduler, learning_rate, T_max,
...
class SplitPaddingTextModelWrapper(PaddingTextModelWrapper):
	...
	def train(self, texts: Union[Collection[str], np.ndarray, pd.Series],
		...
		val_data = (X_test, y_test)
		super().train(X_train, y_train, val_data, max_length, epochs, optimizer, scheduler, learning_rate,
...
class RegressModelWrapper(ModelWrapper):
	...
	def mse(self, X: Union[torch.Tensor, np.ndarray, list[float]], y: Union[torch.Tensor, np.ndarray, list[float]]) -> float:
		if isinstance(X, (List, np.ndarray)):
			X = torch.tensor(X, dtype=torch.float) if is_float(X[0][0]) else torch.tensor(X, dtype=torch.long)
