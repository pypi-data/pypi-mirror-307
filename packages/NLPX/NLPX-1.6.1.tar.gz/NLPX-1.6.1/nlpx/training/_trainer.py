...
from .utils import train_epoch, train_epoch_acc, evaluate, acc_evaluate, r2_evaluate, train_epoch_r2
...
class Trainer:
	...
	def print(self, show_progress, epoch, lr, loss):
		if show_progress:
			log_utils.info(f'Epoch-{epoch}/{self.max_iter}  lr: {lr:.6f}, train_loss: {loss:.4f}\n')
		else:
			log_utils.info(f'Epoch-{epoch}/{self.max_iter}  lr: {lr:.6f}, train_loss: {loss:.4f}')
...
class EvalTrainer(Trainer):
	...
	def print(self, show_progress, epoch, lr, loss, val_loss):
		if show_progress:
			log_utils.info(f'Epoch-{epoch}/{self.max_iter}  lr: {lr:.6f}, train_loss: {loss:.4f}, val_loss: {val_loss:.4f}\n')
		else:
			log_utils.info(f'Epoch-{epoch}/{self.max_iter}  lr: {lr:.6f}, train_loss: {loss:.4f}, val_loss: {val_loss:.4f}')
...
class SimpleEvalTrainer(EvalTrainer):
	...
	def train(self, model, X, y, val_data: Tuple, collate_fn=None, show_progress=False, eps=1e-5):
		...
		if isinstance(y_val, (List, np.ndarray)):
			y_val = torch.LongTensor(y_val) if isinstance(y_val[0], int) else torch.FloatTensor(y_val)
...
class RegressTrainer(Trainer):
	...
	def train(self, model, train_set: Dataset, collate_fn=None, show_progress=False, eps=1e-5):
		...
		best_model = None
		min_loss = float('inf')
		cnt, cnt2, best_r2, last_r2 = 0, 0, 0.0, 0.0
		optimizer, scheduler = self.get_optimizer_scheduler(model)
	...
	def print(self, show_progress, epoch, lr, loss, r2):
		if show_progress:
			log_utils.info(f'Epoch-{epoch}/{self.max_iter}  lr: {lr:.6f}, train_loss: {loss:.4f}, train_r2: {r2:.4f}\n')
		else:
			log_utils.info(f'Epoch-{epoch}/{self.max_iter}  lr: {lr:.6f}, train_loss: {loss:.4f}, train_r2: {r2:.4f}')
...
class EvalRegressTrainer(Trainer):
	...
	def print(self, show_progress, epoch, lr, loss, r2, val_loss, val_r2):
		if show_progress:
			log_utils.info(
				f'Epoch-{epoch}/{self.max_iter}  lr: {lr:.6f}, train_loss: {loss:.4f}, train_r2: {r2:.4f}, '
