...
class ModelWrapper:
	...
	def train(self, train_set: Dataset, val_set: Dataset = None, collate_fn=None, max_iter=100,
		...
		if val_set:
			trainer = EvalTrainer(max_iter, optimizer, scheduler, learning_rate, T_max,
...
class SimpleModelWrapper(ModelWrapper):
	...
	def train(self, X: Union[torch.Tensor, np.ndarray, List], y: Union[torch.Tensor, np.ndarray, List],
	          ...
	          show_progress=False, eps=1e-5):
		if val_data:
			trainer = SimpleEvalTrainer(max_iter, optimizer, scheduler, learning_rate, T_max,
			                            batch_size, eval_batch_size,
			                            num_workers, num_eval_workers,
			                            pin_memory, pin_memory_device,
			                            persistent_workers,
	...
	def evaluate(self, X: Union[torch.Tensor, np.ndarray, List], y: Union[torch.LongTensor, np.ndarray, List],
		...
		if isinstance(y, (List, np.ndarray)):
			y = torch.LongTensor(y) if isinstance(y[0], int) else torch.FloatTensor(y)
...
class SimpleClassModelWrapper(ClassModelWrapper):
	...
	def train(self, X: Union[torch.Tensor, np.ndarray, List], y: Union[torch.LongTensor, np.ndarray, List],
	          ...
	          show_progress=False, eps=1e-5):
		if val_data:
			trainer = SimpleEvalClassTrainer(max_iter, optimizer, scheduler, learning_rate, T_max,
			                                 batch_size, eval_batch_size,
			                                 num_workers, num_eval_workers,
			                                 pin_memory, pin_memory_device,
			                                 persistent_workers,
...
class SplitTextModelWrapper(TextModelWrapper):
	...
	def train(self, texts: Union[Collection[str], np.ndarray, pd.Series],
	          y: Union[torch.LongTensor, np.ndarray, List],
	          max_length: int = None, val_size=0.2, random_state=None, collate_fn=None, max_iter=100,
	          optimizer: Union[type, optim.Optimizer] = None, scheduler: LRScheduler = None,
	          learning_rate=0.001, T_max: int = 0,
	          batch_size=64, eval_batch_size=128,
	          num_workers=0, num_eval_workers=0,
...
class PaddingTextModelWrapper(ClassModelWrapper):
	...
	def train(self, texts: Union[Collection[str], np.ndarray, pd.Series],
		...
		super().train(train_set, val_set, collate_fn=PaddingTokenCollator(self.tokenizer.pad, max_length),
...
class SplitPaddingTextModelWrapper(PaddingTextModelWrapper):
	...
	def train(self, texts: Union[Collection[str], np.ndarray, pd.Series],
	          y: Union[torch.LongTensor, np.ndarray, List],
	          max_length: int = None, val_size=0.2, random_state=None, max_iter=100,
	          optimizer: Union[type, optim.Optimizer] = None, scheduler: LRScheduler = None,
	          learning_rate=0.001, T_max: int = 0,
	          batch_size=64, eval_batch_size=128,
	          num_workers=0, num_eval_workers=0,
...
class RegressModelWrapper(ModelWrapper):
	...
	def train(self, train_set: Dataset, val_set: Dataset = None, collate_fn=None, max_iter=100,
	          ...
	          show_progress=False, eps=1e-5):
		if val_set:
			trainer = EvalRegressTrainer(max_iter, optimizer, scheduler, learning_rate, T_max,
			                             batch_size, eval_batch_size,
			                             num_workers, num_eval_workers,
			                             pin_memory, pin_memory_device,
			                             persistent_workers,
			...
			trainer = RegressTrainer(max_iter, optimizer, scheduler, learning_rate, T_max,
			                         batch_size, num_workers,
			                         pin_memory, pin_memory_device,
			                         persistent_workers,
			                         early_stopping_rounds,  # 早停，等10轮决策，评价指标不在变化，停止
	...
	def mse(self, X: Union[torch.Tensor, np.ndarray, list[float]], y: Union[torch.Tensor, np.ndarray, list[float]]) -> float:
		if isinstance(X, (List, np.ndarray)):
			X = torch.LongTensor(X) if isinstance(X[0][0], int) else torch.FloatTensor(X)
...
class SimpleRegressModelWrapper(RegressModelWrapper):
	...
	def train(self, X: Union[torch.Tensor, np.ndarray, List], y: Union[torch.Tensor, np.ndarray, List],
	          ...
	          show_progress=False, eps=1e-5):
		if val_data:
			trainer = SimpleEvalRegressTrainer(max_iter, optimizer, scheduler, learning_rate, T_max,
			                                   batch_size, eval_batch_size,
			                                   num_workers, num_eval_workers,
			                                   pin_memory, pin_memory_device,
			                                   persistent_workers,
			...
			trainer = SimpleRegressTrainer(max_iter, optimizer, scheduler, learning_rate, T_max,
			                               batch_size,
			                               num_workers,
			                               pin_memory, pin_memory_device,
			                               persistent_workers,
