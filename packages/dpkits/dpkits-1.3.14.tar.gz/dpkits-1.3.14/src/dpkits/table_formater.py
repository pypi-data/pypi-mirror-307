...
class TableFormatter(Logging):
    ...
    def format_ws_table(self, ws: sheets):
        ...
        #
        # side_range = ws.range(f'B{first_row}:{get_column_letter(first_column - 2)}{last_row}')
        ...
        for i in idx_col_data:
            ...
            lst_address = regex.split(r'R|C', visible_cells.GetAddress(ReferenceStyle=False).split(',')[-1].replace(':', ''))[1:]
