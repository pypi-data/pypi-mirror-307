...
from xlwings import sheets, books
...
class TableFormatter(Logging):
    ...
    def format_ws_content(self, wb: books, ws: sheets):
        ...
        for i, v in enumerate(lst_ws):
            ...
            content_cell.api.Hyperlinks.Add(Anchor=content_cell.api, Address=f"#'{v}'!A3", TextToDisplay=v)

            ws_tbl = wb.sheets[v]
            table_cell = ws_tbl.range('A3')
    ...
    def format_ws_table(self, ws: sheets):
        ...
        first_row = ws.range('A3').end('down').row
        ...
        cells_range = ws.range(f'{get_column_letter(first_column - 1)}{first_row}:{last_column_letter}{last_row}')
        cells_range.api.Borders.Weight = 2  # thin line
        cells_range.api.Borders(12).Weight = 1  # dot

        header_range = ws.range(f'{first_column_letter}3:{last_column_letter}{first_row - 1}')
        header_range.api.Borders.Weight = 2  # thin line

        side_range = ws.range(f'A{first_row}:{get_column_letter(first_column - 2)}{last_row}')
        side_range.api.Borders.Weight = 2  # thin line


        # ws.range(f'{first_row}:{first_row}').api.AutoFilter(Field=4, Criteria1='base')  # Applies a filter on column B with a criteria
        ws.range(f'{first_row - 1}:{first_row - 1}').api.AutoFilter(Field=4, Criteria1='<>base')
        filter_range = ws.api.AutoFilter.Range



        # Iterate through the visible cells and clear values in specific columns
        columns_to_clear = ['A', 'B']
        visible_cells = filter_range.SpecialCells(12)
        for cell in visible_cells:
            col_letter = cell.GetAddress(0, 0).split('$')[0]  # Extract column letter
            if col_letter in columns_to_clear:
                cell.ClearContents()


        # filter_range.Borders(8).Weight = 2
        # ws.api.AutoFilter.Range.Borders(12).Weight = 2
        # filter_range.Font.Bold = True  # Make the text bold
        # filter_range.Interior.Color = self.hex_to_rgb_int('#DAEEF3')
