    ...
    class VisdomSurface(object):
        ...
        def show(self, var_name, split_name, M):
            if var_name not in self.surface:
                self.surface[var_name] = self.viz.surf(
                    X=np.array([M]),
                    env=self.env,
                    opts=dict(legend=[split_name], title=var_name),
                )
            else:
                self.viz.surf(
                    X=np.array([M]), env=self.env, win=self.surface[var_name]
                )
        ...
        def show(self, X, Y, title, legend, markersize=10):
            if title not in self.scatters:
                self.scatters[title] = self.viz.scatter(
                    ...
                    env=self.env,
                    opts=dict(
                        legend=legend, markersize=markersize, title=title
                    ),
                )
            else:
                self.viz.scatter(
        ...
        def show(self, title, customtext):
            if title not in self.vtext:
                self.vtext[title] = self.vis.text(
                    text=customtext, env=self.env, opts=dict(title=title)
                )
            else:
                self.vis.text(
                    text=customtext,
