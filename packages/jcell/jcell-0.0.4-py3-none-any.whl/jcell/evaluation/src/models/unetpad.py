...
class unetpad(nn.Module):
    ...
    def forward(self, inputs):
        if inputs.ndim == 4:
            return self.forward2D(inputs)
        elif inputs.ndim == 5:
            prob = self.forward2D(inputs[:, :, 0, :, :]).unsqueeze(2)
            for i in range(1, inputs.shape[2]):
                tprob = self.forward2D(inputs[:, :, i, :, :]).unsqueeze(2)
                prob = torch.cat((prob, tprob), 2)
