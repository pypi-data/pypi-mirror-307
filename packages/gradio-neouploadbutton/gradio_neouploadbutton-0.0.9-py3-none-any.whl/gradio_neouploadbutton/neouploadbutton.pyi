...
class NeoUploadButton(Component):
    """
    ...
    Also allows drag and drop functionality, and displays a custom message while loading.
    """
    ...
    def __init__(
        ...
        *,
        ...
        Parameters:
            ...
            interactive: If False, the NeoUploadButton will be in a disabled state.
            ...
            loading_message: the custom message displayed while loading. If None, no message will be displayed.
        """
        valid_types = [
            "filepath",
            "binary",
    ...
    def preprocess(
        ...
        if self.file_count == "single":
            if isinstance(payload, ListFiles):
                print(
                    "value preprocess in upload", self._process_single_file(payload[0])
                )
                return self._process_single_file(payload[0])
            print("value preprocess in upload", self._process_single_file(payload))
            return self._process_single_file(payload)

        if isinstance(payload, ListFiles):
            print(
                "value preprocess in upload",
                [self._process_single_file(f) for f in payload],
            )
    ...
    def postprocess(self, value: str | list[str] | None) -> ListFiles | FileData | None:
        ...
        if value is None:
            return None
        print("value in neoupload", value)
        value = self._download_files(value)
        print("files in neoupload", value)
        if isinstance(value, list):
            return ListFiles(
                root=[
                    FileData(
