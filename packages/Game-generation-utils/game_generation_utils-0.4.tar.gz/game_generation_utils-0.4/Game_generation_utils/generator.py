...
class generator:
    ...
    def getChunk(self, x: np.int16, y: np.int16) -> np.ndarray:
        if os.path.exists(f"./Chunks/T_{self.SEEDTEMP}A_{self.SEEDTEMP}H_{self.SEEDTEMP}/C_{x}/{y}.npy"):
            return np.load(f"./Chunks/T_{self.SEEDTEMP}A_{self.SEEDTEMP}H_{self.SEEDTEMP}/C_{x}/{y}.npy")
        else:
            ...
            np.save(f"./Chunks/T_{self.SEEDTEMP}A_{self.SEEDTEMP}H_{self.SEEDTEMP}/C_{x}/{y}", array_biomas)
            np.save(f"./Chunks/T_{self.SEEDTEMP}A_{self.SEEDTEMP}H_{self.SEEDTEMP}/O_{x}/{y}", array_ruido)
            return array_biomas
    
    def isObject(self, x: np.int32, y: np.int32) -> bool:
        chunkX = x // self.CHUNK_SIZE
        chunkY = y // self.CHUNK_SIZE
        objects = np.load(f"./Chunks/T_{self.SEEDTEMP}A_{self.SEEDTEMP}H_{self.SEEDTEMP}/O_{chunkX}/{chunkY}.npy")
    ...
    def poisson_disc_samples(self, r, k=30):
        tau = 2 * pi
        cellsize = r / sqrt(2)
