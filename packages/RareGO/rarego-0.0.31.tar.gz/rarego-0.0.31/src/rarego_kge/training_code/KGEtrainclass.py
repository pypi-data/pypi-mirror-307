...
class KnowledgeGraphEmbedding:
    ...
    def kge_end2end_pipeline(self):
        ...
        # Perform split if split ratios are provided
        ...
        # If split is None, assume csvpath is already loaded as a DataFrame
        # Convert the DataFrame to a list of triples (subject, predicate, object)
            triples = [tuple(x) for x in self.csvpath.to_numpy()]
            train = TriplesFactory.from_labeled_triples(triples)
            test = None
            val = None
        else:
