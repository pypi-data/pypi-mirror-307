...
class KnowledgeGraphEmbedding:
    ...
    def kge_end2end_pipeline(self):
        if self.csvpath is not None:
            ...
            # If split is None, load the entire dataset as training data
                train = TriplesFactory.from_path(self.csvpath)
                test = None
                val = None
      else:
            train = torch.load('train')
            test = torch.load('test') if os.path.exists('test') else None
            val = torch.load('valid') if os.path.exists('valid') else None
