...
def setup_and_execute_script(model_name: str, dataset, text_column, hash_columns: list, accumulate_columns=[],
                             delimiter: str = ',', oaf_env: str = None, batch_size = 32):
    """
    ...
    try:
        ...
        command = f"""python3 tds_vector_embedding_lake.py {model_name} {batch_size} {text_column_position} {accumulate_positions}"""
...
def compute_vector_embedding(model_name, dataset, schema_name, table_name, text_column, hash_columns = None, accumulate_columns=[], oaf_env = None):
    """
...
def run_tds_vector_embedding_script_locally(df, model_name, text_column, accumulate_columns, batch_size = 32, delimiter = ',', print_warnings = False):
    """
    ...
    args = [sys.executable, script_path, model_name, str(batch_size), str(text_column_), str(accumulate_columns_), delimiter]
    ...
    try:
        ...
        if output:  # Ensure there is output to process
            ...
            df_output = pd.DataFrame(rows,
                                     columns=accumulate_columns + ['jobid','process_time','elapsed_time'] + ['model', 'device','Vector_Dimension', 'V'])
            ...
            df_output = df_output.pivot(columns='Vector_Dimension', values='V',
                                        index=accumulate_columns + ['jobid','process_time','elapsed_time']  + ['model','device'])[[str(i) for i in range(df_output['Vector_Dimension'].astype(int).max()+1)]]
        else:
