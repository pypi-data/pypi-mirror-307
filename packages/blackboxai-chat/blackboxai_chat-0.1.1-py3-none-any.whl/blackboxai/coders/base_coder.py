...
from blackboxai import __version__, models, prompts, urls, utils
from blackboxai.analytics import Analytics
from blackboxai.commands import Commands
from blackboxai.history import ChatSummary
from blackboxai.io import ConfirmGroup, InputOutput
from blackboxai.linter import Linter
from blackboxai.llm import litellm
from blackboxai.repo import ANY_GIT_ERROR, GitRepo
from blackboxai.repomap import RepoMap
from blackboxai.run_cmd import run_cmd
from blackboxai.sendchat import RETRY_TIMEOUT, retry_exceptions, send_completion, asend_completion
from blackboxai.utils import format_content, format_messages, format_tokens, is_image_file, is_in_supported_files
from blackboxai.search import search_code
...
class Coder:
    ...
    repo = None
    last_blackboxai_commit_hash = None
    blackboxai_edited_files = None
    ...
    def create(
        ...
        **kwargs,
        ...
        import blackboxai.coders as coders
        ...
        if from_coder:
            ...
            update = dict(
                ...
                blackboxai_commit_hashes=from_coder.blackboxai_commit_hashes,
    ...
    def get_announcements(self):
        lines = []
        lines.append(f"BLACKBOXAI v{__version__}")
        ...
        if self.repo:
            ...
            if num_files > 1000:
                lines.append(
                    "Warning: For large repos, consider using --subtree-only and .blackboxaiignore"
                )
    ...
    def __init__(
        ...
        self.blackboxai_commit_hashes = set()
        ...
        if blackboxai_commit_hashes:
            self.blackboxai_commit_hashes = blackboxai_commit_hashes
        else:
            self.blackboxai_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} that matches blackboxaiignore spec.")
                continue
    ...
    def get_abs_fnames_content(self, fnames: list = None):
        if fnames:
            abs_fnames = fnames
        else:
            abs_fnames = self.abs_fnames
        for fname in list(abs_fnames):
    ...
    def get_files_using_search(self, fnames=None):
        all_abs_files = set(self.get_all_abs_files())
        repo_abs_read_only_fnames = set(self.abs_read_only_fnames) & all_abs_files
        chat_files = set(self.abs_fnames) | repo_abs_read_only_fnames

        documents = []
        for fname, content in self.get_abs_fnames_content([fn for fn in chat_files or all_abs_files if is_in_supported_files(fn)]):
            if not is_image_file(fname):
                documents.append({
                    'content': content,
                    'path': self.get_rel_fname(fname)
                })
        code_str = '\n'.join([f"[FILE_PATH]: {doc['path']}\n[FILE_CONTENT]: {doc['content']}\n[END_FILE]" for doc in documents])
        query = self.cur_messages[-1]['content']
        context = search_code(code_string=code_str, query=query, code_id=self.sid)
        return context
    ...
    def get_chat_files_messages(self):
        ...
        if self.abs_fnames:
            ...
            files_content = self.get_files_using_search()
            files_reply = self.gpt_prompts.files_no_full_files_with_rag
        else:
    ...
    def run_stream(self, user_message):
        ...
        for token in self.send_message(user_message):
            yield token

    async def arun_stream(self, user_message):
        self.io.user_input(user_message)
        self.init_before_message()
        async for token in self.asend_message(user_message):
            yield token

    def init_before_message(self):
        self.blackboxai_edited_files = set()
    ...
    def send_message(self, inp):
        ...
        try:
            while True:
                try:
                    for token in self.send(messages, functions=self.functions):
                        yield token
        ...
        if edited:
            self.blackboxai_edited_files.update(edited)
    ...
    async def asend_message(self, inp):
        import openai  # for error codes below
        ...
        try:
            while True:
                try:
                    async for token in self.asend(messages, functions=self.functions):
                        yield token
                    break
                except retry_exceptions() as err:
                    # Print the error and its base classes
        ...
        edited, edits = await self.aapply_updates()
        yield edits  # yield edits
    ...
    async def asend(self, messages, model=None, functions=None):
        if not model:
            model = self.main_model
        ...
        completion = None
        try:
            hash_object, completion = await asend_completion(
                model.name,
                messages,
                functions,
                self.stream,
            ...
            if self.stream:
                async for token in self.ashow_send_output_stream(completion):
                    yield token
            else:
                self.show_send_output(completion)
        except KeyboardInterrupt as kbi:
    ...
    async def ashow_send_output_stream(self, completion):
    ...
    def auto_commit(self, edited, context=None):
        ...
        try:
            res = self.repo.commit(fnames=edited, context=context, blackboxai_edits=True)
            if res:
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_blackboxai_commit_hash = commit_hash
        self.blackboxai_commit_hashes.add(commit_hash)
        self.last_blackboxai_commit_message = commit_message
        if self.show_diffs:
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head_commit_sha():
            self.io.tool_output("You can use /undo to undo and discard each blackboxai commit.")
