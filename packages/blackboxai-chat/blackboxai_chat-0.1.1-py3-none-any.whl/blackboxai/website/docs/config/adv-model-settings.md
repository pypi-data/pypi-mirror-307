...
In most cases, you can safely ignore blackboxai's warning about unknown context
window size and model costs.

But, you can register context window limits and costs for models that aren't known
to blackboxai. Create a `.blackboxai.model.metadata.json` file in one of these locations:

- Your home directory.
- The root if your git repo.
- The current directory where you launch blackboxai.
...
in the `.blackboxai.model.metadata.json` file.
For example, use `deepseek/deepseek-chat`, not just `deepseek-chat`.

## Model settings

blackboxai has a number of settings that control how it works with
different models.
...
a model that blackboxai doesn't know about.

To do that,
create a `.blackboxai.model.settings.yml` file in one of these locations:
...
[models.py](https://github.com/blackboxai-AI/blackboxai/blob/main/blackboxai/models.py)
file for more details about all of the model setting that blackboxai supports.

<!--[[[cog
from blackboxai.models import get_model_settings_as_yaml
cog.out("```yaml\n")
