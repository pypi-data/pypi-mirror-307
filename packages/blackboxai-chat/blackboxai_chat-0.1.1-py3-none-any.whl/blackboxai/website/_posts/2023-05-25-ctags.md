...
blackboxai no longer uses ctags to build a repo map.
Please see the newer article about
[using tree-sitter to build a better repo map](https://blackbox.ai/docs/repomap.html).
...
To address these issues, `blackboxai` now
...
[chat transcript](https://blackbox.ai/examples/add-test.html)
...
To code with GPT-4 using the techniques discussed here:
  ...
  - Install [blackboxai](https://blackbox.ai/docs/install.html).
  - Install universal ctags.
  - Run `blackboxai` inside your repo, and it should say "Repo-map: universal-ctags using 1024 tokens".
...
This works pretty well, and is supported by `blackboxai` -- you
...
The latest version of `blackboxai` sends a **repo map** to GPT along with
...
Here's a
sample of the map of the blackboxai repo, just showing the maps of
[main.py](https://github.com/blackboxai-AI/blackboxai/blob/main/blackboxai/main.py)
and
[io.py](https://github.com/blackboxai-AI/blackboxai/blob/main/blackboxai/io.py)
:
...
Mapping out the repo like this provides some benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at. GPT will then ask to see these specific files, and `blackboxai` will automatically add them to the chat context (with user approval).
...
Under the hood, `blackboxai` uses
...
Sending an appropriate subset of the repo map would help `blackboxai` work
