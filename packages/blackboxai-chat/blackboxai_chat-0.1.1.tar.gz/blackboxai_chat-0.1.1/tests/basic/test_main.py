...
from blackboxai.main import check_gitignore, main, setup_git
from blackboxai.utils import GitTemporaryDirectory, IgnorantTemporaryDirectory, make_repo
...
class TestMain(TestCase):
    def setUp(self):
        ...
        os.environ["blackboxai_CHECK_UPDATE"] = "false"
        self.original_cwd = os.getcwd()
        self.tempdir_obj = IgnorantTemporaryDirectory()
        self.tempdir = self.tempdir_obj.name
        os.chdir(self.tempdir)
        # Fake home directory prevents tests from using the real ~/.blackboxai.conf.yml file:
    ...
    @patch("blackboxai.repo.GitRepo.get_commit_message", return_value="mock commit message")
    ...
    def test_main_with_git_config_yml(self):
        ...
        Path(".blackboxai.conf.yml").write_text("auto-commits: false\n")
        with patch("blackboxai.coders.Coder.create") as MockCoder:
            main(["--yes"], input=DummyInput(), output=DummyOutput())
            _, kwargs = MockCoder.call_args
            assert kwargs["auto_commits"] is False

        Path(".blackboxai.conf.yml").write_text("auto-commits: true\n")
        with patch("blackboxai.coders.Coder.create") as MockCoder:
    ...
    def test_main_with_empty_git_dir_new_subdir_file(self):
        ...
        # Because blackboxai will try and `git add` a file that's already in the repo.
    ...
    def test_setup_git(self):
        ...
        self.assertEqual(".blackboxai*", gitignore.read_text().splitlines()[0])
    ...
    def test_encodings_arg(self):
        ...
        with GitTemporaryDirectory():
            with patch("blackboxai.coders.Coder.create") as MockCoder:  # noqa: F841
                with patch("blackboxai.main.InputOutput") as MockSend:
    ...
    def test_main_exit_calls_version_check(self):
        with GitTemporaryDirectory():
            with (
                patch("blackboxai.main.check_version") as mock_check_version,
                patch("blackboxai.main.InputOutput") as mock_input_output,
            ):
                main(["--exit", "--check-update"], input=DummyInput(), output=DummyOutput())
                mock_check_version.assert_called_once()
                mock_input_output.assert_called_once()

    @patch("blackboxai.main.InputOutput")
    @patch("blackboxai.coders.base_coder.Coder.run")
    ...
    def test_dark_mode_sets_code_theme(self):
        ...
        with patch("blackboxai.main.InputOutput") as MockInputOutput:
    ...
    def test_env_file_flag_sets_automatic_variable(self):
        env_file_path = self.create_env_file(".env.test", "blackboxai_DARK_MODE=True")
        with patch("blackboxai.main.InputOutput") as MockInputOutput:
            MockInputOutput.return_value.get_input.return_value = None
    ...
    def test_default_env_file_sets_automatic_variable(self):
        self.create_env_file(".env", "blackboxai_DARK_MODE=True")
        with patch("blackboxai.main.InputOutput") as MockInputOutput:
            MockInputOutput.return_value.get_input.return_value = None
            MockInputOutput.return_value.get_input.confirm_ask = True
            main(["--no-git", "--exit"], input=DummyInput(), output=DummyOutput())
    ...
    def test_false_vals_in_env_file(self):
        self.create_env_file(".env", "blackboxai_SHOW_DIFFS=off")
        with patch("blackboxai.coders.Coder.create") as MockCoder:
    ...
    def test_true_vals_in_env_file(self):
        self.create_env_file(".env", "blackboxai_SHOW_DIFFS=on")
        with patch("blackboxai.coders.Coder.create") as MockCoder:
            main(["--no-git"], input=DummyInput(), output=DummyOutput())
            MockCoder.assert_called_once()
            _, kwargs = MockCoder.call_args
    ...
    def test_lint_option(self):
        with GitTemporaryDirectory() as git_dir:
            ...
            with patch("blackboxai.linter.Linter.lint") as MockLinter:
    ...
    def test_verbose_mode_lists_env_vars(self):
        self.create_env_file(".env", "blackboxai_DARK_MODE=on")
        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            ...
            relevant_output = "\n".join(
                ...
                if "blackboxai_DARK_MODE" in line or "dark_mode" in line
            )  # this bit just helps failing assertions to be easier to read
            self.assertIn("blackboxai_DARK_MODE", relevant_output)
            self.assertIn("dark_mode", relevant_output)
            self.assertRegex(relevant_output, r"blackboxai_DARK_MODE:\s+on")
    ...
    def test_yaml_config_file_loading(self):
        with GitTemporaryDirectory() as git_dir:
            ...
            # Create .blackboxai.conf.yml files in different locations
            home_config = fake_home / ".blackboxai.conf.yml"
            git_config = git_dir / ".blackboxai.conf.yml"
            cwd_config = cwd / ".blackboxai.conf.yml"
            named_config = git_dir / "named.blackboxai.conf.yml"
            ...
            with (
                ...
                patch("blackboxai.coders.Coder.create") as MockCoder,
            ):
    ...
    def test_map_tokens_option(self):
        with GitTemporaryDirectory():
            with patch("blackboxai.coders.base_coder.RepoMap") as MockRepoMap:
    ...
    def test_model_metadata_file(self):
        with GitTemporaryDirectory():
            metadata_file = Path(".blackboxai.model.metadata.json")
