...
from blackboxai.repo import GitRepo
from blackboxai.utils import GitTemporaryDirectory
...
class TestRepo(unittest.TestCase):
    ...
    @patch("blackboxai.repo.simple_send_with_retries")
    ...
    @patch("blackboxai.repo.GitRepo.get_commit_message")
    def test_commit_with_custom_committer_name(self, mock_send):
        ...
        with GitTemporaryDirectory():
            ...
            git_repo.commit(fnames=[str(fname)], blackboxai_edits=True)

            # check the committer name
            commit = raw_repo.head.commit
            self.assertEqual(commit.author.name, "Test User (blackboxai)")
            self.assertEqual(commit.committer.name, "Test User (blackboxai)")

            # commit a change without blackboxai_edits
            fname.write_text("new content again!")
            git_repo.commit(fnames=[str(fname)], blackboxai_edits=False)
    ...
    def test_get_tracked_files_with_blackboxaiignore(self):
        with GitTemporaryDirectory():
            # new repo
            raw_repo = git.Repo()
            ...
            blackboxaiignore = Path(".blackboxaiignore")
            git_repo = GitRepo(InputOutput(), None, None, str(blackboxaiignore))
            ...
            blackboxaiignore.write_text("new.txt\n")
            time.sleep(2)
            ...
            #
            # blackboxaiignore.write_text("new2.txt\n")
