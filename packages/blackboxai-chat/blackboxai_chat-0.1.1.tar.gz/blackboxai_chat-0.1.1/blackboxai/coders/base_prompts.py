...
"""
    ...
    files_no_full_files_with_rag = """Analyze the user's request and Identify relevant file paths and code snippets.
Don't try and edit any existing code without asking me to add the files to the chat!
Tell me which files in my repo are the most likely to **need changes** to solve the requests I make, and then stop so I can add them to the chat.
Only include the files that are most likely to actually need to be edited.
Don't include files that might contain relevant context, just files that will need to be changed.
Always know that you are working on a repository, code files may have connection to others. Please try to breakdown and understand the code files correctly to give a best answer.
"""  # noqa: E501
