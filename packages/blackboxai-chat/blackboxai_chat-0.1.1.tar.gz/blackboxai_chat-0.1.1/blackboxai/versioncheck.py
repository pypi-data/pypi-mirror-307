...
import blackboxai
from blackboxai import utils
from blackboxai.dump import dump  # noqa: F401

VERSION_CHECK_FNAME = Path.home() / ".blackboxai" / "caches" / "versioncheck"


def install_from_main_branch(io):
    """
    Install the latest version of blackboxai from the main branch of the GitHub repository.
    """

    return utils.check_pip_install_extra(
        io,
        None,
        "Install the development version of blackboxai from the main branch?",
        ["git+https://github.com/blackboxai-AI/blackboxai.git"],
        self_update=True,
    )
...
def install_upgrade(io, latest_version=None):
    """
    Install the latest version of blackboxai from PyPI.
    """

    if latest_version:
        new_ver_text = f"Newer blackboxai version v{latest_version} is available."
    else:
        new_ver_text = "Install latest version of blackboxai?"

    docker_image = os.environ.get("blackboxai_DOCKER_IMAGE")
    if docker_image:
        text = f"""
...
"""
    ...
    if success:
        io.tool_output("Re-run blackboxai to use new version.")
        sys.exit()
...
def check_version(io, just_check=False, verbose=False):
    ...
    try:
        response = requests.get("https://pypi.org/pypi/blackboxai-chat/json")
        data = response.json()
        latest_version = data["info"]["version"]
        current_version = blackboxai.__version__
