...
from blackboxai import __version__, models, urls, utils
from blackboxai.analytics import Analytics
from blackboxai.args import get_parser
from blackboxai.coders import Coder
from blackboxai.commands import Commands, SwitchCoder
from blackboxai.format_settings import format_settings, scrub_sensitive_info
from blackboxai.history import ChatSummary
from blackboxai.io import InputOutput
from blackboxai.llm import litellm  # noqa: F401; properly init litellm on launch
from blackboxai.repo import ANY_GIT_ERROR, GitRepo
from blackboxai.report import report_uncaught_exceptions
from blackboxai.versioncheck import check_version, install_from_main_branch, install_upgrade
...
def setup_git(git_root, io):
    ...
    if git_root:
        ...
        io.tool_warning("You should probably run blackboxai in a directory, not your home dir.")
        return
    elif io.confirm_ask("No git repo found, create one to track blackboxai's changes (recommended)?"):
...
def check_gitignore(git_root, io, ask=True):
    ...
    try:
        repo = git.Repo(git_root)
        if repo.ignored(".blackboxai") and repo.ignored(".env"):
            return
    except ANY_GIT_ERROR:
        pass

    patterns = [".blackboxai*", ".env"]
    patterns_to_add = []
...
def check_streamlit_install(io):
    return utils.check_pip_install_extra(
        io,
        "streamlit",
        "You need to install the blackboxai browser feature",
...
def launch_gui(args):
    ...
    # https://github.com/blackboxai-AI/blackboxai/issues/2193
...
def register_models(git_root, model_settings_fname, io, verbose=False):
    model_settings_files = generate_search_path_list(
        ".blackboxai.model.settings.yml", git_root, model_settings_fname
    )
    ...
    try:
        ...
        io.tool_error(f"Error loading blackboxai model settings: {e}")
        return 1
...
def register_litellm_models(git_root, model_metadata_fname, io, verbose=False):
    model_metatdata_files = generate_search_path_list(
        ".blackboxai.model.metadata.json", git_root, model_metadata_fname
    )

    # Add the resource file path
    resource_metadata = importlib_resources.files("blackboxai.resources").joinpath("model-metadata.json")
...
def sanity_check_repo(repo, io):
    ...
    if bad_ver:
        io.tool_error("blackboxai only works with git repos with version number 1 or 2.")
        io.tool_output("You may be able to convert your repo: git update-index --index-version=2")
        io.tool_output("Or run blackboxai --no-git to proceed without using git.")
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    conf_fname = Path(".blackboxai.conf.yml")
    ...
    if args.analytics:
        if analytics.need_to_ask():
            ...
            disable = not io.confirm_ask(
                "Allow collection of anonymous analytics to help improve blackboxai?"
            )
    ...
    io.tool_output('Use /help <question> for help, run "blackboxai --help" to see cmd line args')
...
def create_coder(argv=[], input=None, output=None, force_git_root=None, return_coder=False, files=None):
    report_uncaught_exceptions()
...
def check_and_load_imports(io, verbose=False):
    installs_file = Path.home() / ".blackboxai" / "installs.json"
    ...
    try:
        ...
        if str(key) not in installs:
            ...
            try:
                ...
                io.tool_error(str(err))
                io.tool_output("Error loading required imports. Did you install blackboxai properly?")
                if io.confirm_ask(
