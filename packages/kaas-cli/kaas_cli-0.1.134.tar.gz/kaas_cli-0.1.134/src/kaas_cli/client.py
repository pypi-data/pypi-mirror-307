...
from .types import Job, Test, UploadFileMetadata
...
class KaasClient:
    ...
    def _upload_file(
        ...
        with file_path.open('rb') as file:
            ...
            if response.status_code in [200, 201]:  # Success status codes can vary, e.g., 200 OK or 201 Created
                ...
                print(f"Failed to upload {file_path} to {url}. Status code: {response.status_code}")
                print(response.text)
    ...
    def _upload_files(
        ...
        with ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(self._upload_file, Path(file_path_str), url, metadata, uploaded_files, upload_results)
    ...
    def upload_files_s3(self, directory: str, tag: str | None = None) -> str | None:
        ...
        try:
            ...
            self._upload_files(urls, metadata, cache_hash, tests, tag=tag)
