...
class Dropdown():
    ...
    def draw(self, surface: pg.surface, font: pg.font.Font) -> None:
        ...
        pg.draw.polygon(
            ...
            [
                (
                    new_rect.x + new_rect.width - 2 * self.padding - self.padding,
                    new_rect.y + new_rect.height - self.padding
                ),
                (
                    new_rect.x + new_rect.width - 4 * self.padding - self.padding,
                    new_rect.y + self.padding
                ),
                (
                    new_rect.x + new_rect.width - self.padding,
                    new_rect.y + self.padding
                ),
            ]
        )
        ...
        if self.label != None:
            self.label.draw(surface, font, new_rect, self.center_text) # display text Label.
