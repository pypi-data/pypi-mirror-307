    ...
    def get_results(self, ressource_configurations:List[str]) -> List[BadgeResult]:
        ...
        for rc in ressource_configurations: 
            ...
            if not target_df.empty:
                parallel_efficiency =target_df.sort_values(by=["timestamp"], ascending=False).loc[:,'parallelEfficiency'].head(1).values[0]
                results.append(BadgeResult(parallel_eff=parallel_efficiency,ressource_configuration=rc))
            else:
                logging.error(f"Unable to find region {self.region} in {rc} for {self.df['runFolder'].head(1)}. Continuing with erroneous badge.")
