...
LastEditTime: 2024-11-09 14:06:35
...
class Downloader:
    ...
    def get_pdf_url(self):
        ...
        if response.status_code == 200:
            ...
            if match:
                if r'http' not in match.group(1):
                    self.pdf_url = self.url + match.group(1)
                else:
                    self.pdf_url = match.group(1)
                print(f"PDF Download URL: {self.pdf_url}")
            else:
    ...
    def url_iterate(self, url_index=0):
    ...
    def download_pdf(self):
        ...
        if not self.pdf_url:
            print("PDF URL is not available.")
            self.write_wrong_record()
            return
        url_index = 0
        try_times = 0
        while not already_downloaded:
            self.url_iterate(url_index)
            if not self.pdf_url:
                url_index += 1
                if url_index >= len(self.url_list):
            ...
            if try_times > self.tra_times_each_url:
                url_index += 1
                if url_index >= len(self.url_list):
                    print("Failed to download the PDF file.")
                    self.write_wrong_record()
...
def download5doi(store_path=None, doi_list=None, txt_file=None, excel_file=None, col_name=r'DOI'):
    ...
    for doi in track(doi_list, description='Downloading...'):
        download = Downloader(doi, store_path)
        download.download_pdf()


if __name__ == '__main__':
    store_path = r'I:\Delete\ref_pdf'
    # download5doi(store_path, txt_file=r'I:\Delete\ref_pdf\wrong_record.txt')
