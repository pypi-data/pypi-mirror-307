...
def is_state_variable(node: Dict[str, Any], asts: Dict[str, Dict[int, Any]]) -> bool:
    ...
    if left_hand_side_referenced_declaration_id is None:
        assert False, "Should not happen that a reference declaration does not exist"
...
def handle_single_def(source_snippet: str, finder_running_id: int, assignment: Dict[str, Any],
    ...
    if left_hand_side_node_type == "Identifier":
        ...
        if is_state_variable(left_hand_side, ast):
        ...
        if is_unsupported_type(type_string):
            return handle_multi_def(source_snippet, assignment)
        else:
