...
std::string build_progress_bar(int pos, int bar_width) {
    const char filled_block = '#';  // Filled part of the bar
    const char empty_block = '.';   // Empty part of the bar

    std::string progress_bar;
    for (int i = 0; i < bar_width; ++i) {
        if (i < pos) {
            progress_bar += filled_block;  // Add filled block
        } else {
            progress_bar += empty_block;  // Add empty block
        }
    }
    return progress_bar;
}
...
Results* Simulation::run(unsigned long seed) {
    ...
    for (current_timestep = 0; current_timestep <= total_steps; current_timestep++) { 
        ...
        if (pos != last_pos) { 
            ...
            std::string progress_bar = build_progress_bar(pos, bar_width);

            // Display progress bar with remaining steps
            std::cout << "\r[" << progress_bar << "] " << current_timestep << "/" << total_steps << " steps done";
            std::cout.flush();
        }
    ...
    std::cout << "\n[simulation.cpp] Out of loops, preparing the results." << std::endl;
