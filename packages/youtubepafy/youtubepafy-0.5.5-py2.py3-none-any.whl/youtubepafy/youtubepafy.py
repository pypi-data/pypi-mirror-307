...
https://github.com/np1/youtubepafy
...
backend = "internal"
if os.environ.get("youtubepafy_BACKEND") != "internal":
    try:
        import yt_dlp as youtube_dl
        backend = "youtube-dl"
    except ImportError:
        raise ImportError(
            "youtubepafy: youtube-dl not found; you can use the internal backend by "
            "setting the environmental variable youtubepafy_BACKEND to \"internal\". "
            "It is not enabled by default because it is not as well maintained "
            "as the youtube-dl backend.")

if os.environ.get("youtubepafydebug") == "1":
...
def new(url, basic=True, gdata=False, size=False,
    ...
    """ Return a new youtubepafy instance given a url or video id.
    ...
    many video objects at once, you may want to set basic to False, eg:
        ...
        video = youtubepafy.new(basic=False)
    ...
    """
    global youtubepafy
    if youtubepafy is None:
        if backend == "internal":
            from .backend_internal import Internyoutubepafy as youtubepafy
        else:
            from .backend_youtube_dl import Ytdlyoutubepafy as youtubepafy

    return youtubepafy(url, basic, gdata, size, callback, ydl_opts=ydl_opts)
...
def load_cache(newcache):
    """Loads a dict into youtubepafy's internal cache."""
    set_categories(newcache.get('categories', {}))


def dump_cache():
    """Returns youtubepafy's cache for storing by program."""
    return g.cache
