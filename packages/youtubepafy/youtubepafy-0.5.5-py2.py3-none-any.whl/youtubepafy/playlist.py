...
from . import g
from .youtubepafy import new, get_categoryname, call_gdata, fetch_decode
...
def get_playlist(playlist_url, basic=False, gdata=False,
    ...
    """ Return a dict containing youtubepafy objects from a YouTube Playlist.

    The returned youtubepafy objects are initialised using the arguments to
    get_playlist() in the manner documented for youtubepafy.new()
    ...
    for v in allinfo['video']:
        ...
        try:
            youtubepafy_obj = new(vid_data['encrypted_id'],
                                  basic=basic,
                                  gdata=gdata,
                                  size=size,
                                  callback=callback)
        ...
        youtubepafy_obj.populate_from_playlist(vid_data)
        playlist['items'].append(dict(youtubepafy=youtubepafy_obj,
    ...
    def __iter__(self):
        ...
        while self._pageToken != -1:
            ...
            for v in wdata['items']:
                ...
                youtubepafy_obj.populate_from_playlist(vid_data)
                self._items.append(youtubepafy_obj)
                if self._callback:
...
def dict_for_playlist(v):
    """Returns a dict which can be used to initialise youtubepafy Object for playlist
