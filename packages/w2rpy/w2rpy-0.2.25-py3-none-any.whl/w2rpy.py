...
def pebble_count(photo_file, obj_color='yellow', obj_size_mm=190.5, min_area=50):
    ...
    grain_data.to_csv(photo_file.split('.')[0]+'_grain_labels.png', index=False)
    ...
    for pebble in props:
        ...
        y0, x0 = pebble.centroid
        orientation = pebble.orientation
        x1 = x0 + np.sin(orientation) * 0.5 * pebble.major_axis_length
        y1 = y0 - np.cos(orientation) * 0.5 * pebble.major_axis_length
        x2 = x0 - np.cos(orientation) * 0.5 * pebble.minor_axis_length
        y2 = y0 - np.sin(orientation) * 0.5 * pebble.minor_axis_length
