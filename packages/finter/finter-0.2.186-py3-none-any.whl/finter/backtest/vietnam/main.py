...
class VietnamBacktestor(BaseBacktestor):
    def run(self, future_fee_tax = 5, auto_rebalance=True, debug=False):
        future_fee_tax /= 10000
        for i in range(1, self.num_days):
            # Todo: use base price
            short = False
            if self.weight[i].sum() < 0:
                short = True
            ...
            (
                ...
                available_sell_volume=available_sell_volume if not short else None,
            ...
            (
                ...
                self.actual_sell_volume[i],
                self.buy_price[i],
                self.buy_fee_tax if not short else future_fee_tax,
                self.sell_price[i],
                self.sell_fee_tax if not short else future_fee_tax,
                self.cash[i - 1, 0],
                self.target_buy_volume[i],
            )
