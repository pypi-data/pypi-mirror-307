...
def _load_user_setup():
    ...
    # 1. Current working directory (takes precedence over environment variable)
    ...
    # 3. Path specified by an environment variable
...
def _get_jungle_and_format_and_insert(jinja_repo_source_input, format_dict_input, insert_table_type_input, use_setup = True):
    if use_setup is False:
        _jinja_repo_source = jinja_repo_source_input
        _format_dict = format_dict_input
        _insert_table_type = insert_table_type_input
    else:
        _user_setup = _load_user_setup()
        _jinja_repo_source = jinja_repo_source_input or (_user_setup.mipi_repo if _user_setup else None)
        _format_dict = format_dict_input or (_user_setup.mipi_format_dict if _user_setup else None)
        if _user_setup is not None:
            _insert_table_type = _user_setup.mipi_insert_table_type
        else:
            _insert_table_type = insert_table_type_input

    if _format_dict is None:
        _format_dict = dict()

    return _jinja_repo_source, _format_dict, _insert_table_type
...
class DataManager:
    ...
    def __init__(self, frame: meta.Frame,
                 ...
                 use_setup = True):
        self._use_setup = use_setup
        self.dialect = dialect
        self.default_formatter = _FormattedFunctions(default_format_func_dict)

        self.jinja_repo_source, self.default_format_func_dict, self.insert_table_type = _get_jungle_and_format_and_insert(
            jinja_repo_source, default_format_func_dict, insert_table_type, use_setup)
    ...
    def from_jinja_repo(cls, inner_path: str,
        ...
        """
        jinja_repo_source, _, _ = _get_jungle_and_format_and_insert(jinja_repo_source, default_format_func_dict,
                                                                    insert_table_type, use_setup)
        ...
        return cls(frame,
                   ...
                   dialect=dialect, insert_table_type=insert_table_type,use_setup=use_setup)
    ...
    def from_sql(cls, script_path: str, connection: connection.Odbc, format_parameters_list: list = None,
        ...
        return cls(frame, jinja_repo_source=jinja_repo_source, store_deep_frames=store_deep_frames,
                   ...
                   insert_table_type=insert_table_type,use_setup=use_setup)
    ...
    def join_from_format_sql(self, script_path: str, connection: connection.Odbc, on: str | tuple = None,
        ...
        sql = self.resolve_join_format_sql_file(script_path, _on, format_parameters_list,
                                                rename_columns_dict=rename_dict)
    ...
    def join_from_jinja(self, script_path: str, connection: connection.Odbc, on: str | tuple = None,
        ...
        sql = self.resolve_join_jinja_template(script_path, _on, jinja_parameters_dict=jinja_parameters_dict,
                                               rename_columns_dict=rename_dict, insert_table_name=insert_table_name)
        df = query.execute_sql(sql, connection)
    ...
    def clone(self, base_name: str = None,
        ...
        mipi2 = cls(frame, jinja_repo_source=_jinja_repo_source,
                    store_deep_frames=store_deep_frames, store_deep_base=store_deep_base,
                    default_format_func_dict=new_format_dict, use_setup=self._use_setup)

        return mipi2

    def get_temp_table(self, key: str, frame=None,
                       rename_columns_dict: dict | None = None):  # TODO test from specific frame
        """Get the most current list of inserts where 'on' is the insert key"""

        if self.insert_table_type == "full":
            return self._get_temp_table_full(key, frame=frame, rename_columns_dict=rename_columns_dict)
        elif self.insert_table_type == "records2":
            return self._get_temp_table_records2(key, frame=frame, rename_columns_dict=rename_columns_dict)

    def _get_temp_table_full(self, key: str, frame=None, rename_columns_dict: dict | None = None):
        """Get the most current list of inserts where 'on' is the insert key"""
    ...
    def _get_temp_table_records2(self, key, frame=None, rename_columns_dict: dict | None = None):
        ...
        def handle_insert_errors(key, frame_name):
            if key not in df.columns:
                raise KeyError(f"({key}) does not exist in frame: {frame_name})")
        ...
        if isinstance(key, str):
            handle_insert_errors(key, frame_name)
            return generate_inserts._series_to_record2_inserts(df[key], key)
            ...
            return generate_inserts._df_to_record2_inserts(self.trgt[list(key)],
                                                          "@" + "_".join(key),
                                                          list(key))
    ...
    def resolve_join_format_sql(self, sql: str, key: str, format_parameters_list=None, frame=None,
        ...
        inserts = self.get_temp_table(key, frame=frame, rename_columns_dict=rename_columns_dict)
    ...
    def _maybe_get_jinja_insert_dict(self, key, jinja_parameters_dict=None, frame=None, rename_columns_dict=None,
        ...
        if jinja_parameters_dict:
            ...
            jinja_parameters_dict = {
                insert_table_name: self.get_temp_table(key, frame=frame, rename_columns_dict=rename_columns_dict)}
        return jinja_parameters_dict

    def resolve_join_jinja_repo_template(self, inner_path, key, jinja_parameters_dict, frame=None,
                                         rename_columns_dict: dict | None = None,
                                         temp_table_name: str = "MiPiTempTable"):
    ...
    def resolve_join_jinja_template(self, script_path, key, jinja_parameters_dict, frame=None,
                                    rename_columns_dict: dict | None = None, insert_table_name: str = "MiPiTempTable"):
        path = Path(script_path)
        jenv = JinjaLibrary(path.parent)
        jinja_parameters_dict = self._maybe_get_jinja_insert_dict(key, jinja_parameters_dict, frame,
                                                                  rename_columns_dict,
