...
class ProtectedBot:
    ...
    def show_root(self, user_id: int):
        ...
        for i in range(self.ch.camaras_num()):
            camera_name = self.sh.camera_name(i)
            markup.add(InlineKeyboardButton(f"📷 Камера {i+1} ({camera_name})", callback_data=f"root/camera_{i}"))
    ...
    def show_camera_menu(self, user_id: int, camera_num: int):
        ...
        self.bot.send_message(user_id, f"Выберите действие (Камера {camera_num+1} {self.sh.camera_name(camera_num)}):",
                              reply_markup=markup)
    ...
    def notify_users(self, user_ids: list, message_text: str, pic: "Image" = None):
        for id_i in user_ids:
            self.notify_user(id_i, message_text, pic)

    def notify_user(self, user_id: int, message_text: str, pic: "Image" = None):
        try:
            if pic is None:
                self.bot.send_message(user_id, message_text)
            else:
                byte_array = io.BytesIO()
                pic.save(byte_array, format="PNG")
                byte_array.seek(0)
                self.bot.send_photo(user_id, photo=byte_array, caption=f"{message_text}")
        except Exception as e:
            kek = f"Ошибка при отправке сообщения: {e}"

    def notify_users_man_findings(self, message_text: str, img: "Image"):
        users = self.dbh.get_users_with_sub(1, id_only=True)
        self.notify_users(users, message_text, img)
    ...
    def notify_users_taking_event(self, message_text: str):
        users = self.dbh.get_users_with_sub(7, id_only=True)
        self.notify_users(users, message_text)
