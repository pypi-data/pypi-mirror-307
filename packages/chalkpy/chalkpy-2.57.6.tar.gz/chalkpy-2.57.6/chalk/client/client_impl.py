...
def _does_result_match(
    a: Result, e: Result, float_rel_tolerance: float = 1e-6, float_abs_tolerance: float = 1e-12
) -> bool:
    try:
        import polars as pl
    ...
    try:
        ...
        raise missing_dependency_exception("pytest")

    if a.fqn != e.fqn:
        return False
    if a.pkey != e.pkey:
        return False
    if a.cache_hit != e.cache_hit:
        return False
    if e.error is None:
        ...
        if isinstance(a.value, pl.DataFrame) and isinstance(expected_value, pl.DataFrame):
            ...
            if isinstance(a.value, float) and isinstance(expected_value, (float, int)):
                expected_value = pytest.approx(
                    expected_value, nan_ok=True, rel=float_rel_tolerance, abs=float_abs_tolerance
                )
            are_values_equal = a.value == expected_value
        else:
            are_values_equal = False
        ...
        if not isinstance(a.error, ChalkError):
            return False
        if a.error.code != e.error:
            return False
        return True
    else:
...
"""
    ...
    def check(
        ...
        if not _do_query_errors_match(actual_errors, query_errors):
            ...
            expected_errors = [
                [
                    Color.render("Expect", Color.G if q in safe else Color.R),
                    q.code,
                    q.category,
            ...
            actual_errors = [
                [
                    Color.render("Actual", Color.G if q in safe else Color.R),
                    q.code,
                    q.category,
                    q.feature and render_fqn(q.feature),
                    q.resolver,
            ...
            for row in sorted([*expected_errors, *actual_errors], key=lambda k: str(k[5])):
                errors_table.add_row(*row)

            print("\n")
            console.print(errors_table)
            pytest.fail("errors differed -- see output table above")
        ...
        if resp_data is not None:
            ...
            if (feature_mismatch := (not _do_resultsets_match(actuals, expected))) or show_table:
                ...
                for k in expected:
                    row = [
                        Color.render("Expect", Color.G if k.fqn in safe else Color.R),
                        k.fqn and render_fqn(k.fqn),
                        str(k.value),
                    ]
                    if include_errors:
                        row.append(k.error)
                ...
                for row in sorted([*expected_rows, *actual_rows], key=lambda k: k[1]):
                    result_table.add_row(*row)

                print("\n")
                console.print(result_table)
                if feature_mismatch and not no_expectations:
                    pytest.fail("Results did not match; see output above")
        else:
            ...
            if resp_df is None:
                if not actual_errors:
                    pytest.fail("There were no errors and no results")
            else:
