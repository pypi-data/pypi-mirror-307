...
from lesscode_db.connection_info import ConnectionInfo
from lesscode_db.db_request import get_basic_auth
...
class Pool:
    ...
    async def create_clickhouse_pool(conn_info: ConnectionInfo):
        ...
        if not params:
            params = dict()
        client = await clickhouse_connect.get_async_client(host=conn_info.host, port=conn_info.port,
    ...
    def sync_create_clickhouse_pool(conn_info: ConnectionInfo):
        ...
        if not params:
            params = dict()
        client = clickhouse_connect.get_client(host=conn_info.host, port=conn_info.port, username=conn_info.user,
