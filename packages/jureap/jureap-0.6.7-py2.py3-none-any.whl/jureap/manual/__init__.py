...
def generate_plot_file(output_dir, plot_data):
    ...
    for data in plot_data:
        ...
        if data["name"] != plot_info["min_plot_name"]:
            ...
            minimum_error_bar_ymin = min([yxp - yerr for yxp, yerr in zip(yaxis_point, error_yaxis_transpose[0])])
            max_error_bar_ymax = max([yxp + yerr for yxp, yerr in zip(yaxis_point, error_yaxis_transpose[1])])
