    ...
    def send_file(
        ...
        try:
            ...
            for idx, (f, fname) in enumerate(files_to_process):
                ...
                # Changed to use numbered files for multiple file support
                files.append(
                    (
                        f'files[{idx}]',  # Key format required by Discord
            ...
            def make_request():
                ...
                # Use multipart/form-data with proper payload_json formatting
                return self.session.post(
                    self.webhook_url,
                    files=files,
                    data={"payload_json": json.dumps(payload)},  # This is the key change
                    timeout=self.timeout
                )
