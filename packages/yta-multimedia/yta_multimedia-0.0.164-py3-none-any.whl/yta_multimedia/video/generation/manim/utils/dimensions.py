...
# TODO: Remove this below when unneeded
...
# TODO: Move this above when refactored and unneeded
...
class ManimXGenerator:
    ...
    def mobject_fitting_width(mobject: Mobject, width: float, **kwargs):
        ...
        """
        width = width_to_manim_width(width)
    ...
    def mobject_fitting_height(mobject: Mobject, height: float, **kwargs):
        ...
        """
        height = height_to_manim_height(height)
    ...
    def mobject_fitting_fullscreen(mobject: Mobject, **kwargs):
        ...
        """
        image_width_fitted = mobject(**kwargs).scale_to_fit_width(width_to_manim_width(STANDARD_WIDTH))

        # We want the mobject that occupies the whole screen
        if manim_height_to_height(image_width_fitted.height) >= STANDARD_HEIGHT:
            return image_width_fitted
        
        image_height_fitted = mobject(**kwargs).scale_to_fit_height(height_to_manim_height(STANDARD_HEIGHT))
    ...
    def mobject_fitting_screen(mobject: Mobject, **kwargs):
        ...
        if manim_width_to_width(mobject.width) > STANDARD_WIDTH:
            mobject.scale_to_fit_width(width_to_manim_width(STANDARD_WIDTH))
        if manim_height_to_height(mobject.height) > STANDARD_HEIGHT:
            mobject.scale_to_fit_height(height_to_manim_height(STANDARD_HEIGHT))
