...
def image_to_clip_mask(image_google_drive_url_or_filename: str):
    ...
    #mask_720x1080_google_drive_url = 'https://drive.google.com/file/d/1-K6edepJkK9LTGMEaGBX_-5WPlJsrIrA/view?usp=sharing'
    ...
    Resource.get(image_google_drive_url_or_filename, mask_720x1080_filepath)

    # 1. We need a black and white image with the shape
    # white will be video, black will be transparent
    mask_image = ImageParser.to_numpy(mask_720x1080_filepath)

    # *Check if the provided image is only black and
    # white or not 
    WHITE = [255, 255, 255]
    BLACK = [0, 0, 0]
    # The '~' inverts the array
    if np.any(~np.all((mask_image == WHITE) | (mask_image == BLACK), axis = -1)):
        raise Exception(f'The provided "image" parameter "{str(image_google_drive_url_or_filename)}" is not a black and white image.')

    # 2. Simplify the image into a numpy parseable as mask
