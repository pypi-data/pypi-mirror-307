...
# TODO: Save to .parquet(ParqeutFileSource + '/' + "*") and .csv
# TODO: Add Warehouse to DF
# TODO: This how you remove duplicates
# TODO: df_SatSource.write.format("delta").mode("append").save(pFileDirectory);
...
class EasySparkDeltaTable:
    ...
    def add_from_list(self, records: list[dict], schema: StructType = None,
        ...
        self.save(new_easy_df, mode="append")
        return self

    def add_from_df(self, df: DataFrame, merge_option: str = "overwriteSchema",
                    partition_columns: list[str] = None) -> 'EasySparkDeltaTable':
        new_easy_df = self.spark_instance.create_easy_df(df)
        self.save(new_easy_df, mode="append", merge_option=merge_option,
    ...
    def combine_from_df(self, df: DataFrame, type: str = 'unionByName',
        ...
        self.save(easy_df, merge_option=merge_option, partition_columns=partition_columns)
        return self

    def add_from_pd_df(self, pd_df: DataFramePandas, schema: StructType = None,
                       merge_option: str = "overwriteSchema",
                       partition_columns: list[str] = None) -> 'EasySparkDeltaTable':
    ...
    def combine_from_pd_df(self, pd_df: DataFramePandas, schema: StructType = None,
                           ...
                           allowMissingColumns: bool = True,
                           merge_option: str = "overwriteSchema") -> 'EasySparkDeltaTable':
        easy_df = self.to_easy_df()
        easy_df.combine_from_pd_df(pd_df, schema, type, allowMissingColumns)
        self.save(easy_df, merge_option=merge_option)
        return self
    ...
    def delete_all(self) -> 'EasySparkDeltaTable':
        easy_df = self.to_easy_df()
        easy_df.clear()
        self.save(easy_df, mode="overwrite")

        return self

    def save_from_df(self, df: DataFrame = None, mode="overwrite",
                     merge_option: str = "overwriteSchema",
                     partition_columns: list[str] = None):
        easy_df = EasySparkDF.create(EasySparkDeltaTable._spark, df)
        return self.save(easy_df, mode, merge_option, partition_columns)

    def save(self, easy_df: EasySparkDF = None, mode="overwrite",
             merge_option: str = "overwriteSchema",
             partition_columns: list[str] = None) -> 'EasySparkDeltaTable':
        if not easy_df:
            easy_df = self.to_easy_df()
        ...
        if self.for_name:
            easy_df.save_as_table(self.path, mode=mode, merge_option=merge_option,
                                  partition_columns=partition_columns)
        else:
            easy_df.save_to_table_from_path(self.path, mode=mode, merge_option=merge_option,
                                            partition_columns=partition_columns)

        return self

    def merge_from_list(self, keys: list[str], records: list[dict], schema: StructType = None,
                        add_missing_coloumns=True, add_missing_coloumns_to_current=False, ignore_order=False,
        ...
        return self.merge_from_df(keys, df, add_missing_coloumns, add_missing_coloumns_to_current, to_keys)
    ...
    def merge_from_df(self, keys: list[str], df: DataFrame, add_missing_coloumns=True,
                      add_missing_coloumns_to_current=False, to_keys: dict[str, any] = None) -> 'EasySparkDeltaTable':
        ...
        if add_missing_coloumns_to_current:
            ...
            if current_df_has_new_columns:
                self.save_from_df(current_df, mode='overwrite', merge_option="overwriteSchema")
                self.init_delta()
