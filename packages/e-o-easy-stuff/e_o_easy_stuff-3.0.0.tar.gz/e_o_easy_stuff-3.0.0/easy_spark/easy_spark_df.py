...
class EasySparkDF:
    ...
    def add_hash_column_from_columns(self, hash_column_name: str, column_names: list[str] = None) -> 'EasySparkDF':
        if not column_names:
    ...
    def save_as_table(self, path: str, mode="overwrite", merge_option: str = "overwriteSchema",
                      partition_columns: list[str] = None,
                      options: list[tuple[str, any]] = None) -> 'EasySparkDF':
        save_mode_type = EasySaveModeType(mode)
        merge_option_type = EasyMergeOptionType(merge_option)

        return self.save(path, EasySaveType.TABLE, EasyFormatType.DELTA, save_mode_type, merge_option_type,
                         partition_columns, options)

    def save_to_table_from_path(self, path: str, mode="overwrite",
                                merge_option: str = "overwriteSchema", partition_columns: list[str] = None,
                                options: list[tuple[str, any]] = None) -> 'EasySparkDF':
        save_mode_type = EasySaveModeType(mode)
        merge_option_type = EasyMergeOptionType(merge_option)
