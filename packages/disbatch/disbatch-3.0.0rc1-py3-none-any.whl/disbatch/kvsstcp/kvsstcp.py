    ...
    def modify(self, disp):
        ...
        if disp.mask & self.IN:
            if not (disp.curmask & self.IN):
                c.append(
                    select.kevent(disp.fd, select.KQ_FILTER_READ, select.KQ_EV_ADD)
                )
        ...
        if disp.mask & self.OUT:
            if not (disp.curmask & self.OUT):
                c.append(
                    select.kevent(disp.fd, select.KQ_FILTER_WRITE, select.KQ_EV_ADD)
                )
        elif disp.curmask & self.OUT:
            c.append(
                select.kevent(disp.fd, select.KQ_FILTER_WRITE, select.KQ_EV_DELETE)
            )
        if c:
...
class KVSRequestDispatcher(StreamDispatcher):
    ...
    def handle_opkey(self, op, key):
        ...
        if b'mkey' == op:
            ...
            self.next_read(
                4,
                lambda encoding: self.next_lendata(
                    partial(self.handle_put, key, encoding)
                ),
            )
        else:  # 'get_' or 'view'
    ...
    def __init__(self, host=None, port=0):
        ...
        if snof < hnof:
            try:
                ...
                logger.info(
                    'Failed to raise max open files from %d to %d; continuing anyway',
                    snof,
                    hnof,
                )
                pass
...
if '__main__' == __name__:
    ...
    argp.add_argument(
        '-H', '--host', default='', help='Host interface (default is hostname).'
    )
    argp.add_argument(
        '-p',
        '--port',
        type=int,
        default=0,
        help='Port (default is 0 --- let the OS choose).',
    )
    argp.add_argument(
        '-a',
        '--addrfile',
    ...
    argp.add_argument(
        ...
        '--execcmd',
        default=None,
        metavar='COMMAND SEQUENCE',
        help='Execute command with augmented environment.',
    )
    argp.add_argument(
        '-l',
        '--logfile',
