...
def check_sdmx_uom_mappings(df_mapping_sdmx_uom: DataFrame) -> DataFrame:
    # 1. Column `value_multiplier` cannot be null
    value_multiplier_error_rows = df_mapping_sdmx_uom.filter(
        col("value_multiplier").isNull()
    ).collect()

    # 2. Generic mappings must have `value_multiplier` equal to 0 when `sws_multiplier` and `sdmx_multiplier` are unspecified
    generic_mappings_error_rows = df_mapping_sdmx_uom.filter(
        col_is_null_or_empty("sws_multiplier")
        & col_is_null_or_empty("sdmx_multiplier")
        & (col("value_multiplier") != lit(0))
    ).collect()

    # 3. `sws_multiplier` and `sdmx_multiplier` must both be populated or both be empty
    specific_mappings_error_rows = df_mapping_sdmx_uom.filter(
        (
            ~col_is_null_or_empty("sws_multiplier")
            & col_is_null_or_empty("sdmx_multiplier")
        )
    ...
    if len(value_multiplier_error_rows) > 0:
        flag_error = True
        error = f"Column `value_multiplier` cannot be null. Rows with null values: {value_multiplier_error_rows}"
        logging.error(error)
        errors.append(error)
    if len(generic_mappings_error_rows) > 0:
        flag_error = True
        error = f"Generic mappings must have `value_multiplier` equal to 0 when `sws_multiplier` and `sdmx_multiplier` are unspecified. Rows with mismatches: {generic_mappings_error_rows}"
        logging.error(error)
        errors.append(error)
    if len(specific_mappings_error_rows) > 0:
        flag_error = True
        error = f"`sws_multiplier` and `sdmx_multiplier` must both be populated or both be empty. Rows with inconsistencies: {specific_mappings_error_rows}"
        logging.error(error)
        errors.append(error)
    if flag_error:
