...
class SWSGoldIcebergSparkHelper:
    ...
    def apply_uom_mapping(
        ...
        return (
            ...
            .withColumn(
                ...
                F.coalesce(F.log10(col("unit_of_measure_multiplier")), lit(0)).cast(
                    "int"
                ),
            )
    ...
    def rearrange_sdmx_columns(self, df: DataFrame) -> DataFrame:
        ...
        get_columns_for_type = lambda df, type: [
            ...
            if (row["type"] == type) and (not row["delete"])
        ]

        df = self.df_mapping_sdmx_col_names.filter(~col("delete"))

        dimensions = get_columns_for_type(df, "Dimension")
        time_dimensions = get_columns_for_type(df, "TimeDimension")
        primary_measure = get_columns_for_type(df, "PrimaryMeasure")
        attributes = get_columns_for_type(df, "Attribute")
