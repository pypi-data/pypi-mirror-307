...
def obstacles_game():
    ...
    target = Sprite('green', 'circle', random.randint(-290, 290), random.randint(-290, 290))
    obstacles = [Sprite('red', 'square', random.randint(-290, 290), random.randint(-290, 290), stride=15) for _ in range(3)]
    ...
    while not game_over:
        ...
        if player.is_collide(target):
            ...
            target.goto(random.randint(-290, 290), random.randint(-290, 290))  # Relocate target
            obstacles = [Sprite('red', 'square', random.randint(-290, 290), random.randint(-290, 290), stride=15) for _ in range(score + 3)]
            

        # Check collision with obstacles
        for obstacle in obstacles:
            if player.is_collide(obstacle):
                print("Congradulations! You caught", score, 'points!')
                game_over = True
                target.hideturtle()
                time.sleep(1)
                turtle.bye()
