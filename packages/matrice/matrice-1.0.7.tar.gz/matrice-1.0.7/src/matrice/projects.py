...
class Projects:
    ...
    def __init__(self, session, project_name=None, project_id=None):
        ...
        # Fetch project by name only if only project_name is provided
        if project_name is not None:
            project_by_name, err, msg = self._get_project_by_name()
            if self.project_id is None:
                if project_by_name is None:
                    raise ValueError(f"project with name '{self.project_name}' not found.")
                
                self.project_id = project_by_name['_id']
            
            # If both project_id and project_name are provided, check for mismatch
            elif self.project_id is not None and self.project_name is not None:
                fetched_project_id = project_by_name['_id']
                    
                if fetched_project_id != self.project_id:
                    raise ValueError("Provided project_id does not match the project id of the provided dataset_name.")

        self.project_details, error, message = self._get_a_project_by_id()             
        self.project_id = self.project_details["_id"]
        self.project_name = self.project_details["name"]
        self.project_input = self.project_details["inputType"]
        self.output_type = self.project_details["outputType"]
        self.created_at = self.project_details["createdAt"]
        self.updated_at = self.project_details["updatedAt"]
