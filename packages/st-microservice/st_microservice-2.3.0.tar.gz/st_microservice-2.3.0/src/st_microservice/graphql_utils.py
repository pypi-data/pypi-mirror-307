...
from .querybuilder import Select, Column, Cast, Exists, Unnest, Like, PositionalParameter, Term, Descending
...
def _get_model_from_gql_type(gqltype: GraphQLObjectType, models_dict: dict[str, type[BaseModel]]) -> type[BaseModel] | None:
...
def setup_query(query_type: QueryType, model_list: list[type[BaseModel]]):
    ...
    def resolve_type_inspector(_, info: GraphQLResolveInfo, type_name: str):
        ...
        for field_name, field in gqltype.fields.items():
            ...
            if getattr(field, '__filter__', is_database or all_filter):  # If has filter
                ...
                if field_gql_type.name == 'String':
                    ...
                    raise GraphQLError(f'Type {field_gql_type.name} cannot support filtering on field {field_name}')
        ...
        return {'field_details': field_details, 'primary_keys': primary_keys, 'editable': editable}
...
def setup_mutation(mutation_type: MutationType, model_list: list[type[BaseModel]]):
    ...
    async def resolve_insert_object(_, info: GraphQLResolveInfo, type_name: str, payload: dict):
        ...
        obj = model(**payload)
        try:
            await f.insert((model.__metadata__.registry.db_connection_factory or get_db)(info), model, obj)
...
def apply_filter_to_query(q: Select, column: Column, field_type, value) -> Select:
    try:
        if field_type is str:
            ...
            return q.where(Like(
                Cast(column, 'varchar'),  # cast used to make Enum behave like strings.
