...
def localizer_index(kappa, lambda_param, x_op, y_op, ham):
    ...
    return (
        1 / 2 * (np.sum(np.where(eigenvalues >= 0)) - np.sum(np.where(eigenvalues < 0)))
    )
...
def plot_heatmap(kappa, lambda_3, x_op, y_op, ham, grid_size, side_length):
    ...
    plt.title(
        f"Heatmap of Localizer Index $\\kappa={np.round(kappa,2)}$ and $\\lambda_3={np.round(lambda_3,2)}$"
    )
    plt.xlabel("X")
    plt.ylabel("Y")
    ...
    plt.savefig(
        f"localizer/kappa_{np.round(kappa,2)}_l3_{np.round(lambda_3,2)}.png",
        format="png",
        bbox_inches="tight",
    )
    plt.clf()
...
if __name__ == "__main__":
    ...
    lattice, evals, evects, ham = km.get_finite_kane_mele(
        n_side, n_side, t1, delta, np.imag(t2), rashba, pbc=False
    )
    ...
    sb = spin_bott.spin_bott_vect(
        lattice_x2, evals, evects, sigma, fermi_energy=0, threshold_bott=-0.2
    )
    print(f"{sb=}")
