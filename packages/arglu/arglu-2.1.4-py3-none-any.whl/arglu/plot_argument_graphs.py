...
#       xzklcmzcx
    ...
    def show(self,the_base_size=2,gl_arg=""):
        self.fig, self.axis = plt.subplots()

        if self.title:
            self.axis.set_title(self.title)
            
        self.draw_graph(the_base_size=the_base_size,gl_arg=gl_arg)
    ...
    def draw_graph(self, the_base_size=2, gl_arg=""):
        
        G = self.graph
        G = remove_colons(G)
        self.positions = graphviz_layout(G, prog="dot", args=gl_arg)
        # except:
        ...
        self.nodes = nx.draw_networkx_nodes(G, self.positions, 
            node_size=[len(v) * the_base_size for v in G.nodes()], 
             node_color=node_colours)#, node_size=node_size)
        ...
        nx.draw_networkx_edge_labels(G,self.positions, edge_labels = nx.get_edge_attributes(G,'label'))# edge_labels=edge_labels, font_size=font_size, font_color='red')
...
def show_graph(G, show_perspectives=True, the_base_size=2,gl_arg=""):
    ...
    if show_perspectives:
        ...
        gg = GraphGenerator(G)
    gg.show(the_base_size=the_base_size,gl_arg=gl_arg)
