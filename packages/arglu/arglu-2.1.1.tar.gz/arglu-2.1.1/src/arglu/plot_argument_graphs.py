...
#       xzklcmzcx
    ...
    def show(self,the_base_size=2):
        self.fig, self.axis = plt.subplots()

        if self.title:
            self.axis.set_title(self.title)
            
        self.draw_graph(the_base_size=the_base_size)
    ...
    def draw_graph(self, the_base_size=2, gl_arg=""):
        ...
        nx.draw_networkx_edge_labels(G,self.positions, edge_labels = nx.get_edge_attributes(my_graph,'label'))# edge_labels=edge_labels, font_size=font_size, font_color='red')
...
def show_graph(G, show_perspectives=True, the_base_size=2):
    ...
    if show_perspectives:
        ...
        gg = GraphGenerator(G)
    gg.show(the_base_size=the_base_size)
