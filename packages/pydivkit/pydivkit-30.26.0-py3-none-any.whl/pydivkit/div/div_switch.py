...
# A two-state switch that allows user to toggle a boolean variable. The element has
# a different appearance depending on the platform. On iOS, the switch size is
# fixed.
class DivSwitch(BaseDiv):
    ...
    is_enabled: typing.Optional[typing.Union[Expr, bool]] = Field(
        description="Enables or disables the ability to switch an element.",
    )
    is_on_variable: typing.Union[Expr, str] = Field(
        description="The name of the boolean variable for the switch.",
    )
    ...
    on_color: typing.Optional[typing.Union[Expr, str]] = Field(
        format="color", 
        description=(
            "The color of the switch when it is on. If no color is "
            "specified, the defaultsystem color is used on iOS, and the "
            "color specified in `Div2Context` is used onAndroid."
        ),
    )
