...
## Session Argument for BaseCrud Initialization

The `BaseCrud` class in `reworkd_platform/db/crud/base.py` requires a `session` argument in its `__init__` method. When initializing `BaseCrud`, make sure to pass the `session` argument as shown in the examples below:

### Example for AgentCRUD
