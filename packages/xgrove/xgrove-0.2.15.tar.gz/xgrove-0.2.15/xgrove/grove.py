...
class grove():
    ...
    def calculateGrove(self):
        ...
        groves = []
        interpretation = []
        all_rules = []  # Speichert alle Regeln kumulativ
        data = self.data
        ...
        for nt in self.ntrees:
            ...
            for tid in range(nt):
                ...
                # Iteriere durch jeden Knoten des Baumes
                for node_id in range(tree.node_count):
                    if tree.children_left[node_id] != tree.children_right[node_id]:  # Split-Knoten
                        rule = {
                            'feature': tree.feature[node_id],
                            'threshold': tree.threshold[node_id],
                            'pleft': tree.value[tree.children_left[node_id]][0][0],
            ...
            # Den `rules_df` für die spätere kumulative Zusammenfassung zwischenspeichern
            all_rules.append(rules_df)

            # Ab hier ist der übrige Code wie vorher
            vars = []
            splits = []
            csplits_left = []
            pleft = []
            ...
            # Erstellen der Regeln für den aktuellen Baum
            for i in range(len(rules_df)):
                feature_index = int(rules_df.iloc[i]['feature'])
                ...
                if pd.api.types.is_numeric_dtype(data[var_name]):
                    ...
                    splits.append("")
                    csplits_left.append(" | ".join(str(lev) for lev in data[var_name].unique()))
            ...
            # Kumulative Aggregation aller Regeln
            df_small = pd.concat(all_rules).groupby(["vars", "splits", "left"], as_index=False).agg({
                "pleft": "sum", 
                "pright": "sum"
            })
            
            # Berechnung von `basepred`
            ...
            # Benennung für bessere Lesbarkeit
            df = df.rename(columns={"vars": "variable", "splits": "upper_bound_left", "left": "levels_left"})
            df_small = df_small.rename(columns={"vars": "variable", "splits": "upper_bound_left", "left": "levels_left"})

            # Abspeichern der kumulativen Regeln und Erklärungen
            groves.append(df)
            interpretation.append(df_small)
            explanation.append({
                "trees": nt,
                "rules": len(df_small),
                "upsilon": self.upsilon(predictions)[0],
                "cor": self.upsilon(predictions)[1]
            })
