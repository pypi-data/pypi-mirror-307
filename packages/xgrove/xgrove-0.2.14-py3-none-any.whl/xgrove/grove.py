...
class grove():
    ...
    def calculateGrove(self):
        explanation = []
        all_rules = []  # Kumulative Sammlung aller Regeln
        interpretation = []
        
        for nt in self.ntrees:
            predictions = self.surrGrove.staged_predict(self.data)
            current_predictions = [next(predictions) for _ in range(nt)][-1]
            ...
            rules_df = pd.DataFrame(rules)
            all_rules.append(rules_df)  # Speichere die kumulierten Regeln
            
            # Regelvariablen extrahieren
            vars, splits, csplits_left, pleft, pright = [], [], [], [], []
            for i in range(len(rules_df)):
                feature_index = int(rules_df.iloc[i]['feature'])
                var_name = self.data.columns[feature_index]
                vars.append(var_name)
                
                if pd.api.types.is_numeric_dtype(self.data.iloc[:, feature_index]):
                    splits.append(rules_df.iloc[i]["threshold"])
                    csplits_left.append(pd.NA)
                else:
                    levs = self.data[var_name].unique()
                    csplits_left.append(" | ".join(levs))
                    splits.append("")
                
                pleft.append(round(rules_df.loc[i, "pleft"], 4))
                pright.append(round(rules_df.loc[i, "pright"], 4))

            df = pd.DataFrame({"vars": vars, "splits": splits, "left": csplits_left, "pleft": pleft, "pright": pright})
            df_small = pd.concat(all_rules).groupby(["vars", "splits", "left"], as_index=False).agg({"pleft": "sum", "pright": "sum"})
            
            basepred = self.surrGrove.estimators_[0, 0].tree_.value[0][0]
            df0 = pd.DataFrame({"vars": ["Intercept"], "splits": [pd.NA], "left": [pd.NA], "pleft": [basepred], "pright": [basepred]})
            df = pd.concat([df0, df], ignore_index=True)
            df_small = pd.concat([df0, df_small], ignore_index=True)

            upsilon, rho = self.upsilon(pexp=current_predictions)

            df = df.rename({"vars": "variable", "splits": "upper_bound_left", "left": "levels_left"}, axis=1)
            df_small = df_small.rename({"vars": "variable", "splits": "upper_bound_left", "left": "levels_left"}, axis=1)
            
            interpretation.append(df_small)
            explanation.append({"trees": nt, "rules": len(df_small), "upsilon": upsilon, "cor": rho})

        self.explanation = pd.DataFrame(explanation)
        self.rules = interpretation
        self.groves = all_rules
        self.result = self.get_result()
