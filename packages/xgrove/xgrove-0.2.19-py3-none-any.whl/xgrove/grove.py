        ...
        for nt in self.ntrees:
            ...
            for grove in groves:
                ...
                for nt in range(0, len(grove)):  # Iteriere über die Anzahl der Bäume in grove
                    ...
                    baum = grove[nt]  # Da Index bei 0 beginnt, ist dies die korrekte Referenz
                    ...
                    for i in range(len(baum)):
                        ...
                        pleft = baum.loc[i, "pleft"]
                        pright = baum.loc[i, "pright"]
                    ...
                    # Speichere die kumulierten Regeln als aktuelle "grove"
                    groves[grove_index] = cumulative_rules
                    ...
                    for i in range(len(cumulative_rules)):
                        feature_index = int(cumulative_rules.iloc[i]['feature'])
                        var_name = data.columns[feature_index]
                        ...
                        if pd.api.types.is_string_dtype(data.iloc[:, feature_index]) or isinstance(data.iloc[:, feature_index], str) or isinstance(data.iloc[:, feature_index], object):
                            ...
                            splits_temp.append(cumulative_rules.iloc[i]["threshold"])
                    ...
                    # Fasse alles in ein DataFrame zusammen
                    df = pd.DataFrame({
                        "vars": vars_temp,
                        "splits": splits_temp,
                    ...
                    # Add an intercept for the base model (same as before)
                    df0 = pd.DataFrame({
                        "vars": ["Intercept"],
                        "splits": [pd.NA],
                        "left": [pd.NA],
                    ...
                    # (Optional) Ausgabe der resultierenden Interpretation und Erklärung
