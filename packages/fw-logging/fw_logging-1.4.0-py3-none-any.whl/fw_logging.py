...
# TODO consider making this regex or get_caller configurable
hide_name = re.compile("(gun|uv)icorn.error")
...
def get_caller(record: logging.LogRecord, color: bool = False) -> str:
    ...
    # capture custom logger names not based on __name__ (and not explicitly hidden)
    if not module_path.startswith(record.name) and not hide_name.match(record.name):
