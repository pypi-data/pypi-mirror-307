...
def trace(
    ...
    # DBBUG Only
    # Save sha_list to file
    with open("../shalist", mode="w+", encoding="utf-8") as (f):
        for sha in sha_list:
            f.write(sha + "\n")
    ...
    for sha in sha_list:
        ...
        # # DEBUG Only
        # if sha == "00313983cda6f37f747058e58c1cb8fba02bc134":
        #     logger.info(f"Debugging {sha}")
        #     exit()
            ...
            for diff in diffes:
                ...
                if diff.header.old_path == filename or diff.header.new_path == filename:
                    try:
                        ...
                        # DEBUG Only
                        if sha == "167b95ec887029c5fe0fb0f601fb209323f498fb":
                            # 保存当前的 new_line_list 为文件
                            with open(filename + ".p.patch", mode="w+", encoding="utf-8") as (f):
    ...
    with open(filename + ".ppatch", mode="w", encoding="utf-8") as (f):
        for line in new_line_list:
            if line.status:
                f.write(f"{line.index + 1}: {line.content} {line.flag}\n")
