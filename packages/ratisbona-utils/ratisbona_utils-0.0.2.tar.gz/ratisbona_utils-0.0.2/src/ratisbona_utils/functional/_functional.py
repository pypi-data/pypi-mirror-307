...
def first(iterable: Iterable[T]) -> Optional[T]:
    """
        ...
        Returns:
            first element returned by iterable or None, if iterable was empty.
    """
...
def identity(an_object: T) -> T:
    """
        Useful in cases where simply saying "identity" is clearer and more explicit than for example "lambda x: x"
...
def create_id_index(iterable: Iterable[T], id_extractor: Callable[[T], IDT]) -> IdIndex[IDT, T]:
    """
        The create_id_index function takes an iterable and a function that extracts an id from the elements of the iterable.
        ...
        Args:
            ...
            id_extractor: Callable[[T], IDT]: The function to extract an id from an element of the iterable
...
def nonunique_id_index(iterable: Iterable[T], id_extractor: Callable[[T], IDT]) -> dict[IDT, list[T]]:
    """
        The nonunique_id_index function takes an iterable and a function that extracts an id from the elements of the iterable.
        It returns a dictionary that maps the extracted ids to a list of elements of the iterable that share the same id.
        ...
        Returns:
            A dictionary that maps the extracted ids to a list of elements of the iterable that share the same id.
    """
...
def nth_element(n: int) -> Callable[[tuple[T, ...]], T]:
    """
        Conveniently creates a callable that, as soon as given a tuple as input, selects the n-th element as output.
        Intended use is in situations, where for example a key-function needs to be provided, that simply selects one of
        the elements out of a tuple. That way you don't have to write "key=lambda x: x[n]" each time, but you can simply go
        key=nth_element(n)
...
def map_all_values_of_the_dictionary(mapping_function: Callable[[T], RT], the_dict: dict[KT, T]) -> dict[KT, RT]:
    """
...
def nonordered_groupby(
    ...
    """
        ...
        In contrast to iterators.groupby, the nonordered_groupby function does not require the iterable to be sorted.
