...
from ratisbona_utils.colors.simple_color import (
    ...
    cielch_to_lab,
    hsv_to_rgb,
    normalized_to_rgb, bounded_or_bust,
)
...
def matplotlib_colormap_to_palette(huesteps: int, colormap_name: str):
    colormap = matplotlib.colormaps[colormap_name].reversed()
    palette1 = colormap([x / (huesteps - 1) for x in range(huesteps)])
