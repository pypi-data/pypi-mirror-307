...
def qinglong_task_trigger(total_runs_per_day: int, start_time: str = None, end_time: str = None,
                          interval_between_runs: int = 20, skip_count: int = None,
                          valid_duration: int = 8, print_selected_times: bool = False) -> bool:
    """
    ...
    :param interval_between_runs: 主程序的启动间隔，单位分钟，默认为20分钟
    ...
    :return: 是否需要执行任务的布尔值
    """
    # 校验 valid_duration 是否大于 interval_between_runs
    if valid_duration > interval_between_runs:
        raise ValueError(f"任务有效时间窗口 {valid_duration} 不能大于主程序的启动间隔 {interval_between_runs}。")
    ...
    if total_runs_per_day > available_time // interval_between_runs:
    ...
    # 计算主程序的执行时间段（每 interval_between_runs 分钟从 00:00 开始）
    execution_times = []
    for i in range(0, 1440, interval_between_runs):  # 一天的总分钟数1440
    ...
    for execution_time in selected_times:
        ...
        if execution_time <= current_time_minutes < execution_time + valid_duration:
            log.info(
                f"当前时间 {time.strftime('%H:%M', time.localtime())} 在有效执行时间范围：{selected_times_str}，即将执行任务...")
            return True

    # 打印日志
    log.info(f"当前时间 {time.strftime('%H:%M', time.localtime())} 不在有效执行时间范围：{selected_times_str}，跳过执行。")
    return False
