    ...
    *,
    ...
    while True:
        ...
        if check_destination_gas:
            try:
                gas_estimate = await client.estimate_gas(dest_token.chain)
            ...
            gas_available = await dest_context.get_gas_balance(account.address)
            base_logger.debug(f"Available gas: {gas_available}")

            if gas_available < estimated_gas:
                base_logger.info(
                    f"Insufficient gas on chain {
                        dest_token.chain}, will be excluded from future selection"
                )
        ...
        src_order_book_contract = client_utility.make_order_book_contract(src_context, src_token)
        ...
        src_token_contract = client_utility.make_token_contract(src_context, src_token)

        try:
            tx_hash = await src_context.approve_send_contract_function_transaction(
        ...
        start_dest_balance = await dest_context.get_balance(
        ...
        try:
            order_response = await client.submit_order(src_token.chain, tx_hash)
