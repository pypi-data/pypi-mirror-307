...
if [[ -z "$CONNECT_PATH" ]]; then
    ...
    if [[ -x "$HOME/.local/bin/myfabric-connect" ]]; then
        CONNECT_PATH="$HOME/.local/bin/myfabric-connect"
    elif [[ -x "/usr/local/bin/myfabric-connect" ]]; then
        CONNECT_PATH="/usr/local/bin/myfabric-connect"
    else
        # Запрос пути у пользователя, если не найден
        read -p "Не удалось найти myfabric-connect в стандартных директориях. Укажите полный путь: " user_path
        if [[ -x "$user_path" ]]; then
...
ExecStart=$CONNECT_PATH \$MOONRAKER_URL \$MOONRAKER_LOGIN \$MOONRAKER_PASSWORD \$PRINTER_KEY \$MYFABRIC_LOGIN \$MYFABRIC_PASSWORD --log-file \$LOG_FILE --log-level \$LOG_LEVEL
Restart=on-failure
RestartSec=5s
