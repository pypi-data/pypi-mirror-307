...
def start_proxy(moonraker_url, printer_key, login, password):
    ...
    logging.info("Starting websocket: {}".format(reverb_pusher.connection.state))
    reverb_pusher.connection.run()
    logging.info("Started websocket: {}".format(reverb_pusher.connection.state))
...
def reverb_connect_handler(reverb_pusher, channel_name, bearer, reverb_to_moonraker_queue):
    ...
    while reverb_pusher.connection.state != "connected":
        time.sleep(10)
        logger.info("Waiting connection: {}".format(reverb_pusher.connection.state))
