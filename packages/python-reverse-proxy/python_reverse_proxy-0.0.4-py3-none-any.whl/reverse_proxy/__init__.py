...
def make_application(
    ...
    async def make_response(
        ...
        if callable(read_if) and read_if(response):
            for i in reversed(range(len(response_headers))):
                if response_headers[i][0] in (b"content-encoding", b"content-length", b"content-type"):
                    del response_headers[i]
            data = await response.aread()
            return (Response(
                response.status_code, 
                response_headers, 
                Content(bytes(content_type, "latin-1"), data), 
            ), (content_type, data))
        else:
            async def reader():
                try:
