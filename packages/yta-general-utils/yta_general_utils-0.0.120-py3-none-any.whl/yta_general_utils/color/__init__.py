...
class Color:
    ...
    def parse(color):
        ...
        if is_hexadecimal_color(color):
            return ColorConverter.hex_to_rgba(color)
        elif string_color:
            return Color.parse(string_color.value)
        else:
            try:
    ...
    @staticmethod
    def to_color(color: Union[list, tuple, str, 'ColorString']):
        """
        Parse the provided 'color' and turn it into a Color
        class instance. You can provide an hex string, an
        rgb or rgba array or tuple, and a color string name.
        """
        return Color(Color.parse(color))
