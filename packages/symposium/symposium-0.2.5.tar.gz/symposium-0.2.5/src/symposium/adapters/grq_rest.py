...
def prepared_grq_messages(input):
    """
    :input_format
        messages = [
...
def formatted_grq_message(output):
    """
    :input_format
        messages = [
...
def format_grq_output(output):
    """
    :list of choices
    :return: formatted_output, other
    """
    solo_candidate = output['choices'].pop(0)
    formatted_output = formatted_grq_message(solo_candidate['message'])
    if len(output['choices']) > 0:
        other = []
        for choice in output['choices']:
            other_formatted = formatted_grq_message(choice['message'])
