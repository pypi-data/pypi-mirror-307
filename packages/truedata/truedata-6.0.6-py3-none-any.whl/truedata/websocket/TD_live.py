...
class TD_live:
    def __init__(self, login_id, password, url='push.truedata.in', live_port=8082, log_level=logging.WARNING,
                 log_handler=None, log_format=None, full_feed = False, dry_run = False , change_bar = False ):
    ...
    def connect(self):
        ...
        if self.dry_run:
            remove_all_cache()
        self.symbol_id_map_dict , self.symbol_id_map_df = cache_symbol_id(self.login_id , self.password  , self ) if self.full_feed else 0
