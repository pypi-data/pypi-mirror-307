...
def run_database_test(
    ...
    try:
        ...
        for op in setup:
            ...
            validate_op(op)
            print("Command:", op["command"], op.get("argument"))
...
def test_external_database_device(spec, tango_databaseds):
    "Run the tests with an externally provided tango DB"
    run_database_test(**spec)


@pytest.mark.parametrize("spec", command_specs)
def test_pytango_database_device(spec, pytango_databaseds, monkeypatch):
    "Run the tests with PyTango database device"
    monkeypatch.setenv("TANGO_HOST", pytango_databaseds)
    run_database_test(**spec)
