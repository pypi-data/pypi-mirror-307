...
class clas:
    def __init__(self, path):  # 添加文件路径
        ...
        self.y = las.y  # 存储y
        self.z = las.z  # 存储z
        self.i = las.intensity  # 存储强度值
        self.num = len(las.x)  # 存储点云数量
        self.xyz = np.array([las.x, las.y, las.z]).T  # 以xyz格式存储
        self.xyzi = np.array([las.x, las.y, las.z, las.intensity]).T  # 以xyzi格式存储
        self.xy = np.array([las.x, las.y]).T  # 以xy格式存储
        self.h = las.header  # 头文件
        self.SAR = las.scan_angle_rank  # 找到点云的扫描角数据集
        self.i_255 = p1.normalization(las.intensity,255)
        self.xyzi255 = np.c_[self.xyz,self.i_255]
        # np.savetxt('E:\\2023硕士研究上\\0：盾构隧道论文\\杭州.txt',self.xyzi255,fmt='%.05f')
        # self.xyzic = np.c_[self.xyzi,las.Classification,las.Key_Point,las.Raw_Classification,las.Synthetic,las.scan_dir_flag,las.scan_angle_rank,las.user_data,las.withheld,]
        print("读取的las文件共有", self.num, "个点")
        if las.gps_time is not None:
            self.time = np.array([las.gps_time]).T  # 存储GPS时间

    # 点云分块端口
    def get_block(self, mid=mp.cpu_count()):
        '存在严重bug，现已经停止使用'
        global start, end
        if self.num <= mid:
            start = 0
            ...
            n_pool = ceil(self.num / mid)  # 每个池处理的最大点云数量
            print(n_pool)
    ...
    # 求点云平均最临近距离端口(暂时不能用，速度无法解决)
    def get_ANN(self, ir=0.5, max_num=2):
        linshilinyu = []
        t = s.time()
        for i in range(self.num):
            ...
            now_num = len(Z)
            if now_num >= max_num:
                linshilinyu.append(Z)
            else:
                ir += 0.5
                i -= 1
        print("ANN用时", s.time() - t, "秒")
        return
...
# 输入点云数量、分块数量（默认为CPU最大线程数）
def block(num, Piece=mp.cpu_count()):
    '存在巨大Bug，现以停止使用'
    if num <= Piece:
        start = 0
        end = num
    ...
    return start, end  # 输出每个块的下标 起 末

# 将处理后的点云以空格间隔的方式保存为txt
def output(path, data):
    '已弃用，现阶段推荐使用numpy.savetxt()'
    t = s.time()
    file = open(path, mode="w", encoding='gbk')
    if data.shape[1] == 3:  # xyz
        for i in data:
            rowtxt = '{} {} {}'.format(i[0], i[1], i[2])
            file.write(rowtxt)
            file.write('\n')
        file.close()
    elif data.shape[1] == 4:  # xyzi
        for i in data:
            rowtxt = '{} {} {} {}'.format(i[0], i[1], i[2], i[3])
            file.write(rowtxt)
            file.write('\n')
        file.close()
    elif data.shape[1] == 5:  # xyzic
        for i in data:
            rowtxt = '{} {} {} {} {}'.format(i[0], i[1], i[2], i[3], i[4])
            file.write(rowtxt)
            file.write('\n')
        file.close()
    elif data.shape[1] == 6:  # xyzi1i2i3
        for i in data:
            rowtxt = '{} {} {} {} {} {}'.format(i[0], i[1], i[2], i[3], i[4], i[5])
            file.write(rowtxt)
            file.write('\n')
        file.close()
    elif data.shape[1] == 1:  # i
        for i in data:
            rowtxt = '{}'.format(i)
            file.write(rowtxt)
            file.write('\n')
        file.close()
    print("文件已输出至", path, "输出点云文件用时", s.time() - t, "秒")
    return
...
# 点云噪声剔除
def noise_elimination_(xyz, sigma=15):
    '速度较慢，停止使用'
    x, y, z = xyz[:, 0], xyz[:, 1], xyz[:, 2]
    # 封装x,y
    lasdata = np.array(list(zip(x, y, z)))
    tree = spt.cKDTree(lasdata)
    K = 50  # 领域点数
    dist, index = tree.query(lasdata, K)
    k_dist = np.sum(dist, axis=1)
    # 判断噪点的最大阈值
