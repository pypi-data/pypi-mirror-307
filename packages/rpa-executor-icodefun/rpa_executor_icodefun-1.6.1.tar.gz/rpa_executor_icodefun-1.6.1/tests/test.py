...
config = ExecutorConfig(
    ...
    executor_host="192.168.64.16",  # xxl-admin访问executor的地址
    # executor_listen_host="192.168.64.150",  # xxl-admin监听时绑定的host
    debug=True,
)
...
def test_task4():
    ...
    n = 1
    g.logger.info(
        "Job %s get executor params: %s"
        % (g.xxl_run_data.jobId, g.xxl_run_data.executorParams)
    )
