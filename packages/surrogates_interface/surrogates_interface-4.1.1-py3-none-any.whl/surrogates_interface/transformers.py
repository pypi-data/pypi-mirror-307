...
class StandardScalerTorch(StandardScaler):
    ...
    def __init__(self, scale, mean):
        ...
        def __np2torch(v):  # fmt: skip
            return self.torch.as_tensor(v).to(device)
    ...
    def transform(self, x, inplace=True):
        ...
        x = self.torch.as_tensor(x, device=self.device, dtype=self.torch.float32)
    ...
    def transform_gradient(self, x):
        return self.torch.broadcast_to(self.gain_, x.shape).numpy()

    def inverse_transform_gradient(self, x):
        return self.torch.broadcast_to(self.inv_gain_, x.shape).numpy()

    def __eq__(self, other):
        if isinstance(other, self.__class__):
            test_gain = self.torch.equal(self.gain_, other.gain_)
            test_offset = self.torch.equal(self.offset_, other.offset_)
            return test_gain and test_offset
        return False
