...
def under_determined_system(diagrams='../diagrams'):
    ...
    ma.write_figure(figure=fig, filename='under_determined_system000.svg', directory=diagrams, transparent=True)
...
def distance_distribution(X, title=None, ax=None):
    """Plot histogram of pairwise distances with theoretical curve."""
    import matplotlib.pyplot as plt
    ...
    ax.hist(sq_dists.flatten(), bins=50, density=True)
    
    # Plot theoretical gamma distribution
    if X.shape[1] > 1:
        x = np.linspace(0, np.max(sq_dists), 100)
        D = X.shape[1] # dimensionality
        theory = gamma.pdf(x, D/2, scale=2/D)
        plt.plot(x, theory, 'r-', lw=2, label='Theory')
