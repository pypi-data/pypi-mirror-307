...
class ModernProgressBar:
    ...
    def logging(self, message, level="INFO", modernLogging=None):
        with ModernProgressBar._lock:
            ...
            # カーソルを下に移動してログメッセージを表示
            sys.stdout.write(f"\033[{len(ModernProgressBar._active_bars) - self.index}B")  # 下に移動
            sys.stdout.flush()
            ...
            # カーソルを元の位置に戻す
            sys.stdout.write(f"\033[{len(ModernProgressBar._active_bars) - self.index}A")  # 上に移動
