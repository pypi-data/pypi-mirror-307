...
class Eip712NonceKey(NamedTuple):
    contract: HexAddress
    signer: HexAddress
    nonceGetter: str
...
class BaseContractClient(ABC):
    ...
    def _eip712_consume_nonce(self, nonceGetter: str, signerAddress: HexAddress) -> int:
        # TODO: Consider using locking if interleaved calls may be made.
        key = Eip712NonceKey(self.contract.address, signerAddress, nonceGetter)
        if key not in self._cached_eip712_nonces:
            self._cached_eip712_nonces[key] = self._call_contract_read(
                nonceGetter, signerAddress
            )
        nonce = self._cached_eip712_nonces[key]
        self._cached_eip712_nonces[key] += 1
        return nonce
