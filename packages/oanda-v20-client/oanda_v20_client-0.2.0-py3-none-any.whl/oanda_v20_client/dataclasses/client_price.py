...
class ClientPrice:
    ...
    Attributes:
        type (Optional[str]): The string "PRICE". Used to identify the a Price object when found in a stream.
        instrument (Optional[str]): The Price's Instrument.
        time (Optional[str]): The date/time when the Price was created
        status (Optional[ClientPriceStatus]): The status of the Price.
        tradeable (Optional[bool]): Flag indicating if the Price is tradeable or not
        bids (Optional[List['PriceBucket']]): The list of prices and liquidity available on the Instrument's bid
        ...
        asks (Optional[List['PriceBucket']]): The list of prices and liquidity available on the Instrument's ask
            side. It is possible for this list to be empty if there is no ask liquidity currently available for the
            Instrument in the Account.
        closeout_bid (Optional[str]): The closeout bid Price. This Price is used when a bid is required to closeout
            a Position (margin closeout or manual) yet there is no bid liquidity. The closeout bid is never used to open a
            new position.
        closeout_ask (Optional[str]): The closeout ask Price. This Price is used when a ask is required to closeout
            a Position (margin closeout or manual) yet there is no ask liquidity. The closeout ask is never used to open a
            new position.
        quote_home_conversion_factors (Optional[QuoteHomeConversionFactors]): QuoteHomeConversionFactors represents
            the factors that can be used used to convert quantities of a Price's Instrument's quote currency into the
            Account's home currency.
        units_available (Optional[UnitsAvailable]): Representation of how many units of an Instrument are available
    ...
    def from_dict(cls: Type[T], src_dict: Dict[str, Any]) -> T:
        ...
        quote_home_conversion_factors: Optional[QuoteHomeConversionFactors]
        if _quote_home_conversion_factors is None:
