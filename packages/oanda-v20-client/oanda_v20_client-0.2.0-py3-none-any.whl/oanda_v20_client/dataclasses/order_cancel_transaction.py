...
class OrderCancelTransaction:
    ...
    Attributes:
        ...
        type (Optional[OrderCancelTransactionType]): The Type of the Transaction. Always set to "ORDER_CANCEL" for
            an OrderCancelTransaction.
        order_id (Optional[str]): The ID of the Order cancelled
        client_order_id (Optional[str]): The client ID of the Order cancelled (only provided if the Order has a
            client Order ID).
        reason (Optional[OrderCancelTransactionReason]): The reason that the Order was cancelled.
