...
class PositionBook:
    ...
    Attributes:
        instrument (Optional[str]): The position book's instrument
        time (Optional[str]): The time when the position book snapshot was created
        price (Optional[str]): The price (midpoint) for the position book's instrument at the time of the position
            book snapshot
        bucket_width (Optional[str]): The price width for each bucket. Each bucket covers the price range from the
            bucket's price to the bucket's price + bucketWidth.
        buckets (Optional[List['PositionBookBucket']]): The partitioned position book, divided into buckets using a
