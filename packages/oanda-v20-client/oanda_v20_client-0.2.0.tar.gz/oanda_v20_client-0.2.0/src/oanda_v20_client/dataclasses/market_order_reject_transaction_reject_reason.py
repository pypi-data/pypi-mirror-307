...
MarketOrderRejectTransactionRejectReason = Literal[
...
MARKET_ORDER_REJECT_TRANSACTION_REJECT_REASON_VALUES: Set[
...
def check_market_order_reject_transaction_reject_reason(
    value: str,
) -> MarketOrderRejectTransactionRejectReason:
    if value in MARKET_ORDER_REJECT_TRANSACTION_REJECT_REASON_VALUES:
        return cast(MarketOrderRejectTransactionRejectReason, value)
    raise TypeError(
        f"Unexpected value {value!r}. Expected one of {MARKET_ORDER_REJECT_TRANSACTION_REJECT_REASON_VALUES!r}"
    )
