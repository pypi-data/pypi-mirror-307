...
class GuaranteedStopLossOrderEntryData:
    ...
    Attributes:
        minimum_distance (Optional[str]): The minimum distance allowed between the Trade's fill price and the
            configured price for guaranteed Stop Loss Orders created for this instrument. Specified in price units.
        premium (Optional[str]): The amount that is charged to the account if a guaranteed Stop Loss Order is
            triggered and filled. The value is in price units and is charged for each unit of the Trade.
        level_restriction (Optional[GuaranteedStopLossOrderLevelRestriction]): A
