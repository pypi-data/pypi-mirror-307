...
StopLossOrderRejectTransactionRejectReason = Literal[
...
STOP_LOSS_ORDER_REJECT_TRANSACTION_REJECT_REASON_VALUES: Set[
...
def check_stop_loss_order_reject_transaction_reject_reason(
    value: str,
) -> StopLossOrderRejectTransactionRejectReason:
    if value in STOP_LOSS_ORDER_REJECT_TRANSACTION_REJECT_REASON_VALUES:
        return cast(StopLossOrderRejectTransactionRejectReason, value)
    raise TypeError(
        f"Unexpected value {value!r}. Expected one of {STOP_LOSS_ORDER_REJECT_TRANSACTION_REJECT_REASON_VALUES!r}"
    )
