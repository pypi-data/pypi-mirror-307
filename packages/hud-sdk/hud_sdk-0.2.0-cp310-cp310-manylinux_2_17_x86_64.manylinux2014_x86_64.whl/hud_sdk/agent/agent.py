...
class Agent:
    def __init__(
        ...
        self.perf_monitor = PerformanceMonitor(self.pod_cpu_limit)
        self.manager = None  # type: Optional[Manager]
        self._memory = (
            None
        )  # type: Optional[multiprocessing.shared_memory.SharedMemory]
    ...
    @suppress_exceptions_sync(default_return_factory=lambda: None)
    def _check_queue_utilization(self) -> None:
        if not self._queue:
            return
        utilization = (
            (self._queue.buffer_size - self._queue.available) / self._queue.buffer_size
        ) * 100
    ...
    @suppress_exceptions_sync(default_return_factory=lambda: False)
    def queue_processor(self) -> bool:
    ...
    def process_queue_until_empty(self, _task: "asyncio.Task[Any]") -> None:
        while self.queue_processor():
            pass
    ...
    async def cleanup(self) -> None:
        ...
        if self._memory:
            with contextlib.suppress(Exception):
                self._memory.close()
            with contextlib.suppress(Exception):
                internal_logger.info("Unlinking shared memory from agent")
                self._memory.unlink()
            self._memory = None
    ...
    async def run(self) -> None:
        with contextlib.ExitStack() as stack:
            ...
            self._memory = stack.enter_context(manager.get_shared_memory())
            ...
            queue_task = self.task_manager.register_periodic_task(
                self.queue_processor, config.agent_queue_process_interval
            )
            queue_task.add_done_callback(self.process_queue_until_empty)
