...
class ActiveNetworks(WorkerThread):
    ...
    def build(self):
        ...
        with commit_and_close(self._network_file, spatial=True) as conn:
            ...
            self.activenet.emit(["start", "master", 13, self.master_msg_txt])
            ...
            self.__enforces_stable_ids(conn)
    ...
    def __enforces_stable_ids(self, conn):
        link_df = pd.read_sql("""Select walk_link, ref_link, "length" distance from Transit_Walk""", conn)
        link_df = link_df.sort_values(by=["ref_link", "distance"])

        # Let's create a numbering system that is unique and stable when rebuilding the network
        # We will have a number that only changes when the ref_link changes
        change = link_df["ref_link"] != link_df["ref_link"].shift(1)
        link_id = (change.cumsum() + link_df.ref_link.max() + 1).astype(np.int64)

        # When the link is virtual (not ref_link), then the value for ref_link==0
        link_id[link_df.ref_link.to_numpy() > 0] = link_df.ref_link.to_numpy().astype(np.int64)

        diff_ref_links = link_id * 100 + WALK_LINK_RANGE

        # And one that only changes when ref_link stays the same (for the parts of the ref_links)
        same_ref_links = link_df.groupby("ref_link").cumcount()

        link_df = link_df.assign(ids=diff_ref_links + same_ref_links)

        # Ensures we don't have duplicated IDs
        duplicated_indices = link_df[link_df.duplicated(subset=["ids"], keep=False)].index
        max_id = link_df.ids.max()
        for idx in duplicated_indices:
            max_id += 1
            link_df.at[idx, "ids"] = max_id

        # Push changes to the database
        data = link_df[["ids", "walk_link"]].to_records(index=False)
        conn.executemany("UPDATE Transit_Walk set walk_link=? where walk_link=?", data)

        conn.commit()

        # Do the same for the new nodes we have placed in the transit_stops table
        # We use its coordinates to ensure that we have a somewhat stable ordering
        stop_df = pd.read_sql("""Select stop_id, X, Y from Transit_Stops where stop="walk_access" """, conn)
        stop_df = stop_df.sort_values(by=["X", "Y"])
        stop_df = stop_df.assign(ids=WALK_AGENCY_ID * AGENCY_MULTIPLIER + np.arange(stop_df.shape[0]) + 1)

        data = stop_df[["ids", "stop_id"]].to_records(index=False)
        conn.executemany("UPDATE Transit_Stops set stop_id=? where stop_id=?", data)
        conn.executemany("UPDATE Transit_Walk set from_node=? where from_node=?", data)
        conn.executemany("UPDATE Transit_Walk set to_node=? where to_node=?", data)
        conn.commit()
    ...
    def _save_changes(self, conn: sqlite3.Connection):
        j = 0
        for old_link in self.__delete_links:
