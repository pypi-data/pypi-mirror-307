...
class ProX(cns.sqlX, cns.SQLstrX):
    ...
    # ----------------- tmpl --------------------------24.11.06
    tmpl['select'] = {'头部':'select', '模板':"{连接2} {符}{值}{符}",  '连接':','} # ok
    ...
    tmpl['建表'] = {'头部':'CREATE TABLE `{表名}` (', '模板':"{连接2} {符}{值}{符} {数据类型}" , '连接':',', '尾部':'\n)', '数据类型':'varchar(32) DEFAULT NULL'} #ok
    ...
    tmpl['alter'] = {'模板':"ALTER TABLE {符}{前缀}{值}{后缀}{符} {字段}; ", '前缀':'', '后缀':''}    # ok
    tmpl['column'] = {'模板':"{连接2}{指令} COLUMN {符}{值}{符} {数据类型}", '连接':',', '符s':'`', '符i':'`' }   # ok
