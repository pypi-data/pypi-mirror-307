...
class SEDA2WrapObjectMixin:
    ...
    def postprocess_dataobjectsreference(self, root, namespaces):
        ...
        for reference_node in reference_nodes:
            ...
            reference_node.attrib[f"{{{namespaces['a']}}}defaultValue"] = (
                f"g{node_id[0]}"
            )
...
class SEDA02XSDExport(SEDA1XSDExport):
    ...
    _root_attributes["targetNamespace"] = (
...
def _simple_path_target_values(entity, rtype, role, target_etype):
    ...
    relation_schema_for = entity._cw.vreg.schema.relation_schema_for
    rdefschema = next(iter(relation_schema_for(rtype).relation_definitions.values()))
...
def _complex_path_target_values(entity, path):
    ...
    for rtype, role, target_etype, _ in path:
        try:
            rdefschema = next(
                iter(relation_schema_for(rtype).relation_definitions.values())
            )
        except KeyError:
