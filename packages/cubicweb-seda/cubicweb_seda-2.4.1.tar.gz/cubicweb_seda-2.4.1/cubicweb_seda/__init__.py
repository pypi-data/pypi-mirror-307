...
def _iter_external_rdefs(eschema, skip_rtypes):
    ...
    for rschema, targets, role in eschema.relation_definitions():
        ...
        for target_etype in targets:
            rdef = eschema.relation_definition(rschema, role, target_etype)
            if rdef.composite:
                continue
            yield rdef, role
    ...
    while stack:
        ...
        for (rtype, role), targets in graph.child_relations(etype):
            ...
            for target in targets:
                if role == "subject":
                    rdef = rschema.relation_definitions[(etype, target)]
                else:
                    rdef = rschema.relation_definitions[(target, etype)]
                yield rdef, role
