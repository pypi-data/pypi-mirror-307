...
def page(request) -> Generator[BrowserContext, None, None]:
    ...
    if request.param in ["firefox", "webkit"]:
        browser_context = getattr(playwright, request.param).launch_persistent_context("", headless=headless,
                                                                                       **launch_kwargs)
    else:
        launch_kwargs["channel"] = request.param
        browser_context = playwright.chromium.launch_persistent_context("", headless=headless, **launch_kwargs)
