...
class TestMain(unittest.TestCase):
    ...
    def test_main(
        self, mock_to_csv, MockClient, mock_validate_metadata, MockMetadataDbClient, mock_getenv
    ):
        ...
        mock_validate_metadata.side_effect = lambda record: {"result": f"validated_{record['sample_record']}"}
        ...
        mock_rds_client.read_table.return_value = pd.DataFrame([{"result": "validated_1"}, {"result": "validated_2"}])
        ...
        self.assertEqual(mock_validate_metadata.call_count, 2)  # Called for each record

        # DataFrame calls
        mock_to_csv.assert_any_call("validation_results.csv", index=False)
        mock_to_csv.assert_any_call("validation_results_from_rds.csv", index=False)

        # RDS client overwrite and append calls
        mock_rds_client.overwrite_table_with_df.assert_called_once()
        if len(mock_metadata_client.retrieve_docdb_records.return_value) > 1000:
