...
class FlawlessAPI:
    ...
    def _create_middleware_wrapper(self, middleware, next_handler):
        ...
        async def wrapper(scope, receive, send):
            # 执行中间件前置处理
            try:
                await middleware(scope, 'before', send)
            except Exception as e:
                print(middleware, traceback.format_exc())
            ...
            try:
                ...
                # 即使发生错误也要执行后置处理
                try:
                    await middleware(scope, 'after', send)
                except:
                    print(traceback.format_exc())
                    pass
                raise e
            ...
            try:
                ...
                for middleware in self.middleware_stack:
                    try:
                        ...
                        print(f"中间件{middleware}执行失败: {traceback.format_exc()}")
                        return await self._handle_middleware_error(e, send)
