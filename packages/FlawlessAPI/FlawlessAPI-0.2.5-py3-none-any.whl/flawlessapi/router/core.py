...
from flawlessapi.monitoring.monitoring import PerformanceMonitor
from flawlessapi.monitoring.tracer import DistributedTracer
...
class FlawlessAPI:
    ...
    async def _send_response(self, response, send) -> None:
        ...
        if isinstance(response, dict) and "headers" in response and "body" in response:
            ...
            wrapped_response = success_response(data=response)
            print(wrapped_response.data)
            # 标准JSON响应
            code = response.get('code', 200)
            await self.async_response.send_json_response(send, code, wrapped_response.data)
        else:
            # 其他类型响应
            wrapped_response = success_response(data=response)
            await self.async_response.send_json_response(
    ...
    async def _handle_metrics(self, request: AsyncRequest) -> Dict[str, Any]:
        ...
        return metrics_response(data=metrics_data)
    ...
    async def _handle_traces(self, request: AsyncRequest) -> Dict[str, Any]:
        ...
        return traces_response(data=traces_data)
    ...
    async def _handle_health(self, request: AsyncRequest) -> Dict[str, Any]:
        ...
        return health_response(data=health_data)
    ...
    async def _handle_info(self, request: AsyncRequest) -> Dict[str, Any]:
        ...
        return info_response(data=info_data)
    ...
    async def _handle_middleware_error(self, error: Exception, send) -> None:
        ...
        error_resp = error_response(code=code, message=message, data=detail)
        await self.async_response.send_json_response(send, error_resp.code, error_resp.dict())
