...
class TracerMiddleware(Middleware):
    """分布式追踪中间件"""
    ...
    async def process_request(self, scope, receive, send):
        await self.tracer.trace_request(scope, 'before', send)

    async def process_response(self, scope, receive, send, response):
        await self.tracer.trace_request(scope, 'after', send)
        return response
...
class DistributedTracer:
    ...
    async def trace_request(self, scope, timing, send):
        """
        中间件接口方法，用于追踪HTTP请求
