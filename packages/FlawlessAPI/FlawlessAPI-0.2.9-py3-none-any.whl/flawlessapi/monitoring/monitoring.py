...
from flawlessapi.monitoring.metrics import MetricsCollector
...
class PerformanceMonitorMiddleware(Middleware):
    ...
    async def process_request(self, scope, receive, send):
        """处理请求前的监控"""
        await self.monitor.record_request(scope, 'before', send)
        
    async def process_response(self, scope, receive, send, response):
        """处理响应后的监控"""
        await self.monitor.record_request(scope, 'after', send)
        return response
...
class PerformanceMonitor:
    ...
    async def record_request(self, scope, timing, send):
        """
        记录请求信息
