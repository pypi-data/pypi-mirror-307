...
from flawlessapi.cache.lru_cache import LRUCache
...
class AsyncResponse:
    ...
    async def send_json_response(self, send, status_code: int, body_dict: Dict[str, Any], scope: dict) -> None:
        """发送JSON响应
        ...
        try:
            ...
            if cached_data:
                await self._send_cached_response(send, status_code, cached_data, scope)
                return

            # 序列化逻辑
            try:
                if isinstance(body_dict, ApiResponse):
                    body_dict = await body_dict.dict()
                # 1. 处理 Pydantic 模型
                ...
                await self._send_response(send, 500, error_data, 
                    [[b'content-type', b'application/json; charset=utf-8']] + scope.get('headers', []))
                return

            # 设置响应头
            headers = [[b'content-type', b'application/json; charset=utf-8']] + scope.get('headers', [])
            ...
            if len(bytes_data) > self.COMPRESSION_THRESHOLD:
                ...
                if len(compressed_data) < len(bytes_data) * self.MIN_COMPRESSION_RATIO:
                    self.logger.info(f"[Compression] Ratio: {len(compressed_data)/len(bytes_data):.2%}")
    ...
    async def send_file_response(self, send, scope: dict, file_path: str, content_type: str = None):
        """发送文件响应
        ...
        try:
            ...
            headers = [[b'content-type', content_type.encode() if content_type else b'application/octet-stream']] + scope.get('headers', [])
            await self._send_response(send, 200, content, headers)
        except Exception as e:
            await self._send_response(send, 500, str(e).encode('utf-8'), [])

    async def send_stream_response(self, send, scope: dict, generator, content_type: str = 'text/plain'):
        """发送流式响应
        ...
        Args:
            ...
            scope: 请求上下文
        """
        headers = [[b'content-type', content_type.encode()]] + scope.get('headers', [])
        # 发送响应头
        await send({
    ...
    async def _send_cached_response(self, send, status_code: int, body: bytes, scope: dict):
        """发送缓存的响应
        ...
        Args:
            ...
            scope: 请求上下文
        """
        headers = [[b'content-type', b'application/json']] + scope.get('headers', [])
        if len(body) > self.COMPRESSION_THRESHOLD:
            headers.append([b'content-encoding', b'gzip'])
        await self._send_response(send, status_code, body, headers)


    async def _send_response(self, send, status_code: int, body: bytes, headers: list):
        """发送HTTP响应
        ...
        try:
            ...
            # 记录错误
            self.logger.error(f"Error sending response: {traceback.format_exc()}")
            raise

    async def send_not_found_response(self, send, scope: dict):
        """发送404 Not Found响应
        ...
        await self.send_json_response(send, 404, body_dict, scope)
...
async def success_response(data: Any = None, message: str = "success") -> ApiResponse:
    """成功响应"""
    return ApiResponse(code=200, message=message, data=data)

async def error_response(code: int = 400, message: str = "error", data: Any = None) -> ApiResponse:
    """错误响应"""
    return ApiResponse(code=code, message=message, data=data)

async def not_found_response(message: str = "Resource not found") -> ApiResponse:
    """404响应"""
    return ApiResponse(code=404, message=message)

async def validation_error_response(message: str = "Validation error", errors: Any = None) -> ApiResponse:
    """验证错误响应"""
    return ApiResponse(code=422, message=message, data=errors)

# 添加新的响应类型
async def metrics_response(data: Any) -> ApiResponse:
    """性能指标响应"""
    return ApiResponse(code=200, message="Metrics retrieved successfully", data=data)

async def traces_response(data: Any) -> ApiResponse:
    """追踪信息响应"""
    return ApiResponse(code=200, message="Traces retrieved successfully", data=data)

async def health_response(data: Any) -> ApiResponse:
    """健康检查响应"""
    return ApiResponse(code=200, message="System health status", data=data)

async def info_response(data: Any) -> ApiResponse:
    """系统信息响应"""
    return ApiResponse(code=200, message="System information", data=data)

async def docs_response(data: Any) -> ApiResponse:
    """API文档响应"""
    return ApiResponse(code=200, message="API documentation", data=data)