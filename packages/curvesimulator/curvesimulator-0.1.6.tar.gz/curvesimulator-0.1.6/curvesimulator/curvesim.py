...
def curvesim():
    parameters = csp.CurveSimParameters()  # Read program parameters from config file.
    bodies = csbb.CurveSimBodies(parameters)  # Read physical bodies from config file and initialize them, calculate their state vectors and generate their patches for the animation
    lightcurve = bodies.calc_physics(parameters)  # Calculate body positions and the resulting lightcurve.
    csa.CurveSimAnimation(parameters, bodies, lightcurve)  # Create the video
