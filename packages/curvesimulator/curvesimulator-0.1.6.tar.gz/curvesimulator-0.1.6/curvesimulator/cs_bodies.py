...
class CurveSimBodies(list):
    ...
    def __init__(self, p, debug_L=-1):
        ...
        for section in config.sections():
            if section not in p.standard_sections:  # section describes a physical object
                self.append(csb.CurveSimBody(p=p,
                                                 name=section,
        ...
        for star in stars:
            ...
            for body in self:
                if body != star:  # an object cannot eclipse itself :)
                    ...
                    if eclipsed_area != 0:
                        luminosity -= star.brightness * eclipsed_area * csp.CurveSimPhysics.limbdarkening(relative_radius, star.beta)
        return luminosity
    ...
    def calc_positions_eclipses_luminosity(self, p):
        ...
        lightcurve = csl.CurveSimLightcurve(p.iterations)  # Initialize lightcurve (essentially a np.ndarray)
