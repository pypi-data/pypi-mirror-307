...
class CurveSimBody:
    ...
    def keplerian_elements_to_state_vectors_debug_new(self):
        ...
        ea = csp.CurveSimPhysics.kepler_equation_root_debug(e, ma, ea_guess=ma)  # [d], [e]. Maybe implement alternative version from [f]8-31 and [f]8.10.2???
    ...
    def keplerian_elements_to_state_vectors_debug(self):
        ...
        ω = ϖ - Ω
        ma = L - ϖ
        ea = csp.CurveSimPhysics.kepler_equation_root(e, ma, ea_guess=ma)
        nu = 2 * math.atan(math.sqrt((1 + e) / (1 - e)) * math.tan(ea / 2))  # 3b: true anomaly (from eccentric anomaly)
        r = a * (1 - e * math.cos(ea))  # 4b: radius r
        h = math.sqrt(mu * a * (1 - e ** 2))  # 5b: specific angular momentum h
    ...
    def keplerian_elements_to_state_vectors_chatgpt(self):
        ...
        if ea is not None:
            ...
            if nu is not None:
                ...
                ea = csp.CurveSimPhysics.kepler_equation_root_chatgpt(e, ma, ea_guess=ma)
                # Calculate true anomaly from eccentric anomaly
                nu = 2 * math.atan2(math.sqrt(1 + e) * math.sin(ea / 2), math.sqrt(1 - e) * math.cos(ea / 2))
            elif T is not None:
        ...
        if ea is not None:  # ea provided
            ...
            if nu is not None:  # nu provided
                ...
                if ma is not None:  # ma provided
                    ea = csp.CurveSimPhysics.kepler_equation_root(e, ma, ea_guess=ma)  # A good guess is important. With guess=0 the root finder very often does not converge.
                    nu = 2 * math.atan(math.sqrt((1 + e) / (1 - e)) * math.tan(ea / 2))  # 3b: true anomaly (from eccentric anomaly)
    ...
    def calc_state_vectors(self, p, bodies):
        ...
        self.mu = csp.CurveSimPhysics.gravitational_parameter(bodies, p.g)  # is the same for all bodies in the system, because they are orbiting a common barycenter
    ...
    def eclipsed_by(self, body, iteration):
        ...
        if body.positions[iteration][1] < self.positions[iteration][1]:  # Is body nearer to viewpoint than self? (i.e. its position has a smaller y-coordinate)
            d = csp.CurveSimPhysics.distance_2d_ecl(body, self, iteration)
