...
class Open5GS:
    ...
    def _run_container_scripts(self):
        ...
        def _check_wireshark_installed(self) -> bool:
            """Check if Wireshark is installed properly"""
            paths = ["/usr/bin/wireshark", "/usr/local/bin/wireshark"]
            return any(os.path.exists(path) for path in paths)
        
    def _get_interface_name_sudo(self, ip_address: str) -> Optional[str]:
        """Get interface name for the given IP address using sudo"""
        try:
            result = subprocess.run(
                ["sudo", "ip", "-o", "addr", "show"], 
                capture_output=True, 
    ...
    def launch_wireshark(self, 
        ...
        try:
            ...
            # Get interface name
            # Use sudo to get interface name
            interface_name = self._get_interface_name_sudo(ip_address)
            ...
            command = ["sudo", wireshark_cmd, "-i", interface_name, "-k"]
            if display_filter:
