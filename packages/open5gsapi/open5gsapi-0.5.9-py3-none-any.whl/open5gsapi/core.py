...
class Open5GS:
    ...
    def launch_wireshark(self, 
        ...
        try:
            ...
            # Launch Wireshark with clean environment
    ...
    def launch_gtp_wireshark(self) -> bool:
        try:
            gtp_filter = (
                "(gtp) or " +          # GTP protocol
                "(gtpv2) or " +        # GTPv2 protocol
                "(gtp.type == 1) or " + # GTP Echo Request
                "(gtp.type == 2) or " + # GTP Echo Response
                "(diameter) or " +      # Diameter protocol
                "(udp.port == 2123) or " + # GTP-C port
                "(udp.port == 2152)"      # GTP-U port
            )
            
            capture_filter = "port 2123 or port 2152"  # GTP-C and GTP-U ports
