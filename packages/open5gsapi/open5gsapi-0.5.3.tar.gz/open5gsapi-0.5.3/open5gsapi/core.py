...
class Open5GS:
    ...
    def launch_wireshark(self, 
        ...
        try:
            ...
            if not self._check_wireshark_installed():
                raise ConfigurationError("Wireshark is not installed on the system")
            
            # Get interface name for the IP
            ...
            # Build Wireshark command with all arguments
            command = ["wireshark", "-i", interface_name, "-k"]  # -k starts capturing immediately
            ...
            try:
                ...
                if process.poll() is None:  # Still running
                    logger.info(f"Wireshark launched successfully for interface {interface_name}")
                    return True
            except Exception as e:
                logger.debug(f"Direct launch failed: {e}")

            # If direct launch failed, try with xhost
            try:
                subprocess.run(['xhost', '+local:'], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                process = subprocess.Popen(
                    command,
                    env=env,
                    stdout=subprocess.PIPE,
                ...
                logger.debug(f"Launch with xhost failed: {e}")

            raise ConfigurationError("Failed to launch Wireshark after all attempts")
    ...
    def _launch_wireshark_direct(self, command: List[str]) -> bool:
        process = subprocess.Popen(
            command,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            env=self._get_display_environment(),
            start_new_session=True
        )
    ...
    def _launch_wireshark_gtk(self, command: List[str]) -> bool:
        try:
            subprocess.run(['gtk-launch', 'wireshark'], 
                        check=True, 
                        env=self._get_display_environment())
            return True
        except:
    ...
    def _launch_wireshark_dbus(self, command: List[str]) -> bool:
        try:
            subprocess.run(['dbus-launch'] + command, 
                        check=True, 
                        env=self._get_display_environment())
            return True
        except:
    ...
    def launch_gtp_wireshark(self) -> bool:
        """
        Launch Wireshark with GTP filtering for the default interface.
        """
        try:
            gtp_filter = (
                "gtp or gtpv2 or gtp_echo or "  # GTP protocols
                "port 2123 or port 2152 or "    # GTP-C (2123) and GTP-U (2152)
                "diameter"                       # Include Diameter protocol
            )
            
            return self.launch_wireshark(
                ip_address="10.10.0.1",
                display_filter=gtp_filter,
