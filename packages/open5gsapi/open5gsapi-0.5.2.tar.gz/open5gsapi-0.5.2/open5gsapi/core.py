...
class Open5GS:
    ...
    def _get_interface_name(self, ip_address: str) -> Optional[str]:
        """Get interface name for the given IP address"""
        try:
            result = subprocess.run(
                ["ip", "-o", "addr", "show"], 
                capture_output=True, 
    ...
    def launch_wireshark(self, 
        ...
        try:
            ...
            command = ["wireshark", "-i", interface_name, "-k"]  
            ...
            for launch_method in [self._launch_wireshark_direct, 
                                self._launch_wireshark_gtk, 
                                self._launch_wireshark_dbus]:
                try:
                    if launch_method(command):
                        logger.info(f"Wireshark launched successfully for interface {interface_name}")
                        return True
                except Exception as e:
                    logger.debug(f"Launch method failed: {str(e)}")
                    continue
            
            raise ConfigurationError("All launch methods failed")
    ...
    def launch_gtp_wireshark(self) -> bool:
        ...
        try:
            return self.launch_wireshark(
                ...
                capture_filter="port 2123 or port 2152"  # Capture only GTP traffic
            )
        except Exception as e:
