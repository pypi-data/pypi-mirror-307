...
class Open5GS:
    ...
    def _check_wireshark_installed(self) -> bool:
        """Check if Wireshark is installed on the system"""
        try:
    ...
    def launch_wireshark(self, 
        ...
        try:
            ...
            process = subprocess.Popen(
                ...
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                start_new_session=True  # This ensures Wireshark runs independently
            )
            ...
            if process.poll() is None:  # None means process is still running
                ...
                logger.error(f"Wireshark failed to start: {stderr.decode()}")
                return False
    ...
    def launch_gtp_wireshark(self) -> bool:
        """
        Convenience method to launch Wireshark with GTP filtering for the default interface.
        
        Returns:
            bool: True if Wireshark was launched successfully, False otherwise
        """
        # Common GTP related protocols and ports
        gtp_filter = (
