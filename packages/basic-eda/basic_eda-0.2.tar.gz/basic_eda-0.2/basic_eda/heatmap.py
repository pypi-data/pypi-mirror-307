...
def heatmap(*, df: pd.DataFrame, col1: str, col2: str, ax, n_top: int = 10, cmap: str):
    ...
    data = data.pivot(index=col1, columns=col2, values="count").fillna(0)
...
def categorical_heatmap(*,
    ...
    parallel = ProgressParallel(n_jobs=1, total=len(pairs), require="sharedmem")
    with parallel:
        parallel(delayed(heatmap)(df=df,
                                  col1=col1,
                                  col2=col2,
                                  ax=ax,
                                  n_top=n_top,
                                  cmap="Reds")
                 for ax, (col1, col2) in zip(axes.ravel(), pairs))

    # Off unused plot's axis
    unused = n_cols * n_rows - len(pairs)
    if unused > 0:
