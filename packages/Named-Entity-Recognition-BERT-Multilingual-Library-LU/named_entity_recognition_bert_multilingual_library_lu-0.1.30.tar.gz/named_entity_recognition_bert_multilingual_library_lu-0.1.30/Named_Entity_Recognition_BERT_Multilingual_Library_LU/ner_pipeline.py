...
class NERPipeline:
    ...
    def prepare_data(self, train_path, val_path, test_path):
        ...
        self.validate_data(val_dataset)
    ...
    def tokenize_and_align_labels(self, examples):
        tokenized_inputs = self.tokenizer(
            ...
            max_length=128,  # Set a reasonable max length to avoid excessive sequences
        ...
        for i, label in enumerate(examples["ner_tags"]):
            ...
            for word_idx in word_ids:
                if word_idx is None:
                    ...
                    label_ids.append(label[word_idx])  # Use label of first wordpiece
                else:
    ...
    def validate_data(self, dataset):
        """Validate that each token sequence matches its corresponding label sequence."""
        for idx, example in enumerate(dataset):
            tokens = example["tokens"]
            labels = example["ner_tags"]
            if len(tokens) != len(labels):
                raise ValueError(
                    f"Mismatch found in sample {idx}: {len(tokens)} tokens vs {len(labels)} labels."
                )
    ...
    def train(self, train_dataset, val_dataset, output_dir="./bio_ner_results"):
        ...
        data_collator = DataCollatorForTokenClassification(tokenizer=self.tokenizer, padding="longest")
