...
class NERPipeline:
    ...
    def prepare_data(self, train_path, val_path, test_path):
        ...
        # Define unique labels and map them to IDs
    ...
    def train(self, train_dataset, val_dataset, output_dir="./bio_ner_results"):
        ...
        training_args = TrainingArguments(
            ...
            num_train_epochs=num_epochs,  # User-provided epochs
            weight_decay=0.01,
            report_to="none",
            logging_dir="./logs",
            logging_steps=1000,
    ...
    def evaluate(self, dataset):
        ...
        logits = predictions.predictions  # These are logits
        labels = predictions.label_ids    # Actual labels

        # Align predictions with labels to use compute_metrics
        predicted_tags = np.argmax(logits, axis=2)
        return predicted_tags, labels
