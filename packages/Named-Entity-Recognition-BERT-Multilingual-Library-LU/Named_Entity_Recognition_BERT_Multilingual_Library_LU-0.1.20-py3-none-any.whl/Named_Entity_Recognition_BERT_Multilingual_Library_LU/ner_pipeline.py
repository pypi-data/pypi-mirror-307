...
class NERPipeline:
    ...
    def prepare_data(self, train_path, val_path, test_path):
        ...
        # Only one mapping to align tokens with labels
        ...
        for i, label in enumerate(examples["ner_tags"]):
            ...
            for word_idx in word_ids:
                if word_idx is None:
                    ...
                    label_ids.append(label[word_idx])  # Align label to first subword
                else:
                    label_ids.append(-100)  # Handle potential misalignment
    ...
    def test(self, test_dataset):
        ...
        trainer = Trainer(
            ...
            args=TrainingArguments(
                ...
                remove_unused_columns=False,
                report_to="none",
                eval_accumulation_steps=10  # Avoids memory overflow
            ),
        )

        # Predictions
        results = trainer.predict(test_dataset)
        logits = torch.tensor(results.predictions).to(device)
        labels = torch.tensor(results.label_ids).to(device)

        # Align predictions and labels
        predictions = np.argmax(logits.cpu().numpy(), axis=2)
        true_labels = [[self.label_list[l] for l in label if l != -100] for label in labels.cpu()]
        true_predictions = [
            [self.label_list[p] for (p, l) in zip(prediction, label) if l != -100]
            for prediction, label in zip(predictions, labels.cpu().numpy())
        ]
