...
class NERPipeline:
    ...
    def tokenize_and_align_labels(self, examples):
        tokenized_inputs = self.tokenizer(
            examples["tokens"],
            padding="max_length",  # Ensures consistent length across batches
            truncation=True,  # Truncate sequences exceeding max model input length
            max_length=128,  # Set this to an appropriate length depending on your model/data
            is_split_into_words=True,
            return_tensors="pt"  # Explicitly use PyTorch tensors
        )
        ...
        for i, label in enumerate(examples["ner_tags"]):
            ...
            for word_idx in word_ids:
                if word_idx is None:
                    ...
                    label_ids.append(label[word_idx])  # Align labels
                else:
