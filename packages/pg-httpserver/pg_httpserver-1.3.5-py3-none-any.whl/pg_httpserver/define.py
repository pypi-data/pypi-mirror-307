...
from pg_common import FuncDecoratorManager, Context
...
__all__ = [
    ...
    "RequestHeader", "RequestData", "SESSION_EXPIRED_TIME", "ENV_CONTEXT_KEY", "GameContext"
]
...
class GameContext(object):
    ...
    def load_data(self, clazz, pri_keys: dict)->ObjectBase:
        name = clazz.__name__
        if name not in self.cache:
            ...
            key = "_".join([self.ctx.user.game, self.ctx.user.channel])
            ret.load(prefix=key, redis_server_name=key,
