...
from .define import ENV_NEEDS_BODY_MIDDLEWARE, ENV_NEEDS_CHECK_SESSION, ENV_CHECK_SESSION_IGNORE_URI, \
    ...
    SESSION_EXPIRED_TIME, ENV_CONTEXT_KEY, GameContext, ENV_NEEDS_CHECK_REQUEST_ID, ENV_CHECK_REQUEST_ID_IGNORE_URI, \
    ENV_GAME_CONTEXT_KEY
...
def get_game_context(req: Request)->GameContext:
    return req.scope[config.get_conf(ENV_GAME_CONTEXT_KEY, "_game_context_")]
...
async def http_log_inspector(request, call_next):
    ...
    request.scope[config.get_conf(ENV_GAME_CONTEXT_KEY, "_game_context_")] = GameContext(_container)
    response = await call_next(request)
