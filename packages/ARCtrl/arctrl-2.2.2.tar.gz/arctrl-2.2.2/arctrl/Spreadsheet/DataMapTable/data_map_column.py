...
def to_fs_columns(dc: Array[DataContext]) -> FSharpList[FSharpList[FsCell]]:
    ...
    class ObjectExpr1316:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow1315(x: str, y: str) -> bool:
                return x == y
    ...
    comment_keys: FSharpList[str] = to_list(distinct(collect(mapping_1, dc), ObjectExpr1316()))
    ...
    def _arrow1327(__unit: None=None, dc: Any=dc) -> IEnumerable_1[FSharpList[FsCell]]:
        def _arrow1326(__unit: None=None) -> IEnumerable_1[FSharpList[FsCell]]:
            def _arrow1325(dc_4: DataContext) -> FSharpList[FsCell]:
                dc_3: DataContext = dc_4
                def _arrow1324(__unit: None=None) -> IEnumerable_1[FsCell]:
                    def _arrow1317(__unit: None=None) -> FSharpList[FsCell]:
                            ...
                            def _arrow1321(__unit: None=None) -> IEnumerable_1[FsCell]:
                                def _arrow1320(__unit: None=None) -> IEnumerable_1[FsCell]:
                                    def _arrow1319(__unit: None=None) -> IEnumerable_1[FsCell]:
                                        ...
                                        return append(create_text(DataContext__get_GeneratedBy(dc_3)), delay(_arrow1318))

                                    return append(create_text(DataContext__get_Description(dc_3)), delay(_arrow1319))

                                return append(create_term(DataContext__get_ObjectType(dc_3)), delay(_arrow1320))

                            return append(create_term(DataContext__get_Unit(dc_3)), delay(_arrow1321))

                        return append(create_term(DataContext__get_Explication(dc_3)), delay(_arrow1322))

                    return append(_arrow1317(), delay(_arrow1323))

                return to_list(delay(_arrow1324))

            return map_1(_arrow1325, dc)

        return append(singleton_1(headers), delay(_arrow1326))

    return transpose(to_list(delay(_arrow1327)))
