...
def FsCellsCollection__GetFirstAddress(this: FsCellsCollection) -> FsAddress:
    if True if is_empty(this._rowsCollection) else is_empty(this._rowsCollection.keys()):
        ...
        def _arrow162(__unit: None=None, this: Any=this) -> int:
            def projection(d: Any) -> int:
                ...
                return min(d.keys(), ObjectExpr159())
            ...
            d_1: Any = min_by(projection, this._rowsCollection.values(), ObjectExpr160())
            class ObjectExpr161:
                @property
                def Compare(self) -> Callable[[int, int], int]:
                    return compare_primitives

            return min(d_1.keys(), ObjectExpr161())

        return FsAddress__ctor_Z37302880(min(this._rowsCollection.keys(), ObjectExpr158()), _arrow162())
