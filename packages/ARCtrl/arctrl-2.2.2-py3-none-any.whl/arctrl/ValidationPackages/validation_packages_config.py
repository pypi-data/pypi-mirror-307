...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        ...
        def _arrow2903(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2902(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2901(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2900(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow2899(__unit: None=None) -> IEnumerable_1[str]:
                            def _arrow2898(__unit: None=None) -> IEnumerable_1[str]:
                                return singleton("}")

                            return append(singleton("]"), delay(_arrow2898))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow2899))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow2900))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow2901))

            return append(singleton("{"), delay(_arrow2902))

        return join("\n", to_list(delay(_arrow2903)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        def sort(arg: Array[ValidationPackage]) -> Array[ValidationPackage]:
            ...
            return sort_by(projection, list(arg), ObjectExpr2904())
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow2906(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow2905(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow2905, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow2906))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr2907
