...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow2882(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2881(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2880(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2879(__unit: None=None) -> IEnumerable_1[str]:
                        return singleton("}")

                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow2879))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow2880))

            return append(singleton("{"), delay(_arrow2881))

        return join("\n", to_list(delay(_arrow2882)))
...
ValidationPackage_reflection = _expr2884
