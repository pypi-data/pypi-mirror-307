...
## optimization of speed (loading package)

* generate_song_cover.py should be moved to subpackage.
  * __init__.py for this package should then import static_ffmpeg and static_sox
  * and call the add_path methods they expose
* get_sample_models should be moved to manage_models.py
* __init__.py for core package should only download base models
* for cli we should expose get sample models under models router
* cli should also expose function that does all initialization?
* for entry points scripts, we will at least be doing this for `init.py`

## optimization of package installation
* add remaining wheels to diffq-fixed, fairseq-fixed and pyworld-fixed
...
## eliminiation of entry point scripts
* check if it is possible to run web app from cli in normal and dev mode by exposing start_app from web.main module
* also make sure that it is actually always possible to run cli directly with urvc (and not ./urvc cli)
* if so then add a call to the "total initialization" fnction at the beginning of main.py so that we can remove init.py
* we can then also remove cli.py and w can remove web.py

## optimization distributable package structure
* first of all, stubs should not be included in distributable package but everything else in src should be
* we should replace the prefix src with ultimate-rvc for the redistributable package.
  * This can be done with hatch using:
    * [tool.hatch.build.targets.wheel.sources]
      "src" = "ultimate_rvc"
* we should renaming src to ultimate-rvc and placing it in a new src subfolder which will also have stubs included
  * that way we dont need to do anything fancy to get building to work
