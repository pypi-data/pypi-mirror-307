...
CHANGELOG_URL = "https://forgeapp.io/changelog"
...
class Logger:
    ...
    def prod_no_prefix(self, *args, **kwargs):
        """Same as prod, but without the [Forge] prefix"""
        print(*args, **kwargs)
    ...
    def info_no_prefix(self, *args, **kwargs):
        """Same as info, but without the [Forge] prefix"""
        if self.log_level != "quiet":
            print(*args, **kwargs)
    ...
    def handle_sdk_alert(self, sdk_alert: SdkAlert):
        ...
        if sdk_alert.severity == "INFO":
            self.info("🆕\tA new Forge SDK version is available.")
            if sdk_alert.message:
                self.info(sdk_alert.message)
        elif sdk_alert.severity == "WARNING":
            self.warn(
                f"{WARN_EMOJI}\tThis version of the Forge SDK has been deprecated. Please update as soon as possible, it will not work in a future update."
            )
            if sdk_alert.message:
            ...
            self.error(
                f"{ERROR_EMOJI}\tThis version of the Forge SDK is no longer supported. Your app will not work until you update."
            )
            if sdk_alert.message:
