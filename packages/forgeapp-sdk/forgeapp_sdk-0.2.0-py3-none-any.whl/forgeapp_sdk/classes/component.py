...
class Component(Generic[MN, PropsModel_co, StateModel_co]):
    ...
    async def handle_validation(self, return_value: Any) -> Optional[str]:
        try:
            ...
            print(
                f"[Forge] Received invalid return value ({return_value}):",
                err,
                file=sys.stderr,
            )
    ...
    def set_return_value(self, value: Any):
        ...
        try:
            ...
            print(
                f"[Forge] Received invalid return value ({value}):",
                err,
                file=sys.stderr,
            )
            traceback.print_exc(file=sys.stderr)
    ...
    async def set_state(self, value: Any):
        try:
            ...
            if self._handle_state_change:
                ...
                print(
                    "[Forge] Received state, but no method was defined to handle.",
                    file=sys.stderr,
                )

        except ValidationError as err:
            print("[Forge] Received invalid state:", value, err, file=sys.stderr)
