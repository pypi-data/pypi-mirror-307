...
class FormInternalObj(qsa.FormDBWidget):
    ...
    def controlDatosCacheo(self, cursor) -> bool:
        """Return default controlDatosCacheo."""

        table_name: str = cursor.table()
        qsa.debug("controlDatosCacheo: %s" % table_name)
        if not "fldatatables_cache" in qsa.aqApp.db().manager().cache_metadata_.keys():
            qsa.debug("PASO1 %s" % qsa.aqApp.db().manager().cache_metadata_.keys())
            return True
