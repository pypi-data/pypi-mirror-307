...
from mle_toolkit import common
...
def test_get_extension(case: tp.Dict) -> None:
    assert common.get_extension(case["path"]) == case["expected"]
...
def test_construct_path(case: tp.Dict) -> None:
    output = common.construct_path(folder=case["folder"],
    ...
    if case["fold_id"] is not None:
        assert case["fold_id"] == common.get_fold_id(output)

    if case["mode"] is not None:
        assert case["mode"] == common.get_mode(output)

    assert case["trial_number"] == common.get_trial_id(output)
...
def test_get_list_of_files(tmp_path, case: tp.Dict):
    ...
    # Act
    output = common.get_files(folder=d,
...
def test_squeeze_target(tmp_path, case: tp.Dict) -> None:
    ...
    # Act & Assert
    assert common.get_last_study(tmp_path, case["study"]) == case["expected"]
