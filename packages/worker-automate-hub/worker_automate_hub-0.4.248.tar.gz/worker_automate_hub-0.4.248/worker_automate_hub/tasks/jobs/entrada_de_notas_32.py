...
async def entrada_de_notas_32(task: RpaProcessoEntradaDTO) -> RpaRetornoProcessoDTO:
    ...
    try:
        ...
        try:
            max_attempts = 60
            i = 0
            aguarde_information_window = True
            ...
            while i < max_attempts:
                ...
                for window in Desktop(backend="uia").windows():
                    if "Information" in window.window_text():
                        aguarde_information_window = False
                        console.print(
                            "A janela 'Information' foi encontrada. Continuando para andamento do processo...\n"
                        )
                        break
                await worker_sleep(5)

                if not aguarde_information_window:
                    break
                i += 1
