...
class Scenario(TagAndStatusStatement, Replayable):
    ...
    def compute_status(self):
        ...
        for step in self.all_steps:
            cur = step.status
            if step.status not in (Status.skipped, Status.untested):
                break
        else:
            return cur
        ...
        for step in self.all_steps:
            if step.status == Status.undefined:
                if self.was_dry_run:
                    ...
                    return Status.failed
            elif step.status == Status.failed and step.step_type not in ("IF", "ELIF"):
                return step.status
    ...
    def jump_index(self):
        """
        every step jump index
        """
        all_steps = list(self.all_steps)
        n = len(all_steps)
        jumps = [
            [i + 1, n, False] for i in range(n)
        ]  # pass, fail, continue_after_failed_step
        st = []
        for_block = False
        for index, step in enumerate(all_steps):
            st.append((index, step.step_type))

            if step.step_type == "FOR":
                # 循环中存在if的时候，把if重新压栈
                for_block = True

            if step.step_type == "ENDIF":
                st_temp = []
                while True:
                    # 把if-else之间的语句正序
                    cur = st.pop()
                    st_temp.append(cur)
                    if cur[1] == "IF" and (not st or st[-1][1] != "IF"):
                        break

                if for_block:
                    st.extend(list(reversed(st_temp)))

                prev = None
                while st_temp:
                    cur = st_temp.pop()
                    jumps[cur[0]][2] = True  # 所有的if block里的都应该变为True
                    if cur[1] == "ENDIF":
                        # ENDIF保留默认设置
                        continue
                    elif cur[1] in ("IF", "ELIF", "ELSE"):
                        # 先默认每个if/elif/else失败，都跳到endif
                        # 如果发现有其他分支，为前一个条件赋予当前的index
                        jumps[cur[0]][1] = index
                        if prev:
                            jumps[prev[0]][1] = cur[0]
                        prev = cur
                    else:
                        # 非if/elif/else失败了，都到endif
                        jumps[cur[0]][1] = index
                        if st_temp[-1][1] in ("IF", "ELIF", "ELSE"):
                            # 如果下一步就是if/elif/else，那么pass就到endif
                            # 如果是st_temp的最后一步，那就按照默认值i+1,正常到endif
                            jumps[cur[0]][0] = index

            elif step.step_type == "ENDFOR":
                st_temp = []
                loop_index = index  # 循环体第一条语句的索引
                while True:
                    # 把for-endfor之间的语句正序
                    cur = st.pop()
                    st_temp.append(cur)
                    if cur[1] == "FOR":
                        if not st or st[-1][1] != "FOR":
                            # 循环开始可能有多个for语句
                            # 找到第一条for的index，每次应跳转到第一条for
                            # 这也标志着整个for block结束
                            loop_index = cur[0]
                            break
                ...
                while st_temp:
                    cur = st_temp.pop()
                    jumps[cur[0]][2] = True  # 所有的for block里的都应该变为True
                    if cur[1] == "ENDFOR":
                        # ENDIF保留默认设置
                        continue
                    elif cur[1] == "FOR":
                        # 任何一个for判断条件失败，都跳到endfor
                        jumps[cur[0]][1] = index
                    elif cur[1] == "IF":
                        # 只修改 if block内break的跳转，成功失败都跳到endfor
                        # 其余语句的jumps都不做修改，失败了仍然跳到endif，而不是跳到endfor
                        while cur[1] != "ENDIF":
                            cur = st_temp.pop()
                            if cur[1] == "BREAK":
                                jumps[cur[0]][0] = index
                        ...
                        # 非for statement和if block的任何一步失败了，都到endfor
                        jumps[cur[0]][1] = index
                        if st_temp[-1][1] == "ENDFOR":
                            jumps[cur[0]][0] = loop_index
                for_block = False

            elif step.step_type == "ENDTRY":
                # 记录catch语句的索引
                catch_index = -1
                while st:
                    cur = st.pop()
                    jumps[cur[0]][2] = True  # 所有语句设为True
                    if cur[1] == "ENDTRY":
                        continue
                    jumps[cur[0]][1] = index  # 默认失败了跳去endtry
                    if catch_index != -1:
                        # try-catch之间的语句失败了到catch
                        jumps[cur[0]][1] = catch_index
                    if cur[1] == "CATCH":
                        # catch前一句成功了到endtry
                        prev = st[-1]
                        jumps[prev[0]][0] = index
                        catch_index = cur[0]
                    if cur[1] == "TRY":
                        break
    ...
    def run(self, runner):
        ...
        if not skip_scenario_untested:
            jumps = self.jump_index()
            all_steps = list(self.all_steps)
            n = len(all_steps)
            i = 0
            while i < n:
                changed = False
                self.continue_after_failed_step = jumps[i][2]
                step = all_steps[i]
                step.status = Status.untested
                if run_steps:
                    if sr := not step.run(runner):
                        i = jumps[i][1]
                        changed = True
