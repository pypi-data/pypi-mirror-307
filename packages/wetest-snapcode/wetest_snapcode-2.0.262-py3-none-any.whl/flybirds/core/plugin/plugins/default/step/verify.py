...
import flybirds.core.plugin.plugins.default.ui_driver.poco.poco_findsnap as poco_find_snap
...
def wait_text_disappear(context, param):
    ...
    if "timeout" in param_dict.keys():
        ...
        optional["timeout"] = gr.get_frame_config_value("wait_ele_disappear", 10)
...
def ele_text_equal(context, param1, param2):
    ...
    handled_target_str = re.sub(pattern, "", target_str.replace("\u200b", ""))
    handled_selector_str = re.sub(pattern, "", selector_str.replace("\u200b", ""))
...
def wait_ele_appear(context, param):
    ...
    if "timeout" in param_dict.keys():
        ...
        optional["timeout"] = gr.get_frame_config_value("page_render_timeout", 30)
    optional["context"] = context
    poco_ele.wait_exists(poco_instance, selector_str, optional)
...
def wait_ocr_text_appear(context, param):
    timeout = gr.get_frame_config_value("page_render_timeout", 3)
    while timeout:
        flag = False
        matchRes = []
        ocr(context)
        ...
        for txt, line in zip(fixed_txt, g_Context.ocr_result):
            ...
            if line_param in line_txt:
                log.warn(
                    f"[ocr txt contain line replace] param: {param} found in txt: {txt}"
                )
                flag = True
            if flag:
                box = line[0]
                ...
                flag = False
        if matchRes:
            matchRes.sort(key=lambda x: (x[1], x[0]))
            return matchRes
        else:
            result = poco_ele.detect_error(context)
    ...
    message = "text not found in {} seconds, expect text:{}".format(timeout, param)
        ...
        for i in range(len(txt)):
            for key in paddle_fix:
                ...
                if key in origin_txt:
                    ...
                    log.info(
                        f"[paddle_fix_txt] paddle fix success origin_txt: {origin_txt}, fixed txt: {txt[i]}"
                    )
    # replace  blank value
    if trim is True:
        for i in range(len(txt)):
...
def ocr_regional_txt_exist(context, param1, param2):
    ...
    if len(g_Context.ocr_regional_result) >= 1:
        ...
        if "id=" in selector_str or "ID=" in selector_str:
            ...
            message = (
                "[ocr regional txt exist] please input regional id with format: id=xx"
            )
            raise FlybirdVerifyException(message)
        regional_id = int(str_list[1])
        regional_dic = list(
            filter(
                lambda item: item["id"] == regional_id, g_Context.ocr_regional_result
            )
        )
...
def ocr_regional_txt_contain(context, param1, param2):
    ...
    if len(g_Context.ocr_regional_result) >= 1:
        ...
        if "id=" in selector_str or "ID=" in selector_str:
            ...
            message = (
                "[ocr regional txt contain] please input regional id with format: id=xx"
            )
            raise FlybirdVerifyException(message)
        regional_id = int(str_list[1])
        regional_dic = list(
        ...
        for txt in fixed_txt:
            if trim_param in txt:
                log.info(
                    f"[ocr regional txt contain] param: {param2} found in txt: {txt}"
                )
                return True
            try:
                if re.search(param2, txt, flags=0) is not None:
                    log.info(
                        f"[ocr regional txt contain re] param: {param2} found in txt: {txt}"
                    )
                    return True
            except:
                pass
            ...
            if line_param in line_txt:
                log.warn(
                    f"[ocr regional txt contain line replace] param: {param2} found in txt: {txt}"
                )
                return True
