...
def air_bdd_ele_swipe(
    ...
    poco_object = pm.create_poco_object_by_dsl(poco, container_dsl_str, optional)
    ...
    if start_point[0] > 1:
        ...
        start_point[0] = (
            target_position[0] - target_size[0] / 2 + target_size[0] * start_point[0]
        )
    if start_point[1] > 1:
        start_point[1] = (
        ...
        start_point[1] = (
            target_position[1] - target_size[1] / 2 + target_size[1] * start_point[1]
        )
    ...
    max_x = (
        target_position[0] - target_size[0] / 2 + target_size[0] - 2 / screen_size[0]
    )
    max_y = (
        target_position[1] - target_size[1] / 2 + target_size[1] - 2 / screen_size[1]
    )
...
def air_bdd_direction_swipe(poco, start_point, direction, distance, duration=None):
    """
...
def full_screen_swipe_search(
    ...
    start_point = point_helper.get_swipe_search_start_point(direction, start_x, start_y)
...
def air_bdd_ele_drag(poco, container_dsl_str, optional, coord):
    """
    Drags a specified element to fixed coordinate.
    """
    poco_ele.wait_exists(poco, container_dsl_str, optional)
    poco_object = pm.create_poco_object_by_dsl(poco, container_dsl_str, optional)
    poco_object.start_gesture().hold(1).to(coord).up()
