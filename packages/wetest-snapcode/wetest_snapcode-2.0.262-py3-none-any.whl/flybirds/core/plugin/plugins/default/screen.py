...
class BaseScreen:
    ...
    def screen_link_to_behave(scenario, step_index, tag=None, link=True):
        ...
        feature_name = str(time.time())
        scenario_name = str(time.time())

        step_len = len(scenario.steps)
        if (
            scenario is not None
            and hasattr(scenario, "background_steps")
            and len(scenario.background_steps) > 0
        ):
        ...
        if step_len > step_index >= 0:
            ...
            log.debug(
                f"[screen_link_to_behave] screen_shot_dir path :"
    ...
    def image_ocr(
        ...
        regional_box, txts = BaseScreen.struct_ocr_result(
            g_Context.ocr_result,
            right_gap_max,
            left_gap_max,
            height_gap_max,
            skip_height_max,
    ...
    def struct_ocr_result(
        ...
        result = list(
            (n for n in result if n[0][0][1] > skip_height_max and n[1][0] != "0")
        )
        for line in result:
            struct_dic = {
                "box": None,
            ...
            right_result = BaseScreen.right_box_cal(
                right_gap_max, height_gap_max, box, result
            )
            if right_result is not None:
                struct_dic["right_box"] = right_result[0]
                struct_dic["right_txt"] = right_result[1]

            left_result = BaseScreen.left_box_cal(
                left_gap_max, height_gap_max, box, result
            )
            if left_result is not None:
                struct_dic["left_box"] = left_result[0]
                struct_dic["left_txt"] = left_result[1]

            bottom_result = BaseScreen.bottom_box_cal(
                index, height_gap_max, box, result
            )
        ...
        for key in regional_box:
            dic_list = list(
                filter(lambda item: item["regional_id"] == key, struct_ocr_result)
            )
            txts = []
            for dic in dic_list:
                txts.append(dic["txt"])
    ...
    def right_box_cal(gap_max, height_gap_max, box, result):
        ...
        while right_index < len(result):
            ...
            height_gap = (right_box[0][1] + right_box[3][1]) / 2 - (
                box[0][1] + box[3][1]
            ) / 2
            box_gap = right_box[0][0] - box[1][0]
            if (
                gap_max > box_gap > 0
                and height_gap_max * 1.5 > height_gap > -height_gap_max * 1.5
            ):
                matched = {
                    "right_box": right_box,
                    "right_txt": right_txt,
    ...
    def left_box_cal(gap_max, height_gap_max, box, result):
        ...
        while left_index < len(result):
            ...
            height_gap = (left_box[0][1] + left_box[3][1]) / 2 - (
                box[0][1] + box[3][1]
            ) / 2
    ...
    def bottom_box_cal(index, height_gap_max, box, result):
        ...
        while next_line_index < len(result):
            ...
            if (
                height_gap_max >= height_gap >= -height_gap_max
                and wide_length_flag is True
            ):
                matched = {
    ...
    def regional_division(original_list):
        ...
        for box_dic in original_list:
            ...
            for dic in dic_used:
                ...
                BaseScreen.bottom_box_check(
                    dic, regional_index, dic_used, original_list
                )
        ...
        while index < regional_index:
            dic_list = list(
                filter(lambda item: item["regional_id"] == index, original_list)
            )
            box_list = []
            for dic in dic_list:
                box = dic["box"]
            ...
            if len(box_list) > 0:
                box_0 = [
                    min(box_list, key=itemgetter("00")),
                    min(box_list, key=itemgetter("01")),
                ]
                box_1 = [
                    max(box_list, key=itemgetter("10")),
                    min(box_list, key=itemgetter("11")),
                ]
                box_2 = [
                    max(box_list, key=itemgetter("20")),
                    max(box_list, key=itemgetter("21")),
                ]
                box_3 = [
                    min(box_list, key=itemgetter("30")),
                    max(box_list, key=itemgetter("31")),
                ]
                regional_box = [
                    [box_0[0]["00"], box_0[1]["01"]],
    ...
    def right_box_check(box_dic, regional_index, dic_used, original_list):
        ...
        while right_box is not None and right_box not in right_used_box:
            ...
            next_dic = list(
                filter(lambda item: item["box"] == right_box, original_list)
            )
    ...
    def bottom_box_check(box_dic, regional_index, dic_used, original_list):
        ...
        while bottom_box is not None and bottom_box not in bottom_used_box:
            ...
            next_dic = list(
                filter(lambda item: item["box"] == bottom_box, original_list)
            )
