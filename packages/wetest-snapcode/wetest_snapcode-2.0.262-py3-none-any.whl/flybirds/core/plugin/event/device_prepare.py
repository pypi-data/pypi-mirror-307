...
class OnPrepare:
    ...
    def init_device(context):
        ...
        if device_id is not None:
            if gr.get_value(f"deviceInstance{device_id}") is not None:
                device_instance = gr.get_value(f"deviceInstance{device_id}")
            else:
                device_instance = device_manage.device_connect(device_id)
                gr.set_value("deviceInstance", device_instance)
                gr.set_value(f"deviceInstance{device_id}", device_instance)
