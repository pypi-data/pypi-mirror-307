...
class OnBefore:  # pylint: disable=too-few-public-methods
    ...
    def init_ui_driver(context):
        ...
        if device_id is not None:
            ...
            gr.set_value(f"pocoInstance{device_id}", poco_instance)
            ...
            gr.set_value(f"screenSize{device_id}", context.config_manage.device_info.screen_size)
            log.info(
    ...
    def init_ocr_driver(context):
        ...
        gr.set_value(f"ocrInstance{gr.get_device_id()}", ocr_instance)
    ...
    def run(context):
        ...
        try:
            ...
            try:
                ...
                poco = gr.get_value(f"pocoInstance{gr.get_device_id()}", None)
                if poco is not None:
                    poco.stop_running()
                gr.set_value(f"pocoInstance{gr.get_device_id()}", None)
                log.info("poco global object release")
                OnBefore.init_ui_driver(context)
...
class OnAfter:  # pylint: disable=too-few-public-methods
    ...
    def run(context):
        ...
        close screen record
        """
        GlobalContext.device.device_disconnect(gr.get_device_id())
        GlobalContext.ui_driver.close_driver()
