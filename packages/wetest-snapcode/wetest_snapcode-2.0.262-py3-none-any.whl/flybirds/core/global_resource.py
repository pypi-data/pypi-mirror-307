...
def get_app_package_name(def_value=None):
    ...
    if hasattr(_global_dict["configManage"].app_info, "package_name"):
        ...
        if value or (isinstance(value, str) and not str.isspace(value)):
            return value
    return _global_dict.get("packageName", "")
...
def get_cos_upload_dir():
    """
    Or the directory where the screenshots are stored
    """
    if hasattr(_global_dict["configManage"].report_info, "cos_dir"):
        return _global_dict["configManage"].report_info.screen_shot_dir
    else:
        return "car/test/img"
...
def get_test_envs():
    return get_value("test_env", {})


def get_test_env_value(key):
    t_env = get_value("test_env", {})
    return t_env.get(key, 0)


def set_test_env(env):
    set_value("test_env", env)
