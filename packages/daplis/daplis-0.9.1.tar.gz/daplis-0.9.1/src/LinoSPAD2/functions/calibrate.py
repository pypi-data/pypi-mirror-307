...
# TODO: probably not required - remove
# def calibrate_and_save_TDC_data_DNL_corrected(
#     path: str,
#     daughterboard_number: str,
#     motherboard_number: str,
#     firmware_version: str,
...
#                 # TODO
#                 file_DNL = os.path.join(
#                     r"C:\Users\bruce\Documents\GitHub\LinoSPAD2\src\LinoSPAD2\params\calibration_data",
#                     f"{daughterboard_number}_{motherboard_number}_{firmware_version}_DNL.npy",
#                 )
#                 M = np.load(file_DNL)
#                 counts_calib = M[tdc] @ counts
#                 cal_mat[i] = (
#                     np.cumsum(counts_calib)
#                     / np.cumsum(counts_calib).max()
#                     * 2500
#                 )

#             cal_mat_df = pd.DataFrame(cal_mat)
#             cal_mat_df.to_csv(
#                 f"TDC_DNL_{daughterboard_number}_{motherboard_number}_"
#                 f"{firmware_version}_{j}.csv"
#             )
...
#     # Remove the numbered '.csv' files
#     file_pattern = (
#         f"TDC_DNL_{daughterboard_number}_"
#         f"{motherboard_number}_{firmware_version}_*.csv"
#     )
#     files_to_delete = glob.glob(file_pattern)
#     for file_to_delete in files_to_delete:
#         os.remove(file_to_delete)
