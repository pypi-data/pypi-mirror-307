...
from holistic.datasets._dataset import Dataset
from holistic.datasets._utils import convert_float_to_categorical, get_protected_values
...
def load_lastfm_dataset():
    ...
    from holistic.utils import recommender_formatter
...
def load_german_credit_demo():
    import pandas as pd
    import pyarrow.parquet as pq
    import requests
    from io import BytesIO
    from holistic.datasets._dataset import Dataset, DatasetDict
    datasets_dict = {}
    for dataset_name in ['train','test']:
        urls = [
            f'https://huggingface.co/datasets/holistic-ai/german-credit-demo/resolve/main/data/{dataset_name}-00000-of-00001.parquet'
        ]

        # Lista para almacenar cada DataFrame de shard
        dfs = []

        # Descargar cada shard, leerlo y almacenarlo en la lista
        for url in urls:
            response = requests.get(url)
            response.raise_for_status()  # Verifica si la descarga fue exitosa
            file_buffer = BytesIO(response.content)
            df = pq.read_table(file_buffer).to_pandas()
            dfs.append(df)

        # Concatenar todos los DataFrames en uno solo
        datasets_dict[dataset_name] = Dataset(data=pd.concat(dfs, ignore_index=True))
...
def load_dataset(
    ...
    if dataset_name == "german_credit_demo":
        return load_german_credit_demo()
