...
class ProductExecutor(TestClass.TestClass):
    ...
    def getUniqueNumber(self, offset=0, where=None, keyName="SerialNumber"):
        from openProduction.station import Station
        from openProduction.reports import Reports
        
        station = Station.Station.getInstance()
        self.assertNotEqual(station, None, "Keine Station geladen")
        productName, productVersion, productRevision = self._getProductDetails(station)
        reporter = Reports.Report(station.serverIF)
        if where == None:
            where = station.getStationName()
        rv, data = reporter.getDeviceResults2(productName, productVersion, where, resultKeys=[keyName])
        if rv == ConnectorErrors.NOT_FOUND:
            serialNumber = offset
            self.logger.info("Keine Seriennummer gefunden, Start bei %d"%serialNumber)
        elif rv == ConnectorErrors.NO_ERROR:
            arr = data[keyName].dropna()
            if len(arr) > 0:
                serialNumber = max(arr) + 1
            else:
                serialNumber = offset
            self.logger.info("Setze Seriennummer auf %d"%serialNumber)
        else:
            raise RuntimeError("Datenbankabfrage liefert Fehler zurück: " + str(rv))

        self.values[keyName] = serialNumber

        return serialNumber

    def _getProductDetails(self, station):
        product = station.loadedProduct
        self.assertNotEqual(product, None, "Kein Produkt geladen")
        productName = product.name
        productVersion = product.version.version
        productRevision = product.version.revision
