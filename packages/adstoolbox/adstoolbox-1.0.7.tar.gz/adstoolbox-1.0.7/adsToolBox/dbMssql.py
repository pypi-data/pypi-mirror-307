...
class dbMssql(data_factory):
    ...
    def sqlQuery(self, query):
        ...
        try:
            ...
            self.logger.log_to_db("Requête de lecture exécutée.")
            if get_timer():
    ...
    def sqlExec(self, query):
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                self.logger.log_to_db("Requête exécutée avec succès.")
        except Exception as e:
    ...
    def sqlScalaire(self, query):
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                self.logger.log_to_db("Requête scalaire exécutée avec succès.")
                return data
        except Exception as e:
            self.logger.error(f"Échec de l'exécution de la requête: {e}")
            self.logger.log_to_db(str(e))
    ...
    def insert(self, table, cols=[], row=[]):
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                self.logger.log_to_db("Insertion réussie.")
                return ["SUCCESS"]
        except Exception as e:
    ...
    def insertMany(self, table, cols=[], rows=[]):
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                self.connection.commit()
                self.logger.info(f"{cursor.rowcount} lignes insérées avec succès dans la table {table}.")
                self.logger.log_to_db("Insertion(s) réussie(s).")
                return ["SUCCESS"]
        except Exception as e:
            self.logger.error(f"Échec de l'insertion des données: {e}")
            self.logger.log_to_db(str(e))
