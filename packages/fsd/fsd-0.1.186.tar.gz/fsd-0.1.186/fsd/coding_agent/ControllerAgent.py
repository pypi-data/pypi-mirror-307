...
class ControllerAgent:
    def __init__(self, repo):
        ...
        end_time = time.time()
        logger.info(f"IdeaDevelopment initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.short_idea = ShortIdeaDevelopment(repo)
        end_time = time.time()
        logger.info(f"ShortIdeaDevelopment initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.planCheckAgent = PlanCheckAgent(repo)
        end_time = time.time()
        logger.info(f"PlanCheckAgent initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.preprompt = PrePromptAgent(repo)
        end_time = time.time()
        logger.info(f"PrePromptAgent initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.taskPlanner = TaskPlanner(repo)
        end_time = time.time()
        logger.info(f"TaskPlanner initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.taskPlannerPro = TaskPlannerPro(repo)
        end_time = time.time()
        logger.info(f"TaskPlannerPro initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.coder = CodingAgent(repo)
        end_time = time.time()
        logger.info(f"CodingAgent initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.project = ProjectManager(repo)
        end_time = time.time()
        logger.info(f"ProjectManager initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.image = ImageControllerAgent(repo)
        end_time = time.time()
        logger.info(f"ImageControllerAgent initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.compile = CompileControllerAgent(repo)
        end_time = time.time()
        logger.info(f"CompileControllerAgent initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.runner = ProjectsRunner(repo)
        end_time = time.time()
        logger.info(f"ProjectsRunner initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.fileManager = FileManagerAgent(repo)
        end_time = time.time()
        logger.info(f"FileManagerAgent initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.fileFinder = FileFinderAgent(repo)
        end_time = time.time()
        logger.info(f"FileFinderAgent initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.deploy = DeploymentControllerAgent(repo)
        end_time = time.time()
        logger.info(f"DeploymentControllerAgent initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.code_manager = FileContentManager(repo)  # Initialize CodeManager in the constructor
        end_time = time.time()
        logger.info(f"FileContentManager initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.crawler = CrawlerAgent()
        end_time = time.time()
        logger.info(f"CrawlerAgent initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.crawlerPlaner = CrawlerTaskPlanner(repo)
        end_time = time.time()
        logger.info(f"CrawlerTaskPlanner initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.dependency = DependencyControllerAgent(repo)
        end_time = time.time()
        logger.info(f"DependencyControllerAgent initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.context = ContextPrepareAgent(repo)
        end_time = time.time()
        logger.info(f"ContextPrepareAgent initialization took {end_time - start_time:.2f} seconds\n")

        start_time = time.time()
        self.imageAgent = PromptImageUrlAgent(repo)
        end_time = time.time()
        logger.info(f"PromptImageUrlAgent initialization took {end_time - start_time:.2f} seconds\n")
