...
async def start(project_path):
    try:
        ...
        logger.info(f" #### GitRepo initialization took {time.time() - start_time:.2f} seconds\n")

        start_time = time.time()
        explainer_controller = ExplainerController(repo)
        logger.info(f" #### ExplainerController initialization took {time.time() - start_time:.2f} seconds\n")
        ...
        logger.info(f" #### FirstPromptAgent initialization took {time.time() - start_time:.2f} seconds\n")

        start_time = time.time()
        deploy = DeploymentCheckAgent(repo)
        logger.info(f" #### DeploymentCheckAgent initialization took {time.time() - start_time:.2f} seconds\n")

        start_time = time.time()
        explainer_controller.initial_setup()
        logger.info(f" #### ExplainerController setup took {time.time() - start_time:.2f} seconds\n")
