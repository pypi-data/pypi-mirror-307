...
async def start(project_path):
    try:
        ...
        end_time = time.time()
        logger.info(f"Repository initialization took {end_time - start_time:.2f} seconds")

        start_time = time.time()
        explainer_controller = ExplainerController(repo)
        end_time = time.time()
        logger.info(f"Explainer controller initialization took {end_time - start_time:.2f} seconds")

        start_time = time.time()
        coding_controller = ControllerAgent(repo)
        end_time = time.time()
        logger.info(f"Coding controller initialization took {end_time - start_time:.2f} seconds")

        start_time = time.time()
        first_prompt_controller = FirstPromptAgent(repo)
        end_time = time.time()
        logger.info(f"First prompt controller initialization took {end_time - start_time:.2f} seconds")

        start_time = time.time()
        deploy = DeploymentCheckAgent(repo)
        end_time = time.time()
        logger.info(f"Deployment check agent initialization took {end_time - start_time:.2f} seconds")

        start_time = time.time()
        explainer_controller.initial_setup()
        end_time = time.time()
        logger.info(f"Explainer initial setup took {end_time - start_time:.2f} seconds")
