...
async def start(project_path):
    try:
        ...
        # Initialize controllers concurrently
        explainer_controller, coding_controller, first_prompt_controller, deploy = await asyncio.gather(
            asyncio.to_thread(lambda: ExplainerController(repo)),
            asyncio.to_thread(lambda: ControllerAgent(repo)),
            asyncio.to_thread(lambda: FirstPromptAgent(repo)),
            asyncio.to_thread(lambda: DeploymentCheckAgent(repo))
        )
