...
class ExplainerController:
    ...
    def __init__(self, repo):
        ...
        logger.info(f"ExplainablePrePromptAgent initialization took {time.time() - start_time:.2f} seconds\n")
        
        start_time = time.time()
        self.normalExplainer = GeneralExplainerAgent(repo)
        logger.info(f"GeneralExplainerAgent initialization took {time.time() - start_time:.2f} seconds\n")
        
        start_time = time.time()
        self.mainExplainer = MainExplainerAgent(repo)
        logger.info(f"MainExplainerAgent initialization took {time.time() - start_time:.2f} seconds\n")
        
        start_time = time.time()
        self.fileFinder = ExplainableFileFinderAgent(repo)
        logger.info(f"ExplainableFileFinderAgent initialization took {time.time() - start_time:.2f} seconds\n")
        
        start_time = time.time()
        self.crawler = CrawlerAgent()
        logger.info(f"CrawlerAgent initialization took {time.time() - start_time:.2f} seconds\n")
        
        start_time = time.time()
        self.crawlerPlaner = CrawlerTaskPlanner(repo)
        logger.info(f"CrawlerTaskPlanner initialization took {time.time() - start_time:.2f} seconds\n")
        
        start_time = time.time()
        self.imageAgent = PromptImageUrlAgent(repo)
        logger.info(f"PromptImageUrlAgent initialization took {time.time() - start_time:.2f} seconds\n")
        
        start_time = time.time()
        self.actionCheck = ExplainableActionCheckAgent(repo)
        logger.info(f"ExplainableActionCheckAgent initialization took {time.time() - start_time:.2f} seconds\n")
        
        start_time = time.time()
        self.coder = ControllerAgent(repo)
        logger.info(f"ControllerAgent initialization took {time.time() - start_time:.2f} seconds\n")
