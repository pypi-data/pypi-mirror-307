...
class CompileControllerAgent:
    def __init__(self, repo):
        ...
        logger.info(f"CompileProjectAnalysAgent initialization took {time.time() - start_time:.2f} seconds\n")
        
        start_time = time.time()
        self.preprompt = CompilePrePromptAgent(repo)
        logger.info(f"CompilePrePromptAgent initialization took {time.time() - start_time:.2f} seconds\n")
        
        start_time = time.time()
        self.fileFinder = CompileFileFinderAgent(repo)
        logger.info(f"CompileFileFinderAgent initialization took {time.time() - start_time:.2f} seconds\n")
        
        start_time = time.time()
        self.guider = CompileGuiderAgent(repo)
        logger.info(f"CompileGuiderAgent initialization took {time.time() - start_time:.2f} seconds\n")
        
        start_time = time.time()
        self.taskPlanner = CompileTaskPlanner(repo)
        logger.info(f"CompileTaskPlanner initialization took {time.time() - start_time:.2f} seconds\n")
        
        start_time = time.time()
        self.command = CompileCommandRunner(repo)
        logger.info(f"CompileCommandRunner initialization took {time.time() - start_time:.2f} seconds\n")
        ...
        logger.info(f"OSEnvironmentDetector initialization took {time.time() - start_time:.2f} seconds\n")
