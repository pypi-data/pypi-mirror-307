...
def inject_clickthroughs(
    *, xml: str, urls: [str], default: str = ""
) -> Union[bytes, str]:
    """Inject the chained clickthrough redirect URLs in the VAST Linear ClickThrough element.

    Note that if no element exist, it will be created.
    The `default` is used when the element does not exist, or is empty.
    """
    try:
    ...
    try:
        ct_elmt = root.xpath("//Linear//ClickThrough")[0]
        clickthrough = ct_elmt.text
    except IndexError:
        linear_elmt = root.xpath("//Linear")[0]
        # No not override the existing VideoClicks element
        video_clicks = linear_elmt.xpath("./VideoClicks")
        if video_clicks:
            video_clicks_elmt = video_clicks[0]
        else:
            video_clicks_elmt = etree.SubElement(linear_elmt, "VideoClicks")
        ct_elmt = etree.SubElement(video_clicks_elmt, "ClickThrough")
        clickthrough = default

    # Chain the clickthrough redirects in order, so that the first one redirects to the second,
    # which redirects to the third, etc...
    for clr in reversed(urls):
        clickthrough = f"{clr}{quote_plus(clickthrough)}"

    ct_elmt.text = etree.CDATA(clickthrough)
