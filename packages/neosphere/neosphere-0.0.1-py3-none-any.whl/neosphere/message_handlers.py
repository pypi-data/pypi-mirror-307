...
class SafeDict:
    def __init__(self, initial_data: dict=None):
        logger.debug("Initializing SafeDict with data: ", initial_data, type(initial_data))
        self._data: dict = initial_data if initial_data is not None else {}
        logger.debug("SafeDict initialized with data: ", self._data, type(self._data))

    def data_dict(self):
        return self._data.copy()

    def get(self, key):
        logger.debug(f"Getting key: {key} from self._data: {type(self._data)}")
...
class AgentReceiver:
    ...
    async def on_connect(self):
        ...
        # auth_data: dict = {'type': 'userauth', 'code': token, 'connect_as': 'a.b.c-ee.com'}
        # auth_data: dict = {
        #     'cmd': 'aiagent',
        #     'code': self.connection_code, # You get this from the app from your AI Agent's profile
        #     'id': self.agent_share_id, # The agent's ID, displayed on Agent profile as niopub.com/x/john.doe
        #     'client_id': self.client_name
        # }
        # auth_data_js = json.dumps(auth_data)
        # logger.info("Authorizing agent connection...")
        # await self.ws.send(auth_data_js)
        # logger.info("Authorized! Getting reconnection token...")
        # await self.message_send_handler.send_token_request(self.connection_code, self.agent_share_id, self.client_name)
