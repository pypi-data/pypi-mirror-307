...
"""
    ...
    def get_user_defined_apps(self):
        django_apps_prefix = 'django.'
        user_apps = []

        for app_config in apps.get_app_configs():
            app_name = app_config.name
            app_path = str(app_config.path)  # Convert to string to avoid TypeError
            print(f"{app_name}: {app_path}")  # Debug print to show each app's path

            # Filter out Django and third-party apps
            if not app_name.startswith(django_apps_prefix) and app_path.startswith(str(settings.BASE_DIR)):
                user_apps.append(app_name)

        print("User-defined apps:", user_apps)
        return user_apps
