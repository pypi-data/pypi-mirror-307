...
class FPDF(GraphicsStateMixin, TextRegionMixin):
    ...
    def write_html(self, text, *args, **kwargs):
        ...
        html2pdf_copy = copy.deepcopy(html2pdf)
        with self.local_context():
            print(text)
            html2pdf_copy.feed(text)

            if html2pdf_copy.too_high == True:
                self.too_high = True
                self.row_info = html2pdf_copy.row_info
                raise FPDFException("The rows is too high to fit on the page")
            else:
                self.too_high = False
                self.row_info = None
                html2pdf.feed(text)
