...
class HubspotAuto(HubspotBase):
    ...
    def forward(self, node_inputs: InputSchema):
        ...
        for company in node_inputs.input.companies:
            company_entity, is_update = HubspotCompanyCreate.create_or_update_entity(
                self.service,
                company,
                None,
            ...
            if is_update:
                actions.append(f"Company {domain} updated")
            else:
                actions.append(f"Company {domain} created")

        # then create the contacts
        contacts = {}
        for contact in node_inputs.input.contacts:
            contact_entity, is_update = HubspotContactCreate.create_or_update_entity(
                self.service,
                contact,
                None,
            ...
            if is_update:
                actions.append(f"Contact {email} updated")
            else:
                actions.append(f"Contact {email} created")

        # then create the deals
        deals = {}
        for deal in node_inputs.input.deals:
            deal_entity, is_update = HubspotDealCreate.create_or_update_entity(
                self.service,
                deal,
                None,
            ...
            if is_update:
                actions.append(f"Dela {dealname} updated")
            else:
                actions.append(f"Deal {dealname} created")
