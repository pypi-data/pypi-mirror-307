...
class Srv21():
    ...
    def query_to_df(self,session,query):
        df = pd.DataFrame(session.execute(text(query)).fetchall())
        return df

    def update_sql(self,session,query):
        q = text(query)
        session.execute(q)
        session.commit()
        return True
...
def create_server1():
  ...
  server = MyServer(keys, trusted, filename)
  setup_s21(server.keys, server.trusted, server.filename, dataOut=True)
