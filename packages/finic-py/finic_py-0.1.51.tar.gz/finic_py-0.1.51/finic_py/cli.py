...
def finic_init():
    ...
    print(
        f"The /finic_tasks directory has been created.\ncd into it and run `poetry install` to install dependencies, then `finic record` to create your first automation."
    )
...
def record(url):
    ...
    with open("main.py", "r") as f:
        if not re.search(r"@entrypoint\s*$", f.read(), re.MULTILINE):
