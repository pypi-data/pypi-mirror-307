...
def check_command_exists(command):
    ...
    result = subprocess.run(
        ["where" if os.name == "nt" else "command", "-v", command],
    ...
    while True:
        ...
        if change:
            ...
            if event_type == 'modified':
                print(f"{Fore.YELLOW}File modified: {event_path}{Fore.RESET}")
            elif event_type == 'created':
                print(f"{Fore.YELLOW}File created: {event_path}{Fore.RESET}")
            elif event_type == 'deleted':
                print(f"{Fore.YELLOW}File deleted: {event_path}{Fore.RESET}")
...
def main():
    ...
    if not args.command:
        ...
        while True:
            ...
            print(f"{Fore.WHITE}[{Fore.GREEN}09{Fore.WHITE}] Logout{Fore.RESET}")
        ...
        if args.command == "deploy":
            ...
            print(f"{Fore.CYAN}Tracking changes in directory: {args.directory}{Fore.RESET}")
            track_changes(args.directory)
