...
from db.models import Workspace, Workflow, WorkflowRun, WorkflowRunLog

from gcs import save_logs_to_bucket_from_redis
from config import LOGS_BUCKET, REDIS_HOST, REDIS_PORT
...
class DocketWorkflow:
    ...
    @staticmethod
    def update(name:str, state:WorkflowState) -> Workflow:
        session = PgDbManager.get_session()
        wf2update = session.query(Workflow).filter(and_(Workflow.name == name, 1 == 1)).first()
        wf2update.state = state.value
        if state == WorkflowState.DRAFT:
            pass
        if state == WorkflowState.PUBLISHED:
            wf2update.published_dt = datetime.now()
        if state == WorkflowState.ARCHIVED:
            wf2update.archived_dt = datetime.now()
        session.commit()
        return wf2update
