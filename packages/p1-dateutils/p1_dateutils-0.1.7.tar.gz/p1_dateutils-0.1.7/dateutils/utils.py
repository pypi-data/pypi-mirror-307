...
K_TYPE_DATE_OR_DATETIME = TypeVar('K_TYPE_DATE_OR_DATETIME', _date, _datetime)


class relativedeltautils:

    @classmethod
    def create(cls, dt1=None, dt2=None,
        ...
        return _relativedelta(dt1=dt1, dt2=dt2, years=years, months=months, days=days,
                              leapdays=leapdays, weeks=weeks, hours=hours, minutes=minutes, seconds=seconds, microseconds=microseconds,
                              year=year, month=month, day=day, weekday=weekday,
                              yearday=yearday, nlyearday=nlyearday,
                              hour=hour, minute=minute, second=second, microsecond=microsecond)

    @classmethod
    def normalized(cls, relativedelta):
        return _relativedelta.normalized(relativedelta)
...
class dateutils:
    ...
    @classmethod
    def today(cls) -> _date:
        return datetimeutils.date(datetimeutils.now())
...
class timeutils:
    ...
    def make_aware(cls, time: _time) -> _time:
        ...
        if time.tzinfo == DEFAULT_TIMEZONE:
            return time
        temp_datetime = datetimeutils.combine(
            datetimeutils.today(), time).astimezone(DEFAULT_TIMEZONE)
        return temp_datetime.timetz()
...
class datetimeutils:
    ...
    @classmethod
    def make_aware(cls, datetime: K_TYPE_DATE_OR_DATETIME) -> K_TYPE_DATE_OR_DATETIME:
    ...
    def strftime(cls, datetime: _datetime, fmt: str) -> str:
        return cls.make_aware(datetime).strftime(fmt)
