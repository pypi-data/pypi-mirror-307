...
logging.basicConfig(filename=f'{__LOG_DIR}/__system__.log', encoding='utf-8', level=logging.DEBUG, format='%(asctime)s | %(name)s | %(levelname)s | %(thread)d | %(message)s')
logging.handlers.RotatingFileHandler(f"{__LOG_DIR}/__system__.log", maxBytes=10*MEGA_BYTE, backupCount=50)

class SysLogging():
    def __init__(self, filename:str, logging_level=TypeLogging.DEBUG):
        
        self.__filename = f'{__LOG_DIR}/ITSHelper_{filename}.log'
        ...
        handler = RotatingFileHandler(self.__filename, maxBytes=10*MEGA_BYTE, backupCount=50)
