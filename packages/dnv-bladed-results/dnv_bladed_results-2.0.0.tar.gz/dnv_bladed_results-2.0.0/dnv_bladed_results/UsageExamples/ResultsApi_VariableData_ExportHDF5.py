...
# This example requires the h5py library, available via pip.  It has been tested with h5py 3.11.0.
...
def run_script():
    ...
    - Exports data for all variables in every output group in a run to an HDF5 data file.
    - Exports data for all variables in an output group to an HDF5 data file.
    ...
    export_all_variables_in_group(source_run_directory, source_run_name, "Blade 1 Loads: Principal axes", os.path.join(output_directory, "Blade_1_Loads.h5").replace("\\","/"))
...
def create_axes(h5_file: h5py.File, h5_variables_group: h5py.Group, group: Group, variable_names_in_group: np.ndarray[str]):
    ...
    for independent_variable in independent_variables_in_group:
        ...
        if independent_variable.get_axis_type() == AXIS_TYPE_LABELLED_STRING:
