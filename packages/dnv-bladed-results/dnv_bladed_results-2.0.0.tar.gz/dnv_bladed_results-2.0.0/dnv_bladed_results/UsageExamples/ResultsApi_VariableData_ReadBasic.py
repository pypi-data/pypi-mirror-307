...
def run_script():
    ...
    - Gets data for a 1D or 2D variable without specifying the output group.
    - Gets data for a 1D or 2D variable from a specific output group (useful when two or more variables have the same name in a run).
    - Gets data for all 1D or 2D variables from a specific output group.
...
def get_data_for_all_2d_variables_from_specific_group(run_directory, run_name, group_name):
    ...
    try:
        ...
        independent_variable_values = group.get_independent_variable(INDEPENDENT_VARIABLE_ID_SECONDARY).get_values_as_string()
