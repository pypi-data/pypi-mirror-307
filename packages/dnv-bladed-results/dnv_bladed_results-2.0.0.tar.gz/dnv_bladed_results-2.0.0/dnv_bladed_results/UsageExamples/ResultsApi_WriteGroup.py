...
def write_1d_output_group_from_new_data(output_directory, output_run_name):
    ...
    print("\nSuccessfully created 1D group using hard-coded data, containing one variable and {0} points:\n{1}".format(
          str(independent_variable.get_number_of_values()),
...
def write_2d_output_group_from_new_data(output_directory, output_run_name):
    ...
    print("\nSuccessfully created 2D group using hard-coded data, containing one variable, {0} secondary axis values, and {1} points:\n{2}".format(
        str(secondary_independent_variable.get_number_of_values()),
        str(primary_independent_variable.get_number_of_values()),
...
def write_1d_output_group_from_existing_data(source_run_directory, source_run_name, output_directory, output_run_name):
    ...
    output_group_1d.add_variable_with_data(rotating_hub_fx.get_name(), pound_force_units, rotating_hub_fx_data_modified)
    output_group_1d.add_variable_with_data(rotating_hub_fy.get_name(), pound_force_units, rotating_hub_fy_data_modified)
    output_group_1d.add_variable_with_data(rotating_hub_fz.get_name(), pound_force_units, rotating_hub_fz_data_modified)
    output_group_1d.add_variable_with_data(rotating_hub_fyz.get_name(), pound_force_units, rotating_hub_fyz_data_modified)
    ...
    prove_1d_roundtrip(rotating_hub_fx_data_original, rotating_hub_fx.get_name(), output_directory, output_run_name, conversion_factor)
    prove_1d_roundtrip(rotating_hub_fy_data_original, rotating_hub_fy.get_name(), output_directory, output_run_name, conversion_factor)
    prove_1d_roundtrip(rotating_hub_fz_data_original, rotating_hub_fz.get_name(), output_directory, output_run_name, conversion_factor)
    prove_1d_roundtrip(rotating_hub_fyz_data_original, rotating_hub_fyz.get_name(), output_directory, output_run_name, conversion_factor)

    print("\nSuccessfully created 1D group using data transformed from an existing group, containing {0} variables and {1} points:\n{2}".format(
          str(output_group_1d.get_number_of_variables()),
          str(independent_variable.get_number_of_values()),
          output_group_1d.get_path()))
...
def write_2d_output_group_from_existing_data(source_run_directory, source_run_name, output_directory, output_run_name):
    ...
    # Re-use existing primary independent variable (interval axis) representing time
    primary_independent_variable = tower_mx.get_independent_variable(INDEPENDENT_VARIABLE_ID_PRIMARY)

    # Re-use existing secondary independent variable (axis with string labels) representing measurement locations
    secondary_independent_variable = tower_mx.get_independent_variable(INDEPENDENT_VARIABLE_ID_SECONDARY)
    ...
    # Read data as NumPy array
    tower_mx_data_original = tower_mx.get_data_for_all_independent_variable_values()
    tower_my_data_original = tower_my.get_data_for_all_independent_variable_values()
    tower_mz_data_original = tower_mz.get_data_for_all_independent_variable_values()
    tower_myz_data_original = tower_myz.get_data_for_all_independent_variable_values()
    ...
    output_group_2d.add_variable_with_data(tower_mx.get_name(), foot_pounds_units, tower_mx_data_modified)
    output_group_2d.add_variable_with_data(tower_my.get_name(), foot_pounds_units, tower_my_data_modified)
    output_group_2d.add_variable_with_data(tower_mz.get_name(), foot_pounds_units, tower_mz_data_modified)
    output_group_2d.add_variable_with_data(tower_myz.get_name(), foot_pounds_units, tower_myz_data_modified)
    ...
    prove_2d_roundtrip(tower_mx_data_original, tower_mx.get_name(), output_directory, output_run_name, conversion_factor)
    prove_2d_roundtrip(tower_my_data_original, tower_my.get_name(), output_directory, output_run_name, conversion_factor)
    prove_2d_roundtrip(tower_mz_data_original, tower_mz.get_name(), output_directory, output_run_name, conversion_factor)
    prove_2d_roundtrip(tower_myz_data_original, tower_myz.get_name(), output_directory, output_run_name, conversion_factor)
    ...
    print("\nSuccessfully created 2D group using data transformed from an existing group, containing {0} variables, {1} secondary axis values, and {2} points:\n{3}".format(
        str(output_group_2d.get_number_of_variables()),
        str(secondary_independent_variable.get_number_of_values()),
        str(primary_independent_variable.get_number_of_values()),
        output_group_2d.get_path()))
