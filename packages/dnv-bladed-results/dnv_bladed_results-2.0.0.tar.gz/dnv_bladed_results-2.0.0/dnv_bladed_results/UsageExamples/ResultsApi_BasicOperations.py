...
def get_independent_variables_from_group(run: Run, group_1d_name, group_2d_name):
    ...
    primary_ind_var_1d = run.get_group(group_1d_name).get_independent_variable(INDEPENDENT_VARIABLE_ID_PRIMARY)

    # Get the primary independent variable from a 2D group using independent variable identifier
    primary_ind_var_2d = run.get_group(group_2d_name).get_independent_variable(INDEPENDENT_VARIABLE_ID_PRIMARY)
    ...
    secondary_ind_var_2d = run.get_group(group_2d_name).get_independent_variable(INDEPENDENT_VARIABLE_ID_SECONDARY)
...
def get_independent_variables_from_variable(run: Run, variable_1d_name, variable_2d_name):
    ...
    # Get the primary independent variable from a 2D group using independent variable identifier
    primary_ind_var_2d = run.get_variable_2d(variable_2d_name).get_independent_variable(INDEPENDENT_VARIABLE_ID_PRIMARY)

    # Secondary independent variable
    # Get the secondary independent variable from a 2D group using independent variable identifier
    secondary_ind_var_2d = run.get_variable_2d(variable_2d_name).get_independent_variable(INDEPENDENT_VARIABLE_ID_SECONDARY)
