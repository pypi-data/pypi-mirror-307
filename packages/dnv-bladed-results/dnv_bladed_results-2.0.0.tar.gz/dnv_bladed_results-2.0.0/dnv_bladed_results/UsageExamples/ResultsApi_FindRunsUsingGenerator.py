...
def find_runs_using_generator(root_folder):
    ...
    ResultsApi.SearchSettings.completion_state_filter = COMPLETION_STATE_UNSUCCESSFUL_RUNS_ONLY
    ...
    ResultsApi.SearchSettings.completion_state_filter = COMPLETION_STATE_ALL_COMPLETION_STATES

    #########################################################
    #  Iterate all turbine simulation runs using generator  #
    #########################################################

    ResultsApi.SearchSettings.calculation_type_filter = CALCULATION_TYPE_ALL_TURBINE_SIMULATIONS
    iterator = RunsIterator(root_folder)
    turbine_simulation_runs = runs_generator(iterator)
    display_runs("turbine simulation runs", turbine_simulation_runs)
    ResultsApi.SearchSettings.calculation_type_filter = CALCULATION_TYPE_ALL_CALCULATION_TYPES

    ###################################################
    #  Find all post-processing runs using generator  #
    ###################################################

    ResultsApi.SearchSettings.calculation_type_filter = CALCULATION_TYPE_ALL_POST_PROCESSING_CALCULATIONS
    iterator = RunsIterator(root_folder)
