...
def abandon_abnormal_ycl(unique_data):
    ...
    for index, item in enumerate(unique_data):
        if index > 0:
            is_normal = time_delta_check(item, unique_data, index, interval_1_min,
                                         threshold_1_min) and time_delta_check(item, unique_data, index, interval_2_min,
                                                                               threshold_2_min) and time_delta_check(
                item, unique_data, index, interval_3_min, threshold_3_min)
            ...
            if recover_index and index - recover_index < 5:
                ...
                if filtered_time_ycls:
                    ...
                    last_normal_item = {'time': get_time_str(get_time(unique_data[0]['time']) - datetime.timedelta(seconds=30)), 'value': item['value']}
                ...
                if index > 20:
                    ...
                    if index >= len(ori_v_a):
                        print('超限了')
                    for i in range(index - 20, index + 1) :
                        if i < len(ori_v_a):
                            distance_list = np.array([point_to_line_distance(i + 20 - index, ori_v_a[i], slope, truncate)])
                    # distance_list = np.array(
                    #     [point_to_line_distance(i + 20 - index, ori_v_a[i], slope, truncate) if i < len(ori_v_a) else for i in
