...
def consume_gembatch_job(db: fs.Client, model: str):
    ...
    logger.debug("current time: %s", dt.datetime.now(tz=dt.timezone.utc).isoformat())
    logger.debug(
        "last submit time: %s, interval: %d",
        last_submit_time.isoformat(),
        configs.GEMBATCH_BATCH_INTERVAL_SECONDS.value,
    )
    if count >= configs.GEMBATCH_MAX_REQUESTS_PER_BATCH.value:
        ...
        logger.debug("delta: %s", delta.seconds)
        q = utils.CloudRunQueue.open(
            "consumeGemBatchJob",
            delay_seconds=max(0, delta.seconds) + 1,
        )
        q.run(model=model)
