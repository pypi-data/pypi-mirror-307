...
class GeminiBatchJob:
    ...
    def __init__(self, uid: str, model: str, created_at: dt.datetime | None = None):
    ...
    @classmethod
    def from_bq_event(cls, event: cloud_event.CloudEvent):
        ...
        db = firestore.client()
        query = (
            db.collection(configs.GEMBATCH_FIRESTORE_BATCH_QUEUE_COLLECTION)
            .where(filter=fs.FieldFilter("destination", "==", uri))
            .limit(1)
        )
        results = query.get()
        if not results:
    ...
    def from_doc(cls, doc: fs.DocumentSnapshot):
        ...
        return cls(job.uuid, job.model, job.created_at)
    ...
    def poll_job_state(self) -> models.StatusEnum:
        token = self._app.credential.get_access_token().access_token
        res = requests.get(
            f"https://us-central1-aiplatform.googleapis.com/v1/{self.display_name}",
            headers={"Authorization": f"Bearer {token}"},
            timeout=60,
        )
