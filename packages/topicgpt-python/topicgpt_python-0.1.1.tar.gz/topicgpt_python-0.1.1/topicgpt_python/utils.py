...
class TopicTree:
    ...
    def update_tree(self, original_topics, new_topic_name, new_topic_desc):
        """
        Update the topic tree by merging a set of topics into a new topic.
        ...
        """
        # Initialize cumulative count for the new topic
        total_count = 0
        parent_node = None
        nodes_to_merge = []

        # Find all nodes in original_topics and accumulate their counts
        ...
        merged_topic_node = next(
            (node for node in parent_node.children if node.name == new_topic_name), None
        )
        ...
        # Remove original topics from the tree, except for the merged topic
...
def sample_data(data, out_file, num_sample=100):
    """
    Sample training data from the full dataset

    Parameters:
    - data: Path to the full dataset
    - out_file: Path to save the sampled data
