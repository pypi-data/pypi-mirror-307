...
class CalibrateSingle(
    ...
    def _refine(
        ...
        force_fixed = calibrate_utils.parse_fixed(self.inputs.fixed, parametrization)
        ...
        for i, max_rings in enumerate(max_rings_list):
            for j, fixed_parameters in enumerate(it_fixed_parameters):
                ...
                self.progress = (i * n_rings + j) / (n * n_rings) * 100
        return setup.geometry_refinement
