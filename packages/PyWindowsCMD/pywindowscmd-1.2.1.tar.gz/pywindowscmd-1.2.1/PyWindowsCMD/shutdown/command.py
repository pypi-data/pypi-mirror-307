...
def build_shutdown_command(
	...
	Args:
		...
		target_computer (str): The target computer's IPv4 address. Defaults to None.
		...
		shutdown_reason (ShutdownReason): The reason for the shutdown. Defaults to None.
	...
	if enable_fast_startup and shutdown_type != ShutdownType.shutdown:
		raise WrongCommandLineParameter(
				"\"prepare_for_fast_startup\" parameter used only with type \"shutdown\"."
		)
	...
	if shutdown_type == ShutdownType.abort_shutting_down and count_parameters(
			log_shutdown_reason,
			show_advanced_boot_options,
			target_computer,
			force_running_applications_to_close,
		...
		raise WrongCommandLineParameter(
				"Type \"abort shutting down\" has to be alone or with parameter \"fw\"."
		)
	...
	if not (0 <= time_out_period <= 315360000):
		...
		raise WrongCommandLineParameter(
				"\"f\" parameter has to be used only with parameter \"t\" > 0."
		)
