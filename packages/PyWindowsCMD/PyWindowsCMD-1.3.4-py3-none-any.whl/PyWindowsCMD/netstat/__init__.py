...
from PyWindowsCMD.netstat.command import build_netstat_connections_list_command, build_netstat_ethernet_statistics_command, build_netstat_per_protocol_statistics_command, build_netstat_routing_table_command
#
#
#
#
def read_tcp_ipv6_statistics(cmd_output: str):
    """
    ...
    """
    tcp_ipv6_statistics = re.search(
            r"TCP Statistics for IPv6(?:\r\n)+(.+?)(?:(?:\r\n){2}|\Z)",
            cmd_output,
            re.DOTALL
    ).group(1)
    tcp_ipv6_statistics = re.findall(r"(\w+(?: \w+)*)\s{2,}= (\d+)", tcp_ipv6_statistics)
...
#
#
def read_tcp_ipv4_statistics(cmd_output: str):
    """
    ...
    """
    tcp_ipv4_statistics = re.search(
            r"TCP Statistics for IPv4(?:\r\n)+(.+?)(?:(?:\r\n){2}|\Z)",
            cmd_output,
            re.DOTALL
    ).group(1)
    tcp_ipv4_statistics = re.findall(r"(\w+(?: \w+)*)\s{2,}= (\d+)", tcp_ipv4_statistics)
...
#
#
def read_udp_ipv6_statistics(cmd_output: str):
    """
    ...
    """
    udp_ipv6_statistics = re.search(
            r"UDP Statistics for IPv6(?:\r\n)+(.+?)(?:(?:\r\n){2}|\Z)",
            cmd_output,
            re.DOTALL
    ).group(1)
    udp_ipv6_statistics = re.findall(r"(\w+(?: \w+)*)\s{2,}= (\d+)", udp_ipv6_statistics)
...
#
#
def read_udp_ipv4_statistics(cmd_output: str):
    """
    ...
    """
    udp_ipv4_statistics = re.search(
            r"UDP Statistics for IPv4(?:\r\n)+(.+?)(?:(?:\r\n){2}|\Z)",
            cmd_output,
            re.DOTALL
    ).group(1)
    udp_ipv4_statistics = re.findall(r"(\w+(?: \w+)*)\s{2,}= (\d+)", udp_ipv4_statistics)
...
#
#
def read_icmpv6_statistics(cmd_output: str):
    """
    ...
    """
    icmpv6_statistics = re.search(
            r"ICMPv6 Statistics(?:\r\n)+(.+?)(?:(?:\r\n){2}|\Z)",
            cmd_output,
            re.DOTALL
    ).group(1)
    icmpv6_statistics = re.findall(r"(\w+(?: \w+)*)\s{2,}(\d+)\s{2,}(\d+)", icmpv6_statistics)
...
#
#
def read_icmpv4_statistics(cmd_output: str):
    """
    ...
    """
    icmpv4_statistics = re.search(
            r"ICMPv4 Statistics(?:\r\n)+(.+?)(?:(?:\r\n){2}|\Z)",
            cmd_output,
            re.DOTALL
    ).group(1)
    icmpv4_statistics = re.findall(r"(\w+(?: \w+)*)\s{2,}(\d+)\s{2,}(\d+)", icmpv4_statistics)
...
#
#
def read_ipv6_statistics(cmd_output: str):
    """
    ...
    """
    ipv6_statistics = re.search(
            r"IPv6 Statistics(?:\r\n)+(.+?)(?:(?:\r\n){2}|\Z)",
            cmd_output,
            re.DOTALL
    ).group(1)
    ipv6_statistics = re.findall(r"(\w+(?: \w+)*)\s{2,}= (\d+)", ipv6_statistics)
...
#
#
def read_ipv4_statistics(cmd_output: str):
    """
    ...
    """
    ipv4_statistics = re.search(
            r"IPv4 Statistics(?:\r\n)+(.+?)(?:(?:\r\n){2}|\Z)",
            cmd_output,
            re.DOTALL
    ).group(1)
    ipv4_statistics = re.findall(r"(\w+(?: \w+)*)\s{2,}= (\d+)", ipv4_statistics)
...
#
#
def read_per_protocol_statistics(cmd_output: str):
    """
    ...
    Returns:
        dict[str, pandas.DataFrame]: A dictionary containing DataFrames for each protocol's statistics.
...
def get_per_protocol_statistics(
    ...
    Args:
        protocol (Literal["TCP", "UDP", "TCPv6", "UDPv6", "IP", "IPv6", "ICMP", "ICMPv6"]):  The protocol to retrieve statistics for. If None, retrieves statistics for all protocols. Defaults to None.
...
def read_interface_routing_table(cmd_output: str):
    ...
    """
    interface_routing_table = re.search(
            r"Interface List(?:\r\n)+(.+?)(?:={3,}|\Z)",
            cmd_output,
            re.DOTALL
    ).group(1)
...
#
#
def read_ethernet_statistics(cmd_output: str):
