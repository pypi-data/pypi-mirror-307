...
class TestSpecialFunctions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.z_re = torch.linspace(0.01, 5, 500, dtype=torch.float32)
        self.z_im = 1j * torch.linspace(0.01, 5, 500, dtype=torch.complex64)
        ...
        for device in self.devices:
            for n in torch.arange(-3, 4):
                ...
                j_scipy = torch.as_tensor(
                    jv(n, self.z_re.detach().cpu().numpy()), device=device
                )
                j_torch = special.Jn(n, z_re)
                y_scipy = torch.as_tensor(
                    yv(n, self.z_re.detach().cpu().numpy()), device=device
                )
                ...
                j_scipy_im = torch.as_tensor(jv(n, tg.to_np(self.z_im)), device=device)
                j_torch_im = special.Jn(n, z_im)
                y_scipy_im = torch.as_tensor(yv(n, tg.to_np(self.z_im)), device=device)
    ...
    def test_modbessel(self):
        for device in self.devices:
            for n in torch.arange(-3, 4):
                ...
                i_scipy = torch.as_tensor(iv(n, tg.to_np(self.z_re)), device=device)
                i_torch = special.In(n, z_re)
                k_scipy = torch.as_tensor(kv(n, tg.to_np(self.z_re)), device=device)
        ...
        for device in self.devices:
            for n in torch.arange(-3, 4):
                ...
                h1_scipy = torch.as_tensor(
                    hankel1(n, tg.to_np(self.z_re)), device=device
                )
                h1_torch = special.H1n(n, z_re)
                h2_scipy = torch.as_tensor(
                    hankel2(n, tg.to_np(self.z_re)), device=device
                )
                ...
                h1_scipy_im = torch.as_tensor(
                    hankel1(n, tg.to_np(self.z_im)), device=device
                )
                h1_torch_im = special.H1n(n, z_im)
                h2_scipy_im = torch.as_tensor(
                    hankel2(n, tg.to_np(self.z_im)), device=device
                )
