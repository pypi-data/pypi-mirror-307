...
def decomposition_exact(
    ...
    **kwargs,
    ...
    Raises:
        ValueError: Simulation has not been run yet, or magnetic polarizabilities are present.
    ...
    md = -1j * f_m * torch.sum(k * r_cross_P * j1kr, axis=1)
...
def scs(
    ...
    **kwargs,
    ...
    scs_md = sc_factor_dp * torch.sum(torch.abs(mpd[which_md]) ** 2, dim=1)
    scs_eq = sc_factor_Q * torch.sum(torch.abs(k * mpd[which_eq]) ** 2, dim=(1, 2))
    scs_mq = sc_factor_Q * torch.sum(torch.abs(k * mpd[which_mq]) ** 2, dim=(1, 2))

    return dict(scs_ed=scs_ed, scs_md=scs_md, scs_eq=scs_eq, scs_mq=scs_mq)
...
def ecs(
    ...
    *Caution:*
    ...
    prefactor = (4.0 * torch.pi * k0 * 1 / n_env_at_r0 * 1 / e2in).real

    ecs_ed = prefactor * (torch.sum(e_inc_cj * mpd[which_ed], dim=1)).imag

    ecs_md = prefactor / n_env_at_r0 * (torch.sum(h_inc_cj * mpd[which_md], dim=1)).imag
    ...
    ecs_mq = (prefactor / (6.0 * n_env_at_r0)) * (
