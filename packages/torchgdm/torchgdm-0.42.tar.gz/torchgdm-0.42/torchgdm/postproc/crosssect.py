...
def acs(
    ...
    **kwargs,
    ...
    if radiative_correction:
        n_env_p = n_env_at_p.unsqueeze(0).unsqueeze(-1)
        rad_corr_p = (
            abs_factor_p
            * (2 / 3)
            * k0**3
            * (torch.multiply(n_env_p * p, torch.conj(p)))
        ).sum((1, 2))
        cs_abs_p = cs_abs_p - rad_corr_p

        n_env_m = n_env_at_m.unsqueeze(0).unsqueeze(-1)
        rad_corr_m = (
            abs_factor_m
            * (2 / 3)
            * k0**3
            * (torch.multiply(n_env_m**2 * m, torch.conj(m)))
        ).sum((1, 2))
