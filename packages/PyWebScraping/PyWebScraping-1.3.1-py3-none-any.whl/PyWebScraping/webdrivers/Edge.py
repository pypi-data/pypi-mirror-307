...
class EdgeOptionsManager(BrowserOptionsManager):
	...
	def __init__(
		...
		Args:
			debugging_port (int): Port for remote debugging.
			user_agent (list[str]): User agent string or list of strings.
			proxy (str | list[str]): Proxy server address or list of addresses.
...
class EdgeStartArgs(BrowserStartArgs):
	...
	def __init__(
		...
		Args:
			webdriver_dir (str): Directory of the webdriver executable.
			debugging_port (int): Port for remote debugging.
			headless_mode (bool): Run Edge in headless mode.
			mute_audio (bool): Mute audio in Edge.
...
class EdgeWebDriver(BrowserWebDriver):
	...
	def __init__(
		...
		Args:
			...
			webdriver_start_args (EdgeStartArgs): Startup arguments for Edge.
			webdriver_options_manager (EdgeOptionsManager): Options manager for Edge.
...
class EdgeRemoteWebDriver(EmptyWebDriver):
	...
	def __init__(
		...
		"""
		Initializes a new instancе of the EdgeRemoteWebDriver class.

		Args:
			command_executor (str): The address of the remote webdriver server.
			session_id (str): The session ID of the existing webdriver session.
			webdriver_options_manager (EdgeOptionsManager):  An instance of EdgeOptionsManager for customizing browser options.
			implicitly_wait (int): The implicit wait timeout in seconds.
			page_load_timeout (int): The page load timeout in seconds.
	...
	def create_driver(self, command_executor: str = None, session_id: str = None):
		...
		Args:
			command_executor (str): URL of the remote webdriver server. If provided will overwrite server URL during initialization. Defaults to None.
			session_id (str): ID of an existing session to connect to. If provided, will overwrite session ID specified during initialization. Defaults to None.
