...
class asn1toolParseTest(unittest.TestCase):

    maxDiff = None

    def parse_and_verify(self, module, path='.'):
        asn_path = 'tests/files/' + path + '/' + module + '.asn'
        actual = asn1tool.parse_files(asn_path)
    ...
    def test_parse_imports_global_module_reference(self):
        actual = asn1tool.parse_string('A DEFINITIONS ::= BEGIN '
                                        'IMPORTS '
                                        'a FROM B '
    ...
    def test_parse_error_empty_string(self):
        with self.assertRaises(asn1tool.ParseError) as cm:
            asn1tool.parse_string('')
    ...
    def test_parse_error_begin_missing(self):
        with self.assertRaises(asn1tool.ParseError) as cm:
            asn1tool.parse_string('A DEFINITIONS ::= END')
    ...
    def test_parse_error_end_missing(self):
        with self.assertRaises(asn1tool.ParseError) as cm:
            asn1tool.parse_string('A DEFINITIONS ::= BEGIN')
    ...
    def test_parse_error_type_assignment_missing_assignment(self):
        with self.assertRaises(asn1tool.ParseError) as cm:
            asn1tool.parse_string('A DEFINITIONS ::= BEGIN A END')
    ...
    def test_parse_error_value_assignment_missing_assignment(self):
        with self.assertRaises(asn1tool.ParseError) as cm:
            asn1tool.parse_string('A DEFINITIONS ::= BEGIN a INTEGER END')
    ...
    def test_parse_error_end_missing_with_comments(self):
        with self.assertRaises(asn1tool.ParseError) as cm:
            asn1tool.parse_string('A DEFINITIONS -- g -- \n'
                                   '-- hhhh\n'
                                   '::= BEGIN ')
    ...
    def test_parse_error_missing_single_line_comment_end(self):
        with self.assertRaises(asn1tool.ParseError) as cm:
            asn1tool.parse_string('A DEFINITIONS ::= \n'
                                   'BEGIN -- END')
    ...
    def test_parse_x680_duplicated_enum_number_a_c_0(self):
        with self.assertRaises(asn1tool.ParseError) as cm:
            asn1tool.compile_string('A DEFINITIONS ::= BEGIN '
    ...
    def test_parse_encoding(self):
        asn1tool.parse_files('tests/files/foo.asn', encoding='ascii')
