...
class asn1toolCompileTest(unittest.TestCase):
    ...
    def test_pre_process_x683(self):
        ...
        with self.assertRaises(RecursionError):
            actual = asn1tool.pre_process_dict(deepcopy(X683))

            self.assertEqual(actual, X683_PP)

    def test_pre_process_parameterization(self):
        actual = asn1tool.pre_process_dict(deepcopy(PARAMETERIZATION))
        self.assertEqual(actual, PARAMETERIZATION_PP)

    def test_pre_process_time_types(self):
        actual = asn1tool.pre_process_dict(deepcopy(TIME_TYPES))
        self.assertEqual(actual, TIME_TYPES_PP)

    def test_unsupported_codec(self):
        with self.assertRaises(asn1tool.CompileError) as cm:
            asn1tool.compile_files('tests/files/foo.asn', 'bad_codec')
    ...
    def test_encoding(self):
        asn1tool.compile_files('tests/files/foo.asn', encoding='ascii')

    def test_import_imported(self):
        asn1tool.compile_files('tests/files/import_imported.asn')

    def test_missing_type(self):
        with self.assertRaises(asn1tool.CompileError) as cm:
            asn1tool.compile_string('A DEFINITIONS ::= BEGIN A ::= B END')

        self.assertEqual(str(cm.exception), "Type 'B' not found in module 'A'.")

    def test_missing_value(self):
        with self.assertRaises(asn1tool.CompileError) as cm:
            asn1tool.compile_string(
    ...
    def test_cache(self):
        ...
        foo = asn1tool.compile_files('tests/files/foo.asn',
                                      cache_dir=cache_dir)
        foo_cached = asn1tool.compile_files('tests/files/foo.asn',
                                             cache_dir=cache_dir)
