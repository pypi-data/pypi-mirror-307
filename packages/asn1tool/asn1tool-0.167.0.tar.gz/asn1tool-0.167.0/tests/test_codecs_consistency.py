...
class asn1toolCodecsConsistencyTest(asn1toolBaseTest):
    ...
    def encode_decode_all_codecs(self,
        ...
        for codec in CODECS:
            foos.append(asn1tool.compile_string(spec,
                                                 codec,
                                                 numeric_enums=numeric_enums))

        gser = asn1tool.compile_string(spec,
                                        'gser',
                                        numeric_enums=numeric_enums)
    ...
    def encode_decode_codecs(self,
        ...
        for codec, encoded_message in zip(ALL_CODECS, encoded):
            foo = asn1tool.compile_string(spec, codec)
    ...
    def encode_decode_codecs_file(self,
                                  ...
                                  encoded):
        for codec, encoded_message in zip(ALL_CODECS, encoded):
            foo = asn1tool.compile_files(filename, codec)
            self.encode_decode_codec(foo,
                                     codec,
                                     type_name,
                                     decoded,
    ...
    def decode_codecs(self, type_spec, decoded, encoded, numeric_enums=False):
        ...
        for codec, encoded_message in zip(CODECS, encoded):
            foo = asn1tool.compile_string(spec, codec, numeric_enums=numeric_enums)
    ...
    def test_named_numbers_errors(self):
        ...
        for codec in ALL_CODECS:
            foo = asn1tool.compile_files('tests/files/named_numbers.asn',
                                          codec)

            for type_name, decoded, message in datas:
                with self.assertRaises(asn1tool.ConstraintsError) as cm:
    ...
    def test_programming_types(self):
        ...
        for codec in CODECS:
            specs.append(asn1tool.compile_files(
    ...
    def test_parameterization(self):
        ...
        for codec in CODECS:
            specs.append(asn1tool.compile_dict(deepcopy(PARAMETERIZATION),
                                                codec))
