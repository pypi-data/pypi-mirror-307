...
CODECS_AND_MODULES = [
    ('oer', asn1tool.source.c.oer),
    ('uper', asn1tool.source.c.uper)
]


class asn1toolCSourceTest(unittest.TestCase):
    ...
    def test_compile_error_unsupported_type(self):
        for codec, module in CODECS_AND_MODULES:
            ...
            with self.assertRaises(asn1tool.errors.Error) as cm:
    ...
    def test_compile_error_oer_real_not_ieee754(self):
        ...
        with self.assertRaises(asn1tool.errors.Error) as cm:
            asn1tool.source.c.oer.generate(foo, 'foo')
