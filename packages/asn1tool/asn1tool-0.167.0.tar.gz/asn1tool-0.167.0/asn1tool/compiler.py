...
class Specification(object):
    ...
    :func:`~asn1tool.compile_files()`,
    :func:`~asn1tool.compile_string()` and
    :func:`~asn1tool.compile_dict()`.
    ...
    def decode_with_length(self, name, data, check_constraints=False):
        """Same as :func:`~asn1tool.compiler.Specification.decode`, but also
...
def compile_dict(specification,
    ...
    :class:`~asn1tool.compiler.Specification` object that can be used
    ...
    >>> foo = asn1tool.compile_dict(asn1tool.parse_files('foo.asn'))
...
def compile_string(string,
    ...
    ...     foo = asn1tool.compile_string(fin.read())
...
def compile_files(filenames,
    ...
    >>> foo = asn1tool.compile_files('foo.asn')

    Give `cache_dir` as a string to use a cache.

    >>> foo = asn1tool.compile_files('foo.asn', cache_dir='my_cache')
