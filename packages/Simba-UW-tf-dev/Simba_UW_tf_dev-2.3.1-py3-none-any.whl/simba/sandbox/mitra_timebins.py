...
def mitra_timebins(data_dir: Union[str, os.PathLike],
    ...
    for file_cnt, file_path in enumerate(data_paths):
        ...
        end_frm = np.ceil((fps * 60) * 10)
        frm_win_size = np.ceil(fps * window_size)
        video_bins_pre = np.arange(start_frm, 0, -frm_win_size)
        video_bins_pre = np.append(video_bins_pre, 0).astype(np.int32)
        video_bins_post = np.arange(start_frm, end_frm, frm_win_size).astype(np.int32)
...
data_dir = r"D:\troubleshooting\mitra\project_folder\csv\machine_results\straub_tail\test"
window_size = 60
video_video_path = r"D:\troubleshooting\mitra\project_folder\logs\video_info.csv"
mitra_timebins(data_dir=data_dir, frm_path=frm_path, window_size=window_size, video_video_path=video_video_path, clf_name='straub_tail')